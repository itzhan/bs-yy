  ...
  <div v-if="canView" class="front-list-page">
    <div class="breadcrumb-container">
      <el-breadcrumb class="breadcrumb-box" :separator="'>'">
        ...
        <el-breadcrumb-item class="breadcrumb-item item-module">
          <a href="javascript:void(0);">会员卡套餐</a>
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    ...
    <el-form :inline="true" :model="formSearch" class="search-container">
      <el-form-item class="search-item">
        ...
        <el-input
          v-model="formSearch.packagename"
          placeholder="请输入套餐名称"
      ...
      <div class="range-filter">
        <el-form-item class="search-item">
          <span class="label">价格开始</span>
          <el-input-number
            v-model="formSearch.packageprice_start"
            placeholder="价格开始"
            :controls="false"
          />
        </el-form-item>
        <el-form-item class="search-item">
          <span class="label">价格结束</span>
          <el-input-number
            v-model="formSearch.packageprice_end"
            placeholder="价格结束"
            :controls="false"
          />
        </el-form-item>
    ...
    <div class="filter-container">
      ...
      <div class="options">
        <div
          class="option"
          :class="selectState.packagetype === -1 ? 'active' : ''"
          @click="selectDictOption('packagetype', -1)"
        >全部</div>
        <div
          class="option"
          :class="selectState.packagetype === 0 ? 'active' : ''"
          @click="selectDictOption('packagetype', 0)"
        >月卡</div>
        <div
          class="option"
          :class="selectState.packagetype === 1 ? 'active' : ''"
        ...
        <div
          class="option"
          :class="selectState.packagetype === 2 ? 'active' : ''"
          @click="selectDictOption('packagetype', 2)"
        >年卡</div>
      </div>
    </div>
    </div>
    ...
    <section v-if="showHotRecommend && hotRecommend.length" class="area-container hot-recommend">
      <div class="area-title-box">
        <h2 class="area-title">会员卡套餐热门推荐</h2>
        <div class="area-subhead">MEMBERSHIPPACKAGE RECOMMEND</div>
      </div>
...
const canCreate = !false && useAuth('membershippackage', '新增')
...
const detailRoute = '/index/membershippackageDetail'
...
function handleAdd() {
  const query: Record<string, any> = {}
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/membershippackageAdd', query })
}
...
function toDetail(row: Record<string, any>) {
  ...
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/membershippackageDetail', query })
}
...
async function fetchList(page = pageIndex.value) {
  ...
  try {
    ...
    if (token.value) {
      const preferParam = resolveCatePreferParam('membershippackage')
      if (preferParam) {
    ...
    if (formSearch.packagename) {
      query.packagename = '%' + formSearch.packagename + '%'
    }
    ...
    if (formSearch.packageprice_start) query.packageprice_start = formSearch.packageprice_start
    if (formSearch.packageprice_end) query.packageprice_end = formSearch.packageprice_end
    ...
    const endpoint = centerType.value ? 'membershippackage/page' : 'membershippackage/list'
...
async function fetchHotRecommend(force = false) {
  ...
  try {
    const endpoint = 'membershippackage/' + (token.value ? 'autoSort2' : 'autoSort')
