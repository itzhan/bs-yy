...
async function loadDetail(id: number) {
  try {
    resetReadState()
    const res: any = await http.get('discusscourse/detail/' + id)
...
async function handleSubmit() {
  ...
  try {
    ...
    const api = isEdit.value && !isCross.value ? 'discusscourse/update' : 'discusscourse/add'
    const payload = { ...form }
