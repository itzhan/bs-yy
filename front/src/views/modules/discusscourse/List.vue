  ...
  <div v-if="canView" class="front-list-page">
    <div class="breadcrumb-container">
      <el-breadcrumb class="breadcrumb-box" :separator="'>'">
        ...
        <el-breadcrumb-item class="breadcrumb-item item-module">
          <a href="javascript:void(0);">健身课程评论表</a>
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    ...
    <section v-if="showHotRecommend && hotRecommend.length" class="area-container hot-recommend">
      <div class="area-title-box">
        <h2 class="area-title">健身课程评论表热门推荐</h2>
        <div class="area-subhead">DISCUSSCOURSE RECOMMEND</div>
      </div>
...
const canViewPermission = useAuth('discusscourse', '查看')
const canCreate = !false && useAuth('discusscourse', '新增')
...
const detailRoute = '/index/discusscourseDetail'
...
function handleAdd() {
  const query: Record<string, any> = {}
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/discusscourseAdd', query })
}
...
function toDetail(row: Record<string, any>) {
  ...
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/discusscourseDetail', query })
}
...
async function fetchList(page = pageIndex.value) {
  ...
  try {
    ...
    if (token.value) {
      const preferParam = resolveCatePreferParam('discusscourse')
      if (preferParam) {
    ...
    const endpoint = centerType.value ? 'discusscourse/page' : 'discusscourse/list'
...
async function fetchHotRecommend(force = false) {
  ...
  try {
    const endpoint = 'discusscourse/' + (token.value ? 'autoSort2' : 'autoSort')
