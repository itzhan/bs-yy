  ...
  <div v-if="canView" class="front-list-page">
    <div class="breadcrumb-container">
      <el-breadcrumb class="breadcrumb-box" :separator="'>'">
        ...
        <el-breadcrumb-item class="breadcrumb-item item-module">
          <a href="javascript:void(0);">交流分类</a>
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    ...
    <el-form :inline="true" :model="formSearch" class="search-container">
      <el-form-item class="search-item">
        ...
        <el-input
          v-model="formSearch.sharetypename"
          placeholder="请输入分类名称"
    ...
    <section v-if="showHotRecommend && hotRecommend.length" class="area-container hot-recommend">
      <div class="area-title-box">
        <h2 class="area-title">交流分类热门推荐</h2>
        <div class="area-subhead">SHARETYPE RECOMMEND</div>
      </div>
...
const canViewPermission = useAuth('sharetype', '查看')
const canCreate = !false && useAuth('sharetype', '新增')
const canView = computed(() => canViewPermission.value || !token.value)

const imageField = null
const titleField = 'sharetypename'
const summaryFields = [
...
const detailRoute = '/index/sharetypeDetail'
...
function handleAdd() {
  const query: Record<string, any> = {}
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/sharetypeAdd', query })
}
...
function toDetail(row: Record<string, any>) {
  ...
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/sharetypeDetail', query })
}
...
async function fetchList(page = pageIndex.value) {
  ...
  try {
    ...
    if (token.value) {
      const preferParam = resolveCatePreferParam('sharetype')
      if (preferParam) {
    ...
    if (formSearch.sharetypename) {
      query.sharetypename = '%' + formSearch.sharetypename + '%'
    }
    ...
    const endpoint = centerType.value ? 'sharetype/page' : 'sharetype/list'
...
async function fetchHotRecommend(force = false) {
  ...
  try {
    const endpoint = 'sharetype/' + (token.value ? 'autoSort2' : 'autoSort')
