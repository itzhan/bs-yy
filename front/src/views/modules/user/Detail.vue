        ...
        <div class="detail-content">
          ...
          <div class="infos">
            ...
            <div class="info-item">
              ...
              <span class="text">{{ info.userpassword }}</span>
            </div>
            <div class="info-item">
            ...
            <div class="info-item">
              ...
              <span class="text">{{ info.cardno }}</span>
            </div>
            <div class="info-item">
              <span class="label">会员等级：</span>
              <span class="text">{{ info.memberlevel }}</span>
            </div>
            <div class="info-item">
              <span class="label">会员到期时间：</span>
              <span class="text">{{ info.expiretime }}</span>
            </div>
            <div class="info-item">
              <span class="label">剩余课时：</span>
              <span class="text">{{ info.remainingcourses }}</span>
            </div>
          </div>
    ...
    <el-dialog v-model="chatVisible" :title="`与 ${fname} 私聊`" width="600px" @close="handleChatClose">
      ...
      <template #footer>
        <div style="display:flex; align-items:center; width:100%;">
          ...
          <el-button style="margin-left:6px;" type="primary" @click="sendCurrentLabel">发送用户</el-button>
        </div>
      </template>
    </el-dialog>
  </div>
...
const tableButtonsPermission = useTableButtons('user')
...
async function sendCurrentLabel(){
  ...
  if (firstImg) {
    ...
    const payload: any = { uid: myid, fid: fid.value, content: '用户', format: 1 }
    if (chatRoleTable) {
      payload.tablename = chatRoleTable
    }
    const res: any = await http.post('chatmessage/add', payload)
