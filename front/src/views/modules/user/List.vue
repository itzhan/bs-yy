  ...
  <div v-if="canView" class="front-list-page">
    ...
    <el-form :inline="true" :model="formSearch" class="search-container">
      <el-form-item class="search-item">
        ...
        <el-input
          v-model="formSearch.useraccount"
          placeholder="请输入账号"
      ...
      <div class="range-filter">
        <el-form-item class="search-item">
          <span class="label">年龄开始</span>
          <el-input-number
            v-model="formSearch.age_start"
            placeholder="年龄开始"
            :controls="false"
          />
        </el-form-item>
        <el-form-item class="search-item">
          <span class="label">年龄结束</span>
          <el-input-number
            v-model="formSearch.age_end"
            placeholder="年龄结束"
            :controls="false"
          />
        </el-form-item>
    ...
    <div class="filter-container">
      ...
      <div class="options">
        <div
          class="option"
          :class="selectState.sex === -1 ? 'active' : ''"
          @click="selectDictOption('sex', -1)"
        >全部</div>
        <div
          class="option"
          :class="selectState.sex === 0 ? 'active' : ''"
          @click="selectDictOption('sex', 0)"
        >男</div>
        <div
          class="option"
          :class="selectState.sex === 1 ? 'active' : ''"
    ...
    <div v-if="!mapConfig.enabled" class="list-container">
      ...
      >
        ...
        <div class="info-box">
          ...
          <div class="info-item">
            <span class="label">账号：</span>
            <span class="value">{{ formatField(item.useraccount) }}</span>
          </div>
          <div class="info-item">
            <span class="label">性别：</span>
            <span class="value">{{ formatField(item.sex) }}</span>
          </div>
          <div class="info-item">
            <span class="label">年龄：</span>
            <span class="value">{{ formatField(item.age) }}</span>
          </div>
          <div class="info-item">
            <span class="label">手机号码：</span>
            <span class="value">{{ formatField(item.phone) }}</span>
          </div>
          <div class="info-item">
            <span class="label">余额：</span>
            <span class="value">{{ formatField(item.money) }}</span>
          </div>
          <div class="info-item">
    ...
    <section v-if="showHotRecommend && hotRecommend.length" class="area-container hot-recommend">
      <div class="area-title-box">
        <h2 class="area-title">用户热门推荐</h2>
        <div class="area-subhead">USER RECOMMEND</div>
      </div>
...
const canViewPermission = useAuth('user', '查看')
const canCreate = !false && useAuth('user', '新增')
...
const detailRoute = '/index/userDetail'
...
function handleAdd() {
  const query: Record<string, any> = {}
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/userAdd', query })
}
...
function toDetail(row: Record<string, any>) {
  ...
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/userDetail', query })
}
...
async function fetchList(page = pageIndex.value) {
  ...
  try {
    ...
    if (token.value) {
      const preferParam = resolveCatePreferParam('user')
      if (preferParam) {
    ...
    if (formSearch.useraccount) {
      query.useraccount = '%' + formSearch.useraccount + '%'
    }
    if (formSearch.name) {
      query.name = '%' + formSearch.name + '%'
    }

    if (formSearch.sex) {
      query.sex = formSearch.sex
    }

    if (formSearch.age_start) query.age_start = formSearch.age_start
    if (formSearch.age_end) query.age_end = formSearch.age_end
    ...
    const endpoint = centerType.value ? 'user/page' : 'user/list'
...
async function fetchHotRecommend(force = false) {
  ...
  try {
    const endpoint = 'user/' + (token.value ? 'autoSort2' : 'autoSort')
