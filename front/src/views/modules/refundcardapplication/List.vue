  ...
  <div v-if="canView" class="front-list-page">
    <div class="breadcrumb-container">
      <el-breadcrumb class="breadcrumb-box" :separator="'>'">
        ...
        <el-breadcrumb-item class="breadcrumb-item item-module">
          <a href="javascript:void(0);">办卡记录退款</a>
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
      ...
      >
        ...
        <div class="info-box">
          ...
          <div class="info-item">
            ...
            <span class="value">{{ formatField(item.refundreason) }}</span>
          </div>
          <div class="info-item">
    ...
    <section v-if="showHotRecommend && hotRecommend.length" class="area-container hot-recommend">
      <div class="area-title-box">
        <h2 class="area-title">办卡记录退款热门推荐</h2>
        <div class="area-subhead">REFUNDCARDAPPLICATION RECOMMEND</div>
      </div>
...
const canViewPermission = useAuth('refundcardapplication', '查看')
const canCreate = !false && useAuth('refundcardapplication', '新增')
...
const detailRoute = '/index/refundcardapplicationDetail'
...
function handleAdd() {
  const query: Record<string, any> = {}
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/refundcardapplicationAdd', query })
}
...
function toDetail(row: Record<string, any>) {
  ...
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/refundcardapplicationDetail', query })
}
...
async function fetchList(page = pageIndex.value) {
  ...
  try {
    ...
    if (token.value) {
      const preferParam = resolveCatePreferParam('refundcardapplication')
      if (preferParam) {
    ...
    const endpoint = centerType.value ? 'refundcardapplication/page' : 'refundcardapplication/list'
...
async function fetchHotRecommend(force = false) {
  ...
  try {
    const endpoint = 'refundcardapplication/' + (token.value ? 'autoSort2' : 'autoSort')
