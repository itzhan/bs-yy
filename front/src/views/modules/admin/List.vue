  ...
  <div v-if="canView" class="front-list-page">
    ...
    <el-form :inline="true" :model="formSearch" class="search-container">
      <el-form-item class="search-item">
        ...
        <el-input
          v-model="formSearch.adminaccount"
          placeholder="请输入账号"
    ...
    <section v-if="showHotRecommend && hotRecommend.length" class="area-container hot-recommend">
      <div class="area-title-box">
        <h2 class="area-title">管理员热门推荐</h2>
        <div class="area-subhead">ADMIN RECOMMEND</div>
      </div>
...
const canViewPermission = useAuth('admin', '查看')
const canCreate = !false && useAuth('admin', '新增')
const canView = computed(() => canViewPermission.value || !token.value)

const imageField = 'image'
const titleField = 'adminaccount'
const summaryFields = [
...
const detailRoute = '/index/adminDetail'
...
function handleAdd() {
  const query: Record<string, any> = {}
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/adminAdd', query })
}
...
function toDetail(row: Record<string, any>) {
  ...
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/adminDetail', query })
}
...
async function fetchList(page = pageIndex.value) {
  ...
  try {
    ...
    if (token.value) {
      const preferParam = resolveCatePreferParam('admin')
      if (preferParam) {
    ...
    if (formSearch.adminaccount) {
      query.adminaccount = '%' + formSearch.adminaccount + '%'
    }
    if (formSearch.role) {
      query.role = '%' + formSearch.role + '%'
    }
    ...
    const endpoint = centerType.value ? 'admin/page' : 'admin/list'
...
async function fetchHotRecommend(force = false) {
  ...
  try {
    const endpoint = 'admin/' + (token.value ? 'autoSort2' : 'autoSort')
