        ...
        <div class="detail-content">
          ...
          <div class="infos">
            <div class="info-item">
              ...
              <span class="text">{{ info.adminaccount }}</span>
            </div>
            <div class="info-item">
              <span class="label">密码：</span>
              <span class="text">{{ info.adminpassword }}</span>
            </div>
            <div class="info-item">
    ...
    <el-dialog v-model="chatVisible" :title="`与 ${fname} 私聊`" width="600px" @close="handleChatClose">
      ...
      <template #footer>
        <div style="display:flex; align-items:center; width:100%;">
          ...
          <el-button style="margin-left:6px;" type="primary" @click="sendCurrentLabel">发送管理员</el-button>
        </div>
      </template>
    </el-dialog>
  </div>
...
const tableButtonsPermission = useTableButtons('admin')
...
async function loadDetail(id: number) {
  ...
  const res: any = await http.get('admin/detail/' + id)
...
const chatAccountField = 'adminaccount'
const chatRoleTable = 'admin'
...
async function sendCurrentLabel(){
  ...
  if (firstImg) {
    ...
    const payload: any = { uid: myid, fid: fid.value, content: '管理员', format: 1 }
    if (chatRoleTable) {
      payload.tablename = chatRoleTable
    }
    const res: any = await http.post('chatmessage/add', payload)
