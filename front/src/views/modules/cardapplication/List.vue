  ...
  <div v-if="canView" class="front-list-page">
    <div class="breadcrumb-container">
      <el-breadcrumb class="breadcrumb-box" :separator="'>'">
        ...
        <el-breadcrumb-item class="breadcrumb-item item-module">
          <a href="javascript:void(0);">办卡记录</a>
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    ...
    <div class="filter-container">
      ...
      <div class="options">
        <div
          class="option"
          :class="selectState.ispay === -1 ? 'active' : ''"
          @click="selectDictOption('ispay', -1)"
        >全部</div>
        <div
          class="option"
          :class="selectState.ispay === 0 ? 'active' : ''"
          @click="selectDictOption('ispay', 0)"
        >未支付</div>
        <div
          class="option"
          :class="selectState.ispay === 1 ? 'active' : ''"
      ...
      <div class="options">
        <div
          class="option"
          :class="selectState.orderstatus === -1 ? 'active' : ''"
          @click="selectDictOption('orderstatus', -1)"
        >全部</div>
        <div
          class="option"
          :class="selectState.orderstatus === 0 ? 'active' : ''"
          @click="selectDictOption('orderstatus', 0)"
        >未支付</div>
        <div
          class="option"
          :class="selectState.orderstatus === 1 ? 'active' : ''"
        ...
        <div
          class="option"
          :class="selectState.orderstatus === 2 ? 'active' : ''"
          @click="selectDictOption('orderstatus', 2)"
        >已取消</div>
        <div
          class="option"
          :class="selectState.orderstatus === 3 ? 'active' : ''"
          @click="selectDictOption('orderstatus', 3)"
        >已退款</div>
        <div
          class="option"
    ...
    <div v-if="!mapConfig.enabled" class="list-container">
      ...
      >
        ...
        <div class="info-box">
          ...
          <div class="info-item">
            <span class="label">套餐类型：</span>
            <span class="value">{{ formatField(item.packagetype) }}</span>
          </div>
          <div class="info-item">
            <span class="label">价格：</span>
            <span class="value">{{ formatField(item.packageprice) }}</span>
          </div>
          <div class="info-item">
            <span class="label">包含课时：</span>
            <span class="value">{{ formatField(item.includedcourses) }}</span>
          </div>
          <div class="info-item">
          ...
          <div class="info-item">
            <span class="label">是否支付：</span>
            <span class="value">{{ formatField(item.ispay) }}</span>
          </div>
          <div class="info-item">
            <span class="label">状态：</span>
            <span class="value">{{ formatField(item.orderstatus) }}</span>
          </div>
    ...
    <section v-if="showHotRecommend && hotRecommend.length" class="area-container hot-recommend">
      <div class="area-title-box">
        <h2 class="area-title">办卡记录热门推荐</h2>
        <div class="area-subhead">CARDAPPLICATION RECOMMEND</div>
      </div>
...
const canCreate = !false && useAuth('cardapplication', '新增')
const canView = computed(() => canViewPermission.value || !token.value)

const imageField = 'packageimage'
...
const categoryConfig = { field: 'packagename', label: '套餐名称', table: 'membershippackage', column: 'packagename', imageField: 'packageimage' }
...
const detailRoute = '/index/cardapplicationDetail'
...
function handleAdd() {
  const query: Record<string, any> = {}
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/cardapplicationAdd', query })
}
...
function toDetail(row: Record<string, any>) {
  ...
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/cardapplicationDetail', query })
}
...
async function fetchList(page = pageIndex.value) {
  ...
  try {
    ...
    if (token.value) {
      const preferParam = resolveCatePreferParam('cardapplication')
      if (preferParam) {
    ...
    if (formSearch.packagetype) {
      query.packagetype = '%' + formSearch.packagetype + '%'
    }

    if (formSearch.ispay) {
      query.ispay = formSearch.ispay
    }
    if (formSearch.orderstatus) {
      query.orderstatus = formSearch.orderstatus
    }
    ...
    const endpoint = centerType.value ? 'cardapplication/page' : 'cardapplication/list'
...
async function fetchHotRecommend(force = false) {
  ...
  try {
    const endpoint = 'cardapplication/' + (token.value ? 'autoSort2' : 'autoSort')
