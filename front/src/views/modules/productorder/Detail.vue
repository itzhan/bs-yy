        ...
        <div class="detail-content">
          ...
          <div class="actions">
            ...
            <el-button
              size="small"
              type="primary"
              v-if="hasActionPermission('取消商品订单')"
              :disabled="isCrossDisabled('取消商品订单')"
              :title="crossDisabledReason('取消商品订单') || undefined"
              @click="onCross('取消商品订单')"
            >取消商品订单</el-button>
            <el-button
              size="small"
              type="primary"
...
const tableButtonsPermission = useTableButtons('productorder')
...
const discussTableName = 'discussproductorder'
...
const payType = ref('')
const payAmountField = 'productprice'
...
async function submitPay() {
  ...
  try {
    ...
    const res: any = await http.post('productorder/update', payload)
    if (res?.code === 0) {
      info.ispay = '已支付'
...
async function loadDetail(id: number) {
  ...
  const res: any = await http.get('productorder/detail/' + id)
...
async function loadDiscuss(page = discussPage.value) {
  ...
  try {
    ...
    const res: any = await http.get('discussproductorder/list', { params })
    const data = res?.data || {}
...
async function submitDiscuss() {
  ...
  try {
    ...
    const res: any = await http.post('discussproductorder/add', payload)
    if (res?.code === 0) {
      discussForm.content = '<p></p>'
      discussPage.value = 1
      info.discussnum = Number(info.discussnum || 0) + 1
      try { await http.post('productorder/update', info) } catch(e) {}
      await loadDiscuss(1)
...
async function updateDiscussCounters(item: any) {
  try {
    await http.post('discussproductorder/update', {
      id: item.id,
...
async function deleteDiscuss(item: any) {
  ...
  setDiscussActionLoading(item, true)
  try {
    const res: any = await http.post('discussproductorder/delete', [item.id])
    if (res?.code === 0) {
...
const crossButtonsConfig: CrossButtonConfig[] = [
  {
    name: '取消商品订单',
    targetTable: 'cancelproductorder',
    tips: '商品订单已取消',
  ...
  {
    name: '退款',
    targetTable: 'refundproductorder',
...
async function onCross(name: string){
  ...
  try {
    ...
    const inlineOnly = !targetTableName || targetTableName === 'productorder'
    ...
    if (targetTableName === 'cart' && name === '加入购物车') {
      try {
        ...
        const payload: Record<string, any> = {
          tablename: (localStorage.getItem('crossTable') || 'productorder') || 'productorder',
...
async function inlineCrossAction(
  ...
  try {
    if (statusKey && statusTarget) {
      ...
      await http.post('productorder/update', payload)
    ...
    const refId = (info as any)?.crossrefid || (target === 'productorder' ? (info as any)?.id : undefined)
    if (!refId) {
