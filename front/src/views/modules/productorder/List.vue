  ...
  <div v-if="canView" class="front-list-page">
      ...
      >
        ...
        <div class="info-box">
          ...
          <div class="info-item">
            ...
            <span class="value">{{ formatField(item.producttype) }}</span>
          </div>
          <div class="info-item">
            <span class="label">商品单价：</span>
            <span class="value">{{ formatField(item.productprice) }}</span>
          </div>
          <div class="info-item">
            <span class="label">购买数量：</span>
            <span class="value">{{ formatField(item.quantity) }}</span>
          </div>
          <div class="info-item">
            <span class="label">订单总价：</span>
            <span class="value">{{ formatField(item.totalprice) }}</span>
          </div>
          <div class="info-item">
    ...
    <section v-if="showHotRecommend && hotRecommend.length" class="area-container hot-recommend">
      <div class="area-title-box">
        <h2 class="area-title">商品订单热门推荐</h2>
        <div class="area-subhead">PRODUCTORDER RECOMMEND</div>
      </div>
...
const canViewPermission = useAuth('productorder', '查看')
const canCreate = !false && useAuth('productorder', '新增')
...
const categoryConfig = { field: 'productname', label: '商品名称', table: 'product', column: 'productname', imageField: 'productimage' }
const showLike = false
const showFavorite = false
const showClick = false
const showDiscuss = true
...
const detailRoute = '/index/productorderDetail'
...
function handleAdd() {
  const query: Record<string, any> = {}
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/productorderAdd', query })
}
...
function toDetail(row: Record<string, any>) {
  ...
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/productorderDetail', query })
}
...
async function fetchList(page = pageIndex.value) {
  ...
  try {
    ...
    if (token.value) {
      const preferParam = resolveCatePreferParam('productorder')
      if (preferParam) {
    ...
    const endpoint = centerType.value ? 'productorder/page' : 'productorder/list'
...
async function fetchHotRecommend(force = false) {
  ...
  try {
    const endpoint = 'productorder/' + (token.value ? 'autoSort2' : 'autoSort')
