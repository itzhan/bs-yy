        ...
        <div class="detail-content">
          ...
          <div class="infos">
            <div class="info-item">
              ...
              <span class="text">{{ info.sharetitle }}</span>
            </div>
            <div class="info-item">
              <span class="label">内容：</span>
              <div class="text rich-text" v-html="info.sharecontent || ''"></div>
            </div>
            <div class="info-item">
              <span class="label">分类：</span>
              <span class="text">{{ info.sharetype }}</span>
            </div>
          </div>
...
const canViewPermission = useAuth('share', '查看')
const tableButtonsPermission = useTableButtons('share')
...
const discussTableName = 'discussshare'
// 页面标题：优先使用后端富化的稳定标题字段
const titleField = 'sharetitle'
...
async function submitAudit() {
  ...
  try {
    ...
    const url = 'share/audit/batch?auditstatus=' + status + '&auditreply=' + reply
...
async function loadDetail(id: number) {
  ...
  const res: any = await http.get('share/detail/' + id)
...
async function like(type: '1'|'2'){
  await http.get(`share/like/` + info.id, { params: { type } })
...
async function toggleFav(){
  ...
  if (fav.value) {
    ...
    try {
      if (!favId.value) {
        const st: any = await http.get('social/list', { params: { page: 1, limit: 1, tablename: 'share', refid: info.id, type: '1' } })
      ...
      favId.value = null
      try { await http.post('share/update', info) } catch(e) {}
      await refreshSocialStatus(info.id)
    }
  } else {
    // 添加收藏：写 social
    const payload: any = { tablename: 'share', refid: info.id, type: '1' }
    // 供后端展示，可带上名称与图片
    try { payload.name = String(info.id) } catch (e) {}
    const coverField = 'shareimage'
...
async function doLike(){
  ...
  const payload: any = { tablename: 'share', refid: info.id, type: '21' }
  try { payload.name = String(info.id) } catch (e) {}
  const coverField2 = 'shareimage'
...
async function cancelLike(){
  ...
  try {
    if (!likeId.value) {
      const st: any = await http.get('social/list', { params: { page: 1, limit: 1, tablename: 'share', refid: info.id, type: '21' } })
      const arr = st?.data?.list || []
...
async function doStep(){
  if (liked.value || stepped.value) return
  const payload: any = { tablename: 'share', refid: info.id, type: '22' }
  try { payload.name = String(info.id) } catch (e) {}
  const coverField3 = 'shareimage'
...
async function cancelStep(){
  ...
  try {
    if (!stepId.value) {
      const st: any = await http.get('social/list', { params: { page: 1, limit: 1, tablename: 'share', refid: info.id, type: '22' } })
      const arr = st?.data?.list || []
...
async function loadDiscuss(page = discussPage.value) {
  ...
  try {
    ...
    const res: any = await http.get('discussshare/list', { params })
    const data = res?.data || {}
...
async function submitDiscuss() {
  ...
  try {
    ...
    const res: any = await http.post('discussshare/add', payload)
    if (res?.code === 0) {
...
async function updateDiscussCounters(item: any) {
  try {
    await http.post('discussshare/update', {
      id: item.id,
...
async function deleteDiscuss(item: any) {
  ...
  setDiscussActionLoading(item, true)
  try {
    const res: any = await http.post('discussshare/delete', [item.id])
    if (res?.code === 0) {
...
async function refreshSocialStatus(refid: number){
  ...
  try {
    const st: any = await http.get('social/list', { params: { page: 1, limit: 1, tablename: 'share', refid, type: '1' } })
    const arr = st?.data?.list || []
    fav.value = arr.length > 0
    favId.value = arr.length ? arr[0].id : null
  } catch(e) {}
  try {
    const stLike: any = await http.get('social/list', { params: { page: 1, limit: 1, tablename: 'share', refid, type: '21' } })
    const likeArr = stLike?.data?.list || []
    liked.value = likeArr.length > 0
    likeId.value = likeArr.length ? likeArr[0].id : null
  } catch(e) {}
  try {
    const stStep: any = await http.get('social/list', { params: { page: 1, limit: 1, tablename: 'share', refid, type: '22' } })
