  ...
  <div class="form-container front-add-page">
    ...
    <el-form-item label="内容" prop="sharecontent">
      <el-input
        ...
        :rows="6"
        :placeholder="'请输入内容'"
        :readonly="read.sharecontent"
      />
    </el-form-item>
    <el-form-item label="配图" prop="shareimage">
      <UploadField
        v-model="form.shareimage"
        upload-type="image"
        :readonly="read.shareimage"
      />
    </el-form-item>
    <el-form-item label="分类" prop="sharetype">
      <el-select v-model="form.sharetype" placeholder="请选择分类" clearable :disabled="read.sharetype">
        <el-option
          v-for="opt in categoryOptions.sharetype"
          :key="opt"
          :label="opt"
          :value="opt"
        />
...
const rules = reactive<FormRules>({
    ...
    'shareimage': [
      { required: false, message: '请输入配图', trigger: 'blur' }
    ],
    'sharetype': [
...
async function loadCategoryOptions() {
  try {
    const res: any = await http.get('sharetype/list', { params: { page: 1, limit: 1000, sort: 'id', order: 'asc' } })
    const list = Array.isArray(res?.data?.list) ? res.data.list : Array.isArray(res?.data?.data?.list) ? res.data.data.list : []
    categoryOptions['sharetype'] = list
      .map((item: any) => item?.['sharetypename'])
      .filter(Boolean)
  } catch (error) {
    categoryOptions['sharetype'] = []
  }
}
...
async function handleSubmit() {
  ...
  try {
    ...
    const api = isEdit.value && !isCross.value ? 'share/update' : 'share/add'
    const payload = { ...form }
