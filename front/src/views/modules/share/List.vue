  ...
  <div v-if="canView" class="front-list-page">
    <div class="breadcrumb-container">
      <el-breadcrumb class="breadcrumb-box" :separator="'>'">
        ...
        <el-breadcrumb-item class="breadcrumb-item item-module">
          <a href="javascript:void(0);">会员交流</a>
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
      ...
      >
        ...
        <div class="info-box">
          ...
          <div class="info-item">
            ...
            <span class="value">{{ formatField(item.sharetype) }}</span>
          </div>
          <div class="info-item">
          ...
          <div class="info-item">
            ...
            <span class="value">{{ formatField(item.clicknum) }}</span>
          </div>
          <div class="info-item">
    ...
    <section v-if="showHotRecommend && hotRecommend.length" class="area-container hot-recommend">
      <div class="area-title-box">
        <h2 class="area-title">会员交流热门推荐</h2>
        <div class="area-subhead">SHARE RECOMMEND</div>
      </div>
...
const canCreate = !false && useAuth('share', '新增')
const canView = computed(() => canViewPermission.value || !token.value)

const imageField = 'shareimage'
...
const categoryConfig = { field: 'sharetype', label: '分类', table: 'sharetype', column: 'sharetypename' }
const showLike = true
...
const detailRoute = '/index/shareDetail'
...
function handleAdd() {
  const query: Record<string, any> = {}
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/shareAdd', query })
}
...
function toDetail(row: Record<string, any>) {
  ...
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/shareDetail', query })
}
...
async function fetchList(page = pageIndex.value) {
  ...
  try {
    ...
    if (token.value) {
      const preferParam = resolveCatePreferParam('share')
      if (preferParam) {
    ...
    if (formSearch.sharetitle) {
      query.sharetitle = '%' + formSearch.sharetitle + '%'
    }
    ...
    const endpoint = centerType.value ? 'share/page' : 'share/list'
...
async function fetchHotRecommend(force = false) {
  ...
  try {
    const endpoint = 'share/' + (token.value ? 'autoSort2' : 'autoSort')
