  ...
  <div v-if="canView" class="front-list-page">
    <div class="breadcrumb-container">
      <el-breadcrumb class="breadcrumb-box" :separator="'>'">
        ...
        <el-breadcrumb-item class="breadcrumb-item item-module">
          <a href="javascript:void(0);">公告分类</a>
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    ...
    <section v-if="showHotRecommend && hotRecommend.length" class="area-container hot-recommend">
      <div class="area-title-box">
        <h2 class="area-title">公告分类热门推荐</h2>
        <div class="area-subhead">NEWSTYPE RECOMMEND</div>
      </div>
...
const canViewPermission = useAuth('newstype', '查看')
const canCreate = !false && useAuth('newstype', '新增')
const canView = computed(() => canViewPermission.value || !token.value)

const imageField = null
const titleField = 'typename'
const summaryFields = [
...
const detailRoute = '/index/newstypeDetail'
...
function handleAdd() {
  const query: Record<string, any> = {}
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/newstypeAdd', query })
}
...
function toDetail(row: Record<string, any>) {
  ...
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/newstypeDetail', query })
}
...
async function fetchList(page = pageIndex.value) {
  ...
  try {
    ...
    if (token.value) {
      const preferParam = resolveCatePreferParam('newstype')
      if (preferParam) {
    ...
    if (formSearch.typename) {
      query.typename = '%' + formSearch.typename + '%'
    }
    ...
    const endpoint = centerType.value ? 'newstype/page' : 'newstype/list'
...
async function fetchHotRecommend(force = false) {
  ...
  try {
    const endpoint = 'newstype/' + (token.value ? 'autoSort2' : 'autoSort')
