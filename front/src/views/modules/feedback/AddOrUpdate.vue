  ...
  <div class="form-container front-add-page">
    ...
    <el-form-item label="标题" prop="feedbacktitle">
      <el-input
        ...
        :placeholder="'请输入标题'"
        :readonly="read.feedbacktitle"
        clearable
      />
    </el-form-item>
    <el-form-item label="内容" prop="feedbackcontent">
      <el-input
        v-model="form.feedbackcontent"
        type="textarea"
        :rows="4"
        :placeholder="'请输入内容'"
        :readonly="read.feedbackcontent"
        clearable
      />
    </el-form-item>
    <el-form-item label="反馈类型" prop="feedbacktype">
      <el-select v-model="form.feedbacktype" placeholder="请选择反馈类型" clearable :disabled="read.feedbacktype">
        <el-option
          v-for="opt in dictOptions.feedbacktype"
          :key="opt"
          :label="opt"
          :value="opt"
        />
...
const rules = reactive<FormRules>({
    ...
    'feedbacktype': [
      { required: false, message: '请输入反馈类型', trigger: 'blur' }
    ],
    'userid': [
...
async function loadDetail(id: number) {
  try {
    resetReadState()
    const res: any = await http.get('feedback/detail/' + id)
...
async function handleSubmit() {
  ...
  try {
    ...
    const api = isEdit.value && !isCross.value ? 'feedback/update' : 'feedback/add'
    const payload = { ...form }
