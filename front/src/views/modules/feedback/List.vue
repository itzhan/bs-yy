  ...
  <div v-if="canView" class="front-list-page">
    ...
    <el-form :inline="true" :model="formSearch" class="search-container">
      <el-form-item class="search-item">
        ...
        <el-input
          v-model="formSearch.feedbacktitle"
          placeholder="请输入标题"
    ...
    <div class="filter-container">
      ...
      <div class="options">
        <div
          class="option"
          :class="selectState.feedbacktype === -1 ? 'active' : ''"
          @click="selectDictOption('feedbacktype', -1)"
        >全部</div>
        <div
          class="option"
          :class="selectState.feedbacktype === 0 ? 'active' : ''"
          @click="selectDictOption('feedbacktype', 0)"
        >功能建议</div>
        <div
          class="option"
          :class="selectState.feedbacktype === 1 ? 'active' : ''"
        ...
        <div
          class="option"
          :class="selectState.feedbacktype === 2 ? 'active' : ''"
          @click="selectDictOption('feedbacktype', 2)"
        >投诉</div>
      </div>
    </div>
    </div>
      ...
      >
        ...
        <div class="info-box">
          ...
          <div class="info-item">
            ...
            <span class="value">{{ formatField(item.feedbacktype) }}</span>
          </div>
          <div class="info-item">
          ...
          <div class="info-item">
            ...
            <span class="value">{{ formatField(item.auditreply) }}</span>
          </div>
    ...
    <section v-if="showHotRecommend && hotRecommend.length" class="area-container hot-recommend">
      <div class="area-title-box">
        <h2 class="area-title">意见反馈热门推荐</h2>
        <div class="area-subhead">FEEDBACK RECOMMEND</div>
      </div>
...
const canViewPermission = useAuth('feedback', '查看')
const canCreate = !false && useAuth('feedback', '新增')
const canView = computed(() => canViewPermission.value || !token.value)

const imageField = null
const titleField = 'feedbacktitle'
const summaryFields = [
...
const detailRoute = '/index/feedbackDetail'
...
function handleAdd() {
  const query: Record<string, any> = {}
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/feedbackAdd', query })
}
...
function toDetail(row: Record<string, any>) {
  ...
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/feedbackDetail', query })
}
...
async function fetchList(page = pageIndex.value) {
  ...
  try {
    ...
    if (token.value) {
      const preferParam = resolveCatePreferParam('feedback')
      if (preferParam) {
    ...
    if (formSearch.feedbacktitle) {
      query.feedbacktitle = '%' + formSearch.feedbacktitle + '%'
    }

    if (formSearch.feedbacktype) {
      query.feedbacktype = formSearch.feedbacktype
    }
    ...
    const endpoint = centerType.value ? 'feedback/page' : 'feedback/list'
...
async function fetchHotRecommend(force = false) {
  ...
  try {
    const endpoint = 'feedback/' + (token.value ? 'autoSort2' : 'autoSort')
