...
async function loadDetail(id: number) {
  try {
    resetReadState()
    const res: any = await http.get('refundcardrenewal/detail/' + id)
...
async function handleSubmit() {
  ...
  try {
    ...
    const api = isEdit.value && !isCross.value ? 'refundcardrenewal/update' : 'refundcardrenewal/add'
    const payload = { ...form }
