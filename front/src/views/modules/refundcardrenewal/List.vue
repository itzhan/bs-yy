  ...
  <div v-if="canView" class="front-list-page">
    <div class="breadcrumb-container">
      <el-breadcrumb class="breadcrumb-box" :separator="'>'">
        ...
        <el-breadcrumb-item class="breadcrumb-item item-module">
          <a href="javascript:void(0);">续卡记录退款</a>
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    ...
    <section v-if="showHotRecommend && hotRecommend.length" class="area-container hot-recommend">
      <div class="area-title-box">
        <h2 class="area-title">续卡记录退款热门推荐</h2>
        <div class="area-subhead">REFUNDCARDRENEWAL RECOMMEND</div>
      </div>
...
const canViewPermission = useAuth('refundcardrenewal', '查看')
const canCreate = !false && useAuth('refundcardrenewal', '新增')
...
const detailRoute = '/index/refundcardrenewalDetail'
...
function handleAdd() {
  const query: Record<string, any> = {}
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/refundcardrenewalAdd', query })
}
...
function toDetail(row: Record<string, any>) {
  ...
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/refundcardrenewalDetail', query })
}
...
async function fetchList(page = pageIndex.value) {
  ...
  try {
    ...
    if (token.value) {
      const preferParam = resolveCatePreferParam('refundcardrenewal')
      if (preferParam) {
    ...
    const endpoint = centerType.value ? 'refundcardrenewal/page' : 'refundcardrenewal/list'
...
async function fetchHotRecommend(force = false) {
  ...
  try {
    const endpoint = 'refundcardrenewal/' + (token.value ? 'autoSort2' : 'autoSort')
