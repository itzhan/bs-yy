  ...
  <div v-if="canView" class="front-list-page">
    <div class="breadcrumb-container">
      <el-breadcrumb class="breadcrumb-box" :separator="'>'">
        ...
        <el-breadcrumb-item class="breadcrumb-item item-module">
          <a href="javascript:void(0);">商品订单退款</a>
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    ...
    <section v-if="showHotRecommend && hotRecommend.length" class="area-container hot-recommend">
      <div class="area-title-box">
        <h2 class="area-title">商品订单退款热门推荐</h2>
        <div class="area-subhead">REFUNDPRODUCTORDER RECOMMEND</div>
      </div>
...
const canViewPermission = useAuth('refundproductorder', '查看')
const canCreate = !false && useAuth('refundproductorder', '新增')
...
const detailRoute = '/index/refundproductorderDetail'
...
function handleAdd() {
  const query: Record<string, any> = {}
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/refundproductorderAdd', query })
}
...
function toDetail(row: Record<string, any>) {
  ...
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/refundproductorderDetail', query })
}
...
async function fetchList(page = pageIndex.value) {
  ...
  try {
    ...
    if (token.value) {
      const preferParam = resolveCatePreferParam('refundproductorder')
      if (preferParam) {
    ...
    const endpoint = centerType.value ? 'refundproductorder/page' : 'refundproductorder/list'
...
async function fetchHotRecommend(force = false) {
  ...
  try {
    const endpoint = 'refundproductorder/' + (token.value ? 'autoSort2' : 'autoSort')
