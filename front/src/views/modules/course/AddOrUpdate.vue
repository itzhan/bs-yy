<template>
  <div class="form-container front-add-page">
    ...
    <el-form-item label="教练" prop="coachname">
      <el-select v-model="form.coachname" placeholder="请选择教练" clearable :disabled="read.coachname">
        <el-option
          v-for="opt in categoryOptions.coachname"
          :key="opt"
          :label="opt"
          :value="opt"
        />
...
async function loadCategoryOptions() {
  ...
  try {
    const res: any = await http.get('coach/list', { params: { page: 1, limit: 1000, sort: 'id', order: 'asc' } })
    const list = Array.isArray(res?.data?.list) ? res.data.list : Array.isArray(res?.data?.data?.list) ? res.data.data.list : []
    categoryOptions['coachname'] = list
      .map((item: any) => item?.['coachname'])
      .filter(Boolean)
  } catch (error) {
    categoryOptions['coachname'] = []
  }
}
...
async function handleSubmit() {
  ...
  try {
    ...
    const api = isEdit.value && !isCross.value ? 'course/update' : 'course/add'
    const payload = { ...form }
