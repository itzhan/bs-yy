  ...
  <div v-if="canView" class="front-list-page">
    <div class="breadcrumb-container">
      <el-breadcrumb class="breadcrumb-box" :separator="'>'">
        ...
        <el-breadcrumb-item class="breadcrumb-item item-module">
          <a href="javascript:void(0);">健身课程</a>
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    ...
    <el-form :inline="true" :model="formSearch" class="search-container">
      ...
      <div class="range-filter">
        <el-form-item class="search-item">
          <span class="label">上课时间开始</span>
          <el-date-picker
            v-model="formSearch.classtime_start"
            type="date"
        ...
        <el-form-item class="search-item">
          <span class="label">上课时间结束</span>
          <el-date-picker
            v-model="formSearch.classtime_end"
            type="date"
            value-format="YYYY-MM-DD HH:mm:ss"
            placeholder="上课时间结束"
            clearable
      ...
      >
        ...
        <div class="info-box">
          ...
          <div class="info-item">
            ...
            <span class="value" :class="Number(item.quota||0) <= 0 ? 'soldout' : ''">{{ Number(item.quota||0) }}</span>
            <el-tag type="danger" v-if="Number(item.quota||0) <= 0" size="small">售罄</el-tag>
          </div>
          <div class="info-item">
            <span class="label">课程分类：</span>
            <span class="value">{{ formatField(item.coursetype) }}</span>
          </div>
          <div class="info-item">
            <span class="label">上课时间：</span>
            <span class="value">{{ formatField(item.classtime) }}</span>
          </div>
          <div class="info-item">
            <span class="label">课程时长：</span>
            <span class="value">{{ formatField(item.duration) }}</span>
          </div>
          <div class="info-item">
    ...
    <section v-if="showHotRecommend && hotRecommend.length" class="area-container hot-recommend">
      <div class="area-title-box">
        <h2 class="area-title">健身课程热门推荐</h2>
        <div class="area-subhead">COURSE RECOMMEND</div>
      </div>
...
const canCreate = !false && useAuth('course', '新增')
...
const categoryConfig = { field: 'coursetype', label: '课程分类', table: 'coursetype', column: 'coursetypename' }
const showLike = true
...
const detailRoute = '/index/courseDetail'
...
function handleAdd() {
  const query: Record<string, any> = {}
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/courseAdd', query })
}
...
function toDetail(row: Record<string, any>) {
  ...
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/courseDetail', query })
}
...
async function fetchList(page = pageIndex.value) {
  ...
  try {
    ...
    if (token.value) {
      const preferParam = resolveCatePreferParam('course')
      if (preferParam) {
    ...
    if (formSearch.coursename) {
      query.coursename = '%' + formSearch.coursename + '%'
    }


    if (formSearch.classtime_start) query.classtime_start = formSearch.classtime_start
    if (formSearch.classtime_end) query.classtime_end = formSearch.classtime_end
    ...
    const endpoint = centerType.value ? 'course/page' : 'course/list'
...
async function fetchHotRecommend(force = false) {
  ...
  try {
    const endpoint = 'course/' + (token.value ? 'autoSort2' : 'autoSort')
