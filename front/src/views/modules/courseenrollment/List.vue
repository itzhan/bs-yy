  ...
  <div v-if="canView" class="front-list-page">
    <div class="breadcrumb-container">
      <el-breadcrumb class="breadcrumb-box" :separator="'>'">
        ...
        <el-breadcrumb-item class="breadcrumb-item item-module">
          <a href="javascript:void(0);">课程报名记录</a>
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
      ...
      >
        ...
        <div class="info-box">
          ...
          <div class="info-item">
            ...
            <span class="value">
              <el-tag :type="item.auditstatus === '已通过' ? 'success' : item.auditstatus === '已驳回' ? 'danger' : 'warning'" size="small">
    ...
    <section v-if="showHotRecommend && hotRecommend.length" class="area-container hot-recommend">
      <div class="area-title-box">
        <h2 class="area-title">课程报名记录热门推荐</h2>
        <div class="area-subhead">COURSEENROLLMENT RECOMMEND</div>
      </div>
...
const detailRoute = '/index/courseenrollmentDetail'
...
function toDetail(row: Record<string, any>) {
  ...
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/courseenrollmentDetail', query })
}
...
async function fetchList(page = pageIndex.value) {
  ...
  try {
    ...
    if (token.value) {
      const preferParam = resolveCatePreferParam('courseenrollment')
      if (preferParam) {
    ...
    const endpoint = centerType.value ? 'courseenrollment/page' : 'courseenrollment/list'
...
async function fetchHotRecommend(force = false) {
  ...
  try {
    const endpoint = 'courseenrollment/' + (token.value ? 'autoSort2' : 'autoSort')
