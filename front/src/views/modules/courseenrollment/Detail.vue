        ...
        <div class="detail-content">
          ...
          <div class="actions">
            ...
            <el-button
              size="small"
              type="primary"
              v-if="hasActionPermission('取消课程报名记录')"
              :disabled="isCrossDisabled('取消课程报名记录')"
              :title="crossDisabledReason('取消课程报名记录') || undefined"
              @click="onCross('取消课程报名记录')"
            >取消课程报名记录</el-button>
            <el-button
              size="small"
              type="primary"
    ...
    <el-dialog v-model="chatVisible" :title="`与 ${fname} 私聊`" width="600px" @close="handleChatClose">
      ...
      <template #footer>
        <div style="display:flex; align-items:center; width:100%;">
          ...
          <el-button style="margin-left:6px;" type="primary" @click="sendCurrentLabel">发送课程报名记录</el-button>
        </div>
      </template>
    </el-dialog>
  </div>
...
const tableButtonsPermission = useTableButtons('courseenrollment')
...
async function submitPay() {
  ...
  try {
    ...
    const res: any = await http.post('courseenrollment/update', payload)
    if (res?.code === 0) {
      info.ispay = '已支付'
...
const crossButtonsConfig: CrossButtonConfig[] = [
  {
    name: '取消课程报名记录',
    targetTable: 'cancelcourseenrollment',
    tips: '课程报名记录已取消',
  ...
  {
    name: '退款',
    targetTable: 'refundcourseenrollment',
    tips: '退款已处理',
    statusField: 'orderstatus',
    statusValue: '已退款',
    stockTargetTable: 'course',
...
async function onCross(name: string){
  ...
  try {
    ...
    const inlineOnly = !targetTableName || targetTableName === 'courseenrollment'
    ...
    if (targetTableName === 'cart' && name === '加入购物车') {
      try {
        ...
        const payload: Record<string, any> = {
          tablename: (localStorage.getItem('crossTable') || 'courseenrollment') || 'courseenrollment',
...
async function inlineCrossAction(
  ...
  try {
    if (statusKey && statusTarget) {
      ...
      await http.post('courseenrollment/update', payload)
    ...
    const refId = (info as any)?.crossrefid || (target === 'courseenrollment' ? (info as any)?.id : undefined)
    if (!refId) {
...
const chatAccountField = 'useraccount'
const chatRoleTable = 'user'
const chatAvatarField = 'image'
...
async function sendCurrentLabel(){
  ...
  if (firstImg) {
    ...
    const payload: any = { uid: myid, fid: fid.value, content: '课程报名记录', format: 1 }
    if (chatRoleTable) {
      payload.tablename = chatRoleTable
    }
    const res: any = await http.post('chatmessage/add', payload)
