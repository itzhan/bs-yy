  ...
  <div class="form-container front-add-page">
    ...
    <el-form-item label="简介" prop="introduction">
      <el-input
        ...
        :rows="4"
        :placeholder="'请输入简介'"
        :readonly="read.introduction"
        clearable
      />
    </el-form-item>
    <el-form-item label="分类名称" prop="typename">
      <el-select v-model="form.typename" placeholder="请选择分类名称" clearable :disabled="read.typename">
        <el-option
          v-for="opt in categoryOptions.typename"
          :key="opt"
          :label="opt"
          :value="opt"
        />
...
const rules = reactive<FormRules>({
    ...
    'typename': [
      { required: false, message: '请输入分类名称', trigger: 'blur' }
    ],
    'name': [
...
async function loadCategoryOptions() {
  try {
    const res: any = await http.get('newstype/list', { params: { page: 1, limit: 1000, sort: 'id', order: 'asc' } })
    const list = Array.isArray(res?.data?.list) ? res.data.list : Array.isArray(res?.data?.data?.list) ? res.data.data.list : []
    categoryOptions['typename'] = list
      .map((item: any) => item?.['typename'])
      .filter(Boolean)
  } catch (error) {
    categoryOptions['typename'] = []
  }
}
...
async function handleSubmit() {
  ...
  try {
    ...
    const api = isEdit.value && !isCross.value ? 'news/update' : 'news/add'
    const payload = { ...form }
