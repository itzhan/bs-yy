  ...
  <div v-if="canView" class="front-list-page">
    <div class="breadcrumb-container">
      <el-breadcrumb class="breadcrumb-box" :separator="'>'">
        ...
        <el-breadcrumb-item class="breadcrumb-item item-module">
          <a href="javascript:void(0);">公告资讯</a>
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
      ...
      >
        ...
        <div class="info-box">
          ...
          <div class="info-item">
            ...
            <span class="value">{{ formatField(item.introduction) }}</span>
          </div>
          <div class="info-item">
            <span class="label">分类名称：</span>
            <span class="value">{{ formatField(item.typename) }}</span>
          </div>
          <div class="info-item">
            <span class="label">发布人：</span>
            <span class="value">{{ formatField(item.name) }}</span>
          </div>
          <div class="info-item">
    ...
    <section v-if="showHotRecommend && hotRecommend.length" class="area-container hot-recommend">
      <div class="area-title-box">
        <h2 class="area-title">公告资讯热门推荐</h2>
        <div class="area-subhead">NEWS RECOMMEND</div>
      </div>
...
const canCreate = !false && useAuth('news', '新增')
...
const categoryConfig = { field: 'typename', label: '分类名称', table: 'newstype', column: 'typename' }
const showLike = true
...
const detailRoute = '/index/newsDetail'
...
function handleAdd() {
  const query: Record<string, any> = {}
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/newsAdd', query })
}
...
function toDetail(row: Record<string, any>) {
  ...
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/newsDetail', query })
}
...
async function fetchList(page = pageIndex.value) {
  ...
  try {
    ...
    if (token.value) {
      const preferParam = resolveCatePreferParam('news')
      if (preferParam) {
    ...
    const endpoint = centerType.value ? 'news/page' : 'news/list'
...
async function fetchHotRecommend(force = false) {
  ...
  try {
    const endpoint = 'news/' + (token.value ? 'autoSort2' : 'autoSort')
