<template>
  <div class="form-container front-add-page">
    ...
    <el-form-item label="关联表id" prop="refid">
      <el-input-number v-model="form.refid" placeholder="请输入关联表id" :step="1" :controls="false" :disabled="read.refid" />
    </el-form-item>
    <el-form-item label="用户名" prop="nickname">
      <UploadField
        v-model="form.nickname"
        upload-type="image"
        :readonly="read.nickname"
      />
    </el-form-item>
...
const rules = reactive<FormRules>({
    'refid': [
      { required: false, message: '请输入关联表id', trigger: 'blur' }
    ],
    'userid': [
    ...
    'reply': [
      { required: false, message: '请输入回复内容', trigger: 'blur' }
    ],
    'likenum': [
...
async function loadDetail(id: number) {
  try {
    resetReadState()
    const res: any = await http.get('discussproductorder/detail/' + id)
...
async function handleSubmit() {
  ...
  try {
    ...
    const api = isEdit.value && !isCross.value ? 'discussproductorder/update' : 'discussproductorder/add'
    const payload = { ...form }
