...
async function loadDetail(id: number) {
  try {
    resetReadState()
    const res: any = await http.get('discussproduct/detail/' + id)
...
async function handleSubmit() {
  ...
  try {
    ...
    const api = isEdit.value && !isCross.value ? 'discussproduct/update' : 'discussproduct/add'
    const payload = { ...form }
