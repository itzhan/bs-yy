  ...
  <div v-if="canView" class="front-list-page">
    <div class="breadcrumb-container">
      <el-breadcrumb class="breadcrumb-box" :separator="'>'">
        ...
        <el-breadcrumb-item class="breadcrumb-item item-module">
          <a href="javascript:void(0);">公告资讯评论表</a>
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    ...
    <section v-if="showHotRecommend && hotRecommend.length" class="area-container hot-recommend">
      <div class="area-title-box">
        <h2 class="area-title">公告资讯评论表热门推荐</h2>
        <div class="area-subhead">DISCUSSNEWS RECOMMEND</div>
      </div>
...
const canCreate = !false && useAuth('discussnews', '新增')
...
const detailRoute = '/index/discussnewsDetail'
...
function handleAdd() {
  const query: Record<string, any> = {}
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/discussnewsAdd', query })
}
...
function toDetail(row: Record<string, any>) {
  ...
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/discussnewsDetail', query })
}
...
async function fetchList(page = pageIndex.value) {
  ...
  try {
    ...
    if (token.value) {
      const preferParam = resolveCatePreferParam('discussnews')
      if (preferParam) {
    ...
    const endpoint = centerType.value ? 'discussnews/page' : 'discussnews/list'
...
async function fetchHotRecommend(force = false) {
  ...
  try {
    const endpoint = 'discussnews/' + (token.value ? 'autoSort2' : 'autoSort')
