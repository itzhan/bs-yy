...
<template>
  <div v-if="canView" class="front-list-page">
    <div class="breadcrumb-container">
      <el-breadcrumb class="breadcrumb-box" :separator="'>'">
        ...
        <el-breadcrumb-item class="breadcrumb-item item-module">
          <a href="javascript:void(0);">消息表</a>
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    ...
    <el-form :inline="true" :model="formSearch" class="search-container">
      ...
      <el-form-item class="search-item">
        <span class="label">好友用户ID</span>
        <el-input
          v-model="formSearch.fid"
          placeholder="请输入好友用户ID"
          @keyup.enter.native="triggerSearch()"
          clearable
        />
      </el-form-item>
      <el-form-item class="search-item">
        <span class="label">关联表名</span>
        <el-input
          v-model="formSearch.tablename"
          placeholder="请输入关联表名"
          @keyup.enter.native="triggerSearch()"
          clearable
        />
    ...
    <div v-if="!mapConfig.enabled" class="list-container">
      ...
      >
        ...
        <div class="info-box">
          ...
          <div class="info-item">
            <span class="label">格式(1:文字，2:图片)：</span>
            <span class="value">{{ formatField(item.format) }}</span>
          </div>
          <div class="info-item">
            <span class="label">消息已读(0:未读，1:已读)：</span>
            <span class="value">{{ formatField(item.isread) }}</span>
          </div>
          <div class="info-item">
    ...
    <section v-if="showHotRecommend && hotRecommend.length" class="area-container hot-recommend">
      <div class="area-title-box">
        <h2 class="area-title">消息表热门推荐</h2>
        <div class="area-subhead">CHATMESSAGE RECOMMEND</div>
      </div>
...
const canViewPermission = useAuth('chatmessage', '查看')
const canCreate = !false && useAuth('chatmessage', '新增')
...
const detailRoute = '/index/chatmessageDetail'
...
function handleAdd() {
  const query: Record<string, any> = {}
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/chatmessageAdd', query })
}
...
function toDetail(row: Record<string, any>) {
  ...
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/chatmessageDetail', query })
}
...
async function fetchList(page = pageIndex.value) {
  ...
  try {
    ...
    if (token.value) {
      const preferParam = resolveCatePreferParam('chatmessage')
      if (preferParam) {
    ...
    if (formSearch.uid) {
      query.uid = formSearch.uid
    }
    if (formSearch.fid) {
      query.fid = formSearch.fid
    }
    if (formSearch.tablename) {
      query.tablename = formSearch.tablename
    }
    ...
    const endpoint = centerType.value ? 'chatmessage/page' : 'chatmessage/list'
...
async function fetchHotRecommend(force = false) {
  ...
  try {
    const endpoint = 'chatmessage/' + (token.value ? 'autoSort2' : 'autoSort')
