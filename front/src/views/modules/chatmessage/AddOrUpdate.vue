<template>
  <div class="form-container front-add-page">
    ...
    <el-form-item label="用户ID" prop="uid">
      <el-input-number v-model="form.uid" placeholder="请输入用户ID" :step="1" :controls="false" :disabled="read.uid" />
    </el-form-item>
    <el-form-item label="好友用户ID" prop="fid">
      <el-input-number v-model="form.fid" placeholder="请输入好友用户ID" :step="1" :controls="false" :disabled="read.fid" />
    </el-form-item>
    ...
    <el-form-item label="格式(1:文字，2:图片)" prop="format">
      <el-input-number v-model="form.format" placeholder="请输入格式(1:文字，2:图片)" :step="1" :controls="false" :disabled="read.format" />
    </el-form-item>
    <el-form-item label="消息已读(0:未读，1:已读)" prop="isread">
      <el-input-number v-model="form.isread" placeholder="请输入消息已读(0:未读，1:已读)" :step="1" :controls="false" :disabled="read.isread" />
    </el-form-item>
...
const rules = reactive<FormRules>({
    ...
    'fid': [
      { required: false, message: '请输入好友用户ID', trigger: 'blur' }
    ],
    'tablename': [
      { required: false, message: '请输入关联表名', trigger: 'blur' }
    ],
    'content': [
      { required: false, message: '请输入内容', trigger: 'blur' }
    ],
    'format': [
      { required: false, message: '请输入格式(1:文字，2:图片)', trigger: 'blur' }
    ],
    'isread': [
      { required: false, message: '请输入消息已读(0:未读，1:已读)', trigger: 'blur' }
    ],
    'addtime': [
...
async function loadDetail(id: number) {
  try {
    resetReadState()
    const res: any = await http.get('chatmessage/detail/' + id)
...
async function handleSubmit() {
  ...
  try {
    ...
    const api = isEdit.value && !isCross.value ? 'chatmessage/update' : 'chatmessage/add'
    const payload = { ...form }
