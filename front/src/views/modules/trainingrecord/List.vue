<template>
  <div v-if="canView" class="front-list-page">
    ...
    <el-card class="list-card" shadow="hover">
      <template #header>
        <div class="list-card-header">训练记录</div>
      </template>
...
const canView = computed(() => isAuth('trainingrecord', '查看'))
...
const fetchList = async () => {
  ...
  try {
    ...
    const endpoint = centerType.value ? 'trainingrecord/page' : 'trainingrecord/list'
...
const toDetail = (row: any) => {
  router.push({ path: '/index/trainingrecordDetail', query: { id: row.id, centerType: centerType.value ? '1' : undefined } })
}
