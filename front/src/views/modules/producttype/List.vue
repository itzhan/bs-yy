  ...
  <div v-if="canView" class="front-list-page">
    ...
    <el-form :inline="true" :model="formSearch" class="search-container">
      <el-form-item class="search-item">
        ...
        <el-input
          v-model="formSearch.producttypename"
          placeholder="请输入分类名称"
    ...
    <section v-if="showHotRecommend && hotRecommend.length" class="area-container hot-recommend">
      <div class="area-title-box">
        <h2 class="area-title">商品分类热门推荐</h2>
        <div class="area-subhead">PRODUCTTYPE RECOMMEND</div>
      </div>
...
const canViewPermission = useAuth('producttype', '查看')
const canCreate = !false && useAuth('producttype', '新增')
const canView = computed(() => canViewPermission.value || !token.value)

const imageField = null
const titleField = 'producttypename'
const summaryFields = [
...
const detailRoute = '/index/producttypeDetail'
...
function handleAdd() {
  const query: Record<string, any> = {}
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/producttypeAdd', query })
}
...
function toDetail(row: Record<string, any>) {
  ...
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/producttypeDetail', query })
}
...
async function fetchList(page = pageIndex.value) {
  ...
  try {
    ...
    if (token.value) {
      const preferParam = resolveCatePreferParam('producttype')
      if (preferParam) {
    ...
    if (formSearch.producttypename) {
      query.producttypename = '%' + formSearch.producttypename + '%'
    }
    ...
    const endpoint = centerType.value ? 'producttype/page' : 'producttype/list'
...
async function fetchHotRecommend(force = false) {
  ...
  try {
    const endpoint = 'producttype/' + (token.value ? 'autoSort2' : 'autoSort')
