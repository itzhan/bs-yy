  ...
  <div v-if="canView" class="front-list-page">
    <div class="breadcrumb-container">
      <el-breadcrumb class="breadcrumb-box" :separator="'>'">
        ...
        <el-breadcrumb-item class="breadcrumb-item item-module">
          <a href="javascript:void(0);">取消课程报名记录</a>
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    ...
    <section v-if="showHotRecommend && hotRecommend.length" class="area-container hot-recommend">
      <div class="area-title-box">
        <h2 class="area-title">取消课程报名记录热门推荐</h2>
        <div class="area-subhead">CANCELCOURSEENROLLMENT RECOMMEND</div>
      </div>
...
const canViewPermission = useAuth('cancelcourseenrollment', '查看')
const canCreate = !false && useAuth('cancelcourseenrollment', '新增')
...
const detailRoute = '/index/cancelcourseenrollmentDetail'
...
function handleAdd() {
  const query: Record<string, any> = {}
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/cancelcourseenrollmentAdd', query })
}
...
function toDetail(row: Record<string, any>) {
  ...
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/cancelcourseenrollmentDetail', query })
}
...
async function fetchList(page = pageIndex.value) {
  ...
  try {
    ...
    if (token.value) {
      const preferParam = resolveCatePreferParam('cancelcourseenrollment')
      if (preferParam) {
    ...
    const endpoint = centerType.value ? 'cancelcourseenrollment/page' : 'cancelcourseenrollment/list'
...
async function fetchHotRecommend(force = false) {
  ...
  try {
    const endpoint = 'cancelcourseenrollment/' + (token.value ? 'autoSort2' : 'autoSort')
