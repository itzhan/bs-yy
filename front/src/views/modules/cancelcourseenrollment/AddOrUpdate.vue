...
async function loadDetail(id: number) {
  try {
    resetReadState()
    const res: any = await http.get('cancelcourseenrollment/detail/' + id)
...
async function handleSubmit() {
  ...
  try {
    ...
    const api = isEdit.value && !isCross.value ? 'cancelcourseenrollment/update' : 'cancelcourseenrollment/add'
    const payload = { ...form }
