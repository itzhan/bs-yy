  ...
  <div v-if="canView" class="front-list-page">
    ...
    <el-form :inline="true" :model="formSearch" class="search-container">
      <el-form-item class="search-item">
        ...
        <el-input
          v-model="formSearch.coachaccount"
          placeholder="请输入账号"
    ...
    <div v-if="!mapConfig.enabled" class="list-container">
      ...
      >
        ...
        <div class="info-box">
          ...
          <div class="info-item">
            <span class="label">工号：</span>
            <span class="value">{{ formatField(item.jobno) }}</span>
          </div>
          <div class="info-item">
            <span class="label">擅长领域：</span>
            <span class="value">{{ formatField(item.specialty) }}</span>
          </div>
          <div class="info-item">
            <span class="label">教练等级：</span>
            <span class="value">{{ formatField(item.coachlevel) }}</span>
          </div>
    ...
    <section v-if="showHotRecommend && hotRecommend.length" class="area-container hot-recommend">
      <div class="area-title-box">
        <h2 class="area-title">教练热门推荐</h2>
        <div class="area-subhead">COACH RECOMMEND</div>
      </div>
...
const canCreate = !false && useAuth('coach', '新增')
const canView = computed(() => canViewPermission.value || !token.value)

const imageField = 'coachimage'
...
const detailRoute = '/index/coachDetail'
...
function handleAdd() {
  const query: Record<string, any> = {}
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/coachAdd', query })
}
...
function toDetail(row: Record<string, any>) {
  ...
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/coachDetail', query })
}
...
async function fetchList(page = pageIndex.value) {
  ...
  try {
    ...
    if (token.value) {
      const preferParam = resolveCatePreferParam('coach')
      if (preferParam) {
    ...
    if (formSearch.coachaccount) {
      query.coachaccount = '%' + formSearch.coachaccount + '%'
    }
    if (formSearch.coachname) {
      query.coachname = '%' + formSearch.coachname + '%'
    }
    if (formSearch.jobno) {
      query.jobno = '%' + formSearch.jobno + '%'
    }
    ...
    const endpoint = centerType.value ? 'coach/page' : 'coach/list'
...
async function fetchHotRecommend(force = false) {
  ...
  try {
    const endpoint = 'coach/' + (token.value ? 'autoSort2' : 'autoSort')
