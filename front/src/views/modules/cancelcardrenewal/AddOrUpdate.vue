<template>
  <div class="form-container front-add-page">
    ...
    <el-form-item label="续费时长" prop="renewaldays">
      <el-input-number v-model="form.renewaldays" placeholder="请输入续费时长" :step="1" :controls="false" :disabled="read.renewaldays" />
    </el-form-item>
...
const rules = reactive<FormRules>({
    ...
    'renewaldays': [
      { required: false, message: '请输入续费时长', trigger: 'blur' }
    ],
    'userid': [
...
async function loadDetail(id: number) {
  try {
    resetReadState()
    const res: any = await http.get('cancelcardrenewal/detail/' + id)
...
async function handleSubmit() {
  ...
  try {
    ...
    const api = isEdit.value && !isCross.value ? 'cancelcardrenewal/update' : 'cancelcardrenewal/add'
    const payload = { ...form }
