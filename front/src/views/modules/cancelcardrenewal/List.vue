  ...
  <div v-if="canView" class="front-list-page">
    <div class="breadcrumb-container">
      <el-breadcrumb class="breadcrumb-box" :separator="'>'">
        ...
        <el-breadcrumb-item class="breadcrumb-item item-module">
          <a href="javascript:void(0);">取消续卡记录</a>
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    ...
    <section v-if="showHotRecommend && hotRecommend.length" class="area-container hot-recommend">
      <div class="area-title-box">
        <h2 class="area-title">取消续卡记录热门推荐</h2>
        <div class="area-subhead">CANCELCARDRENEWAL RECOMMEND</div>
      </div>
...
const canViewPermission = useAuth('cancelcardrenewal', '查看')
const canCreate = !false && useAuth('cancelcardrenewal', '新增')
...
const detailRoute = '/index/cancelcardrenewalDetail'
...
function handleAdd() {
  const query: Record<string, any> = {}
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/cancelcardrenewalAdd', query })
}
...
function toDetail(row: Record<string, any>) {
  ...
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/cancelcardrenewalDetail', query })
}
...
async function fetchList(page = pageIndex.value) {
  ...
  try {
    ...
    if (token.value) {
      const preferParam = resolveCatePreferParam('cancelcardrenewal')
      if (preferParam) {
    ...
    const endpoint = centerType.value ? 'cancelcardrenewal/page' : 'cancelcardrenewal/list'
...
async function fetchHotRecommend(force = false) {
  ...
  try {
    const endpoint = 'cancelcardrenewal/' + (token.value ? 'autoSort2' : 'autoSort')
