  ...
  <div v-if="canView" class="front-list-page">
    <div class="breadcrumb-container">
      <el-breadcrumb class="breadcrumb-box" :separator="'>'">
        ...
        <el-breadcrumb-item class="breadcrumb-item item-module">
          <a href="javascript:void(0);">取消商品订单</a>
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    ...
    <section v-if="showHotRecommend && hotRecommend.length" class="area-container hot-recommend">
      <div class="area-title-box">
        <h2 class="area-title">取消商品订单热门推荐</h2>
        <div class="area-subhead">CANCELPRODUCTORDER RECOMMEND</div>
      </div>
...
const canViewPermission = useAuth('cancelproductorder', '查看')
const canCreate = !false && useAuth('cancelproductorder', '新增')
...
const detailRoute = '/index/cancelproductorderDetail'
...
function handleAdd() {
  const query: Record<string, any> = {}
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/cancelproductorderAdd', query })
}
...
function toDetail(row: Record<string, any>) {
  ...
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/cancelproductorderDetail', query })
}
...
async function fetchList(page = pageIndex.value) {
  ...
  try {
    ...
    if (token.value) {
      const preferParam = resolveCatePreferParam('cancelproductorder')
      if (preferParam) {
    ...
    const endpoint = centerType.value ? 'cancelproductorder/page' : 'cancelproductorder/list'
...
async function fetchHotRecommend(force = false) {
  ...
  try {
    const endpoint = 'cancelproductorder/' + (token.value ? 'autoSort2' : 'autoSort')
