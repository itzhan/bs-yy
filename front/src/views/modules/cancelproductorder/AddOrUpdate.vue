<template>
  <div class="form-container front-add-page">
    ...
    <el-form-item label="商品名称" prop="productname">
      <el-select v-model="form.productname" placeholder="请选择商品名称" clearable :disabled="read.productname">
        <el-option
          v-for="opt in categoryOptions.productname"
          :key="opt"
          :label="opt"
          :value="opt"
        />
    ...
    <el-form-item label="商品图片" prop="productimage">
      <UploadField
        ...
        upload-type="image"
        :readonly="read.productimage"
      />
    </el-form-item>
    <el-form-item label="商品分类" prop="producttype">
      <el-select v-model="form.producttype" placeholder="请选择商品分类" clearable :disabled="read.producttype">
        <el-option
          v-for="opt in categoryOptions.producttype"
          :key="opt"
          :label="opt"
          :value="opt"
        />
    ...
    <el-form-item label="商品单价" prop="productprice">
      <el-input-number v-model="form.productprice" placeholder="请输入商品单价" :step="1" :controls="false" :disabled="read.productprice" />
    </el-form-item>
    <el-form-item label="购买数量" prop="quantity">
      <el-input-number v-model="form.quantity" placeholder="请输入购买数量" :step="1" :controls="false" :disabled="read.quantity" />
    </el-form-item>
    <el-form-item label="订单总价" prop="totalprice">
      <el-input-number v-model="form.totalprice" placeholder="请输入订单总价" :step="1" :controls="false" :disabled="read.totalprice" />
    </el-form-item>
...
const rules = reactive<FormRules>({
    ...
    'productimage': [
      { required: false, message: '请输入商品图片', trigger: 'blur' }
    ],
    'producttype': [
      { required: false, message: '请输入商品分类', trigger: 'blur' }
    ],
    'productprice': [
      { required: false, message: '请输入商品单价', trigger: 'blur' }
    ],
    'quantity': [
      { required: false, message: '请输入购买数量', trigger: 'blur' }
    ],
    'totalprice': [
    ...
    'cancelreason': [
      { required: false, message: '请输入取消原因', trigger: 'blur' }
    ],
    'addtime': [
...
async function loadCategoryOptions() {
  try {
    const res: any = await http.get('product/list', { params: { page: 1, limit: 1000, sort: 'id', order: 'asc' } })
    const list = Array.isArray(res?.data?.list) ? res.data.list : Array.isArray(res?.data?.data?.list) ? res.data.data.list : []
    categoryOptions['productname'] = list
  ...
  try {
    const res: any = await http.get('producttype/list', { params: { page: 1, limit: 1000, sort: 'id', order: 'asc' } })
    const list = Array.isArray(res?.data?.list) ? res.data.list : Array.isArray(res?.data?.data?.list) ? res.data.data.list : []
    categoryOptions['producttype'] = list
      .map((item: any) => item?.['producttypename'])
      .filter(Boolean)
  } catch (error) {
    categoryOptions['producttype'] = []
  }
}
...
async function loadDetail(id: number) {
  try {
    resetReadState()
    const res: any = await http.get('cancelproductorder/detail/' + id)
...
async function handleSubmit() {
  ...
  try {
    ...
    const api = isEdit.value && !isCross.value ? 'cancelproductorder/update' : 'cancelproductorder/add'
    const payload = { ...form }
