...
async function loadDetail(id: number) {
  try {
    resetReadState()
    const res: any = await http.get('discussshare/detail/' + id)
...
async function handleSubmit() {
  ...
  try {
    ...
    const api = isEdit.value && !isCross.value ? 'discussshare/update' : 'discussshare/add'
    const payload = { ...form }
