  ...
  <div v-if="canView" class="front-list-page">
    ...
    <el-form :inline="true" :model="formSearch" class="search-container">
      <el-form-item class="search-item">
        <span class="label">配置参数名称</span>
        <el-input
          v-model="formSearch.name"
          placeholder="请输入配置参数名称"
    ...
    <div v-if="!mapConfig.enabled" class="list-container">
      ...
      >
        ...
        <div class="info-box">
          ...
          <div class="info-item">
            <span class="label">配置参数值：</span>
            <span class="value">{{ formatField(item.value) }}</span>
          </div>
    ...
    <section v-if="showHotRecommend && hotRecommend.length" class="area-container hot-recommend">
      <div class="area-title-box">
        <h2 class="area-title">轮播图热门推荐</h2>
        <div class="area-subhead">CONFIG RECOMMEND</div>
      </div>
...
const canViewPermission = useAuth('config', '查看')
const canCreate = !false && useAuth('config', '新增')
...
const detailRoute = '/index/configDetail'
...
function handleAdd() {
  const query: Record<string, any> = {}
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/configAdd', query })
}
...
function toDetail(row: Record<string, any>) {
  ...
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/configDetail', query })
}
...
async function fetchList(page = pageIndex.value) {
  ...
  try {
    ...
    if (token.value) {
      const preferParam = resolveCatePreferParam('config')
      if (preferParam) {
    ...
    const endpoint = centerType.value ? 'config/page' : 'config/list'
...
async function fetchHotRecommend(force = false) {
  ...
  try {
    const endpoint = 'config/' + (token.value ? 'autoSort2' : 'autoSort')
