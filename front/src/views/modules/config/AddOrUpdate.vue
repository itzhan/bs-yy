...
const rules = reactive<FormRules>({
    'name': [
      { required: false, message: '请输入配置参数名称', trigger: 'blur' }
    ],
    'value': [
      { required: false, message: '请输入配置参数值', trigger: 'blur' }
    ],
    'url': [
...
async function loadDetail(id: number) {
  try {
    resetReadState()
    const res: any = await http.get('config/detail/' + id)
...
async function handleSubmit() {
  ...
  try {
    ...
    const api = isEdit.value && !isCross.value ? 'config/update' : 'config/add'
    const payload = { ...form }
