<template>
  <div class="form-container front-add-page">
    ...
    <el-form-item label="套餐名称" prop="packagename">
      <el-select v-model="form.packagename" placeholder="请选择套餐名称" clearable :disabled="read.packagename">
        <el-option
          v-for="opt in categoryOptions.packagename"
          :key="opt"
          :label="opt"
          :value="opt"
        />
    ...
    <el-form-item label="套餐类型" prop="packagetype">
      <el-input
        ...
        :placeholder="'请输入套餐类型'"
        :readonly="read.packagetype"
        clearable
      />
    </el-form-item>
    <el-form-item label="价格" prop="packageprice">
      <el-input-number v-model="form.packageprice" placeholder="请输入价格" :step="1" :controls="false" :disabled="read.packageprice" />
    </el-form-item>
    <el-form-item label="有效期" prop="validdays">
      <el-input-number v-model="form.validdays" placeholder="请输入有效期" :step="1" :controls="false" :disabled="read.validdays" />
    </el-form-item>
    <el-form-item label="包含课时" prop="includedcourses">
      <el-input-number v-model="form.includedcourses" placeholder="请输入包含课时" :step="1" :controls="false" :disabled="read.includedcourses" />
    </el-form-item>
...
const rules = reactive<FormRules>({
    'packagename': [
      { required: false, message: '请输入套餐名称', trigger: 'blur' }
    ],
    'packageimage': [
      { required: false, message: '请输入套餐图片', trigger: 'blur' }
    ],
    'packagetype': [
      { required: false, message: '请输入套餐类型', trigger: 'blur' }
    ],
    'packageprice': [
    ...
    'includedcourses': [
      { required: false, message: '请输入包含课时', trigger: 'blur' }
    ],
    'userid': [
...
async function loadCategoryOptions() {
  try {
    const res: any = await http.get('membershippackage/list', { params: { page: 1, limit: 1000, sort: 'id', order: 'asc' } })
    const list = Array.isArray(res?.data?.list) ? res.data.list : Array.isArray(res?.data?.data?.list) ? res.data.data.list : []
    categoryOptions['packagename'] = list
      .map((item: any) => item?.['packagename'])
      .filter(Boolean)
  } catch (error) {
    categoryOptions['packagename'] = []
  }
}
...
async function loadDetail(id: number) {
  try {
    resetReadState()
    const res: any = await http.get('cancelcardapplication/detail/' + id)
...
async function handleSubmit() {
  ...
  try {
    ...
    const api = isEdit.value && !isCross.value ? 'cancelcardapplication/update' : 'cancelcardapplication/add'
    const payload = { ...form }
