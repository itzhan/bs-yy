  ...
  <div v-if="canView" class="front-list-page">
    <div class="breadcrumb-container">
      <el-breadcrumb class="breadcrumb-box" :separator="'>'">
        ...
        <el-breadcrumb-item class="breadcrumb-item item-module">
          <a href="javascript:void(0);">取消办卡记录</a>
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
      ...
      >
        ...
        <div class="info-box">
          ...
          <div class="info-item">
            ...
            <span class="value">{{ formatField(item.cancelreason) }}</span>
          </div>
          <div class="info-item">
    ...
    <section v-if="showHotRecommend && hotRecommend.length" class="area-container hot-recommend">
      <div class="area-title-box">
        <h2 class="area-title">取消办卡记录热门推荐</h2>
        <div class="area-subhead">CANCELCARDAPPLICATION RECOMMEND</div>
      </div>
...
const canViewPermission = useAuth('cancelcardapplication', '查看')
const canCreate = !false && useAuth('cancelcardapplication', '新增')
...
const detailRoute = '/index/cancelcardapplicationDetail'
...
function handleAdd() {
  const query: Record<string, any> = {}
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/cancelcardapplicationAdd', query })
}
...
function toDetail(row: Record<string, any>) {
  ...
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/cancelcardapplicationDetail', query })
}
...
async function fetchList(page = pageIndex.value) {
  ...
  try {
    ...
    if (token.value) {
      const preferParam = resolveCatePreferParam('cancelcardapplication')
      if (preferParam) {
    ...
    const endpoint = centerType.value ? 'cancelcardapplication/page' : 'cancelcardapplication/list'
...
async function fetchHotRecommend(force = false) {
  ...
  try {
    const endpoint = 'cancelcardapplication/' + (token.value ? 'autoSort2' : 'autoSort')
