        ...
        <div class="detail-content">
          ...
          <div class="infos">
            ...
            <div class="info-item">
              ...
              <span class="text">{{ info.renewaldays }}</span>
            </div>
            <div class="info-item">
          ...
          <div class="actions">
            ...
            <el-button
              size="small"
              type="primary"
              v-if="hasActionPermission('取消续卡记录')"
              :disabled="isCrossDisabled('取消续卡记录')"
              :title="crossDisabledReason('取消续卡记录') || undefined"
              @click="onCross('取消续卡记录')"
            >取消续卡记录</el-button>
            <el-button
              size="small"
              type="primary"
...
const tableButtonsPermission = useTableButtons('cardrenewal')
...
const payQuantityField = 'renewaldays'
...
async function submitPay() {
  ...
  try {
    ...
    const res: any = await http.post('cardrenewal/update', payload)
    if (res?.code === 0) {
      info.ispay = '已支付'
...
async function onCross(name: string){
  ...
  try {
    ...
    const inlineOnly = !targetTableName || targetTableName === 'cardrenewal'
    ...
    if (targetTableName === 'cart' && name === '加入购物车') {
      try {
        ...
        const payload: Record<string, any> = {
          tablename: (localStorage.getItem('crossTable') || 'cardrenewal') || 'cardrenewal',
...
async function inlineCrossAction(
  ...
  try {
    if (statusKey && statusTarget) {
      ...
      await http.post('cardrenewal/update', payload)
    ...
    const refId = (info as any)?.crossrefid || (target === 'cardrenewal' ? (info as any)?.id : undefined)
    if (!refId) {
