  ...
  <div v-if="canView" class="front-list-page">
    <div class="breadcrumb-container">
      <el-breadcrumb class="breadcrumb-box" :separator="'>'">
        ...
        <el-breadcrumb-item class="breadcrumb-item item-module">
          <a href="javascript:void(0);">续卡记录</a>
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
      ...
      >
        ...
        <div class="info-box">
          ...
          <div class="info-item">
            ...
            <span class="value">{{ formatField(item.renewaldays) }}</span>
          </div>
          <div class="info-item">
    ...
    <section v-if="showHotRecommend && hotRecommend.length" class="area-container hot-recommend">
      <div class="area-title-box">
        <h2 class="area-title">续卡记录热门推荐</h2>
        <div class="area-subhead">CARDRENEWAL RECOMMEND</div>
      </div>
...
const canViewPermission = useAuth('cardrenewal', '查看')
const canCreate = !false && useAuth('cardrenewal', '新增')
const canView = computed(() => canViewPermission.value || !token.value)

const imageField = null
...
const detailRoute = '/index/cardrenewalDetail'
...
function handleAdd() {
  const query: Record<string, any> = {}
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/cardrenewalAdd', query })
}
...
function toDetail(row: Record<string, any>) {
  ...
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/cardrenewalDetail', query })
}
...
async function fetchList(page = pageIndex.value) {
  ...
  try {
    ...
    if (token.value) {
      const preferParam = resolveCatePreferParam('cardrenewal')
      if (preferParam) {
    ...
    const endpoint = centerType.value ? 'cardrenewal/page' : 'cardrenewal/list'
...
async function fetchHotRecommend(force = false) {
  ...
  try {
    const endpoint = 'cardrenewal/' + (token.value ? 'autoSort2' : 'autoSort')
