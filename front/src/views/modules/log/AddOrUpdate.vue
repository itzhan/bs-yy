  ...
  <div class="form-container front-add-page">
    ...
    <el-form-item label="操作类型" prop="operatetype">
      <el-input
        ...
        :placeholder="'请输入操作类型'"
        :readonly="read.operatetype"
        clearable
      />
    </el-form-item>
    <el-form-item label="业务ID" prop="businessid">
      <el-input-number v-model="form.businessid" placeholder="请输入业务ID" :step="1" :controls="false" :disabled="read.businessid" />
    </el-form-item>
    <el-form-item label="操作人ID" prop="operatorid">
      <el-input-number v-model="form.operatorid" placeholder="请输入操作人ID" :step="1" :controls="false" :disabled="read.operatorid" />
    </el-form-item>
    <el-form-item label="操作人名称" prop="operatorname">
      <el-input
        v-model="form.operatorname"
        :placeholder="'请输入操作人名称'"
        :readonly="read.operatorname"
        clearable
      />
    </el-form-item>
    ...
    <el-form-item label="请求IP" prop="requestip">
      <el-input
        ...
        :placeholder="'请输入请求IP'"
        :readonly="read.requestip"
        clearable
      />
    </el-form-item>
    <el-form-item label="操作明细" prop="detail">
      <el-input
        v-model="form.detail"
        type="textarea"
        :rows="4"
        :placeholder="'请输入操作明细'"
        :readonly="read.detail"
        clearable
      />
    </el-form-item>
...
const rules = reactive<FormRules>({
    'tablename': [
      { required: false, message: '请输入业务表', trigger: 'blur' }
    ],
    'module': [
    ...
    'operatorid': [
      { required: false, message: '请输入操作人ID', trigger: 'blur' }
    ],
    'operatorname': [
      { required: false, message: '请输入操作人名称', trigger: 'blur' }
    ],
    'requesturl': [
      { required: false, message: '请输入请求路径', trigger: 'blur' }
    ],
    'requestmethod': [
      { required: false, message: '请输入请求方法', trigger: 'blur' }
    ],
    'requestip': [
      { required: false, message: '请输入请求IP', trigger: 'blur' }
    ],
    'detail': [
      { required: false, message: '请输入操作明细', trigger: 'blur' }
    ],
    'addtime': [
      { required: false, message: '请输入记录时间', trigger: 'blur' }
    ],
    'userid': [
...
async function loadDetail(id: number) {
  try {
    resetReadState()
    const res: any = await http.get('log/detail/' + id)
...
async function handleSubmit() {
  ...
  try {
    ...
    const api = isEdit.value && !isCross.value ? 'log/update' : 'log/add'
    const payload = { ...form }
