  ...
  <div v-if="canView" class="front-list-page">
    ...
    <el-form :inline="true" :model="formSearch" class="search-container">
      ...
      <el-form-item class="search-item">
        ...
        <el-input
          v-model="formSearch.operatetype"
          placeholder="请输入操作类型"
          @keyup.enter.native="triggerSearch()"
          clearable
        />
      ...
      <el-form-item class="search-item">
        <span class="label">操作人名称</span>
        <el-input
          v-model="formSearch.operatorname"
    ...
    <div v-if="!mapConfig.enabled" class="list-container">
      ...
      >
        ...
        <div class="info-box">
          ...
          <div class="info-item">
            <span class="label">模块名称：</span>
            <span class="value">{{ formatField(item.module) }}</span>
          </div>
          <div class="info-item">
            <span class="label">操作类型：</span>
            <span class="value">{{ formatField(item.operatetype) }}</span>
          </div>
          <div class="info-item">
            <span class="label">操作人名称：</span>
            <span class="value">{{ formatField(item.operatorname) }}</span>
          </div>
          <div class="info-item">
            <span class="label">请求路径：</span>
            <span class="value">{{ formatField(item.requesturl) }}</span>
          </div>
          <div class="info-item">
            <span class="label">请求方法：</span>
            <span class="value">{{ formatField(item.requestmethod) }}</span>
          </div>
          <div class="info-item">
            <span class="label">请求IP：</span>
            <span class="value">{{ formatField(item.requestip) }}</span>
          </div>
    ...
    <section v-if="showHotRecommend && hotRecommend.length" class="area-container hot-recommend">
      <div class="area-title-box">
        <h2 class="area-title">操作日志热门推荐</h2>
        <div class="area-subhead">LOG RECOMMEND</div>
      </div>
...
const canViewPermission = useAuth('log', '查看')
const canCreate = !true && useAuth('log', '新增')
const canView = computed(() => canViewPermission.value || !token.value)

const imageField = null
const titleField = 'tablename'
const summaryFields = [
...
const detailRoute = '/index/logDetail'
...
function handleAdd() {
  const query: Record<string, any> = {}
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/logAdd', query })
}
...
function toDetail(row: Record<string, any>) {
  ...
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/logDetail', query })
}
...
async function fetchList(page = pageIndex.value) {
  ...
  try {
    ...
    if (token.value) {
      const preferParam = resolveCatePreferParam('log')
      if (preferParam) {
    ...
    if (formSearch.module) {
      query.module = '%' + formSearch.module + '%'
    }
    if (formSearch.operatetype) {
      query.operatetype = '%' + formSearch.operatetype + '%'
    }
    if (formSearch.operatorname) {
      query.operatorname = '%' + formSearch.operatorname + '%'
    }
    if (formSearch.requesturl) {
      query.requesturl = '%' + formSearch.requesturl + '%'
    }
    if (formSearch.requestmethod) {
      query.requestmethod = '%' + formSearch.requestmethod + '%'
    }
    ...
    const endpoint = centerType.value ? 'log/page' : 'log/list'
...
async function fetchHotRecommend(force = false) {
  ...
  try {
    const endpoint = 'log/' + (token.value ? 'autoSort2' : 'autoSort')
