  ...
  <div v-if="canView" class="front-list-page">
    <div class="breadcrumb-container">
      <el-breadcrumb class="breadcrumb-box" :separator="'>'">
        ...
        <el-breadcrumb-item class="breadcrumb-item item-module">
          <a href="javascript:void(0);">健身商品</a>
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    ...
    <el-form :inline="true" :model="formSearch" class="search-container">
      ...
      <div class="range-filter">
        <el-form-item class="search-item">
          ...
          <el-input-number
            v-model="formSearch.productprice_start"
            placeholder="价格开始"
            :controls="false"
          />
        </el-form-item>
        <el-form-item class="search-item">
          ...
          <el-input-number
            v-model="formSearch.productprice_end"
            placeholder="价格结束"
            :controls="false"
          />
        </el-form-item>
      ...
      >
        ...
        <div class="info-box">
          ...
          <div class="info-item">
            ...
            <span class="value" :class="Number(item.stock||0) <= 0 ? 'soldout' : ''">{{ Number(item.stock||0) }}</span>
            <el-tag type="danger" v-if="Number(item.stock||0) <= 0" size="small">售罄</el-tag>
          </div>
          <div class="info-item">
          ...
          <div class="info-item">
            <span class="label">库存：</span>
            <span class="value">{{ formatField(item.stock) }}</span>
          </div>
          <div class="info-item">
          ...
          <div class="info-item">
            ...
            <span class="value">{{ formatField(item.auditstatus) }}</span>
          </div>
    ...
    <section v-if="showHotRecommend && hotRecommend.length" class="area-container hot-recommend">
      <div class="area-title-box">
        <h2 class="area-title">健身商品热门推荐</h2>
        <div class="area-subhead">PRODUCT RECOMMEND</div>
      </div>
...
const canCreate = !false && useAuth('product', '新增')
...
const categoryConfig = { field: 'producttype', label: '商品分类', table: 'producttype', column: 'producttypename' }
const showLike = true
const showFavorite = true
const showClick = true
...
const detailRoute = '/index/productDetail'
...
function handleAdd() {
  const query: Record<string, any> = {}
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/productAdd', query })
}
...
function toDetail(row: Record<string, any>) {
  ...
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/productDetail', query })
}
...
async function fetchList(page = pageIndex.value) {
  ...
  try {
    ...
    if (token.value) {
      const preferParam = resolveCatePreferParam('product')
      if (preferParam) {
    ...
    if (formSearch.productname) {
      query.productname = '%' + formSearch.productname + '%'
    }


    if (formSearch.productprice_start) query.productprice_start = formSearch.productprice_start
    if (formSearch.productprice_end) query.productprice_end = formSearch.productprice_end
    ...
    const endpoint = centerType.value ? 'product/page' : 'product/list'
...
async function fetchHotRecommend(force = false) {
  ...
  try {
    const endpoint = 'product/' + (token.value ? 'autoSort2' : 'autoSort')
