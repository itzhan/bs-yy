        ...
        <div class="detail-content">
          ...
          <div class="infos">
            ...
            <div class="info-item">
              ...
              <span class="text">{{ info.producttype }}</span>
            </div>
            <div class="info-item">
              <span class="label">价格：</span>
              <span class="text">{{ info.productprice }}</span>
            </div>
            <div class="info-item">
              <span class="label">库存：</span>
              <span class="text">{{ info.stock }}</span>
            </div>
            <div class="info-item">
              <span class="label">商品描述：</span>
              <div class="text rich-text" v-html="info.productdesc || ''"></div>
            </div>
          </div>
          ...
          <div class="actions">
            ...
            <el-button
              size="small"
              type="primary"
              v-if="hasActionPermission('购买')"
              :disabled="isCrossDisabled('购买')"
              :title="crossDisabledReason('购买') || undefined"
              @click="onCross('购买')"
            >购买</el-button>
          </div>
        </div>
...
const canViewPermission = useAuth('product', '查看')
const tableButtonsPermission = useTableButtons('product')
...
const discussTableName = 'discussproduct'
...
async function submitAudit() {
  ...
  try {
    ...
    const url = 'product/audit/batch?auditstatus=' + status + '&auditreply=' + reply
...
async function loadDetail(id: number) {
  ...
  const res: any = await http.get('product/detail/' + id)
...
async function like(type: '1'|'2'){
  await http.get(`product/like/` + info.id, { params: { type } })
...
async function toggleFav(){
  ...
  if (fav.value) {
    ...
    try {
      if (!favId.value) {
        const st: any = await http.get('social/list', { params: { page: 1, limit: 1, tablename: 'product', refid: info.id, type: '1' } })
      ...
      favId.value = null
      try { await http.post('product/update', info) } catch(e) {}
      await refreshSocialStatus(info.id)
    }
  } else {
    // 添加收藏：写 social
    const payload: any = { tablename: 'product', refid: info.id, type: '1' }
    // 供后端展示，可带上名称与图片
    try { payload.name = String(info.id) } catch (e) {}
    const coverField = 'productimage'
...
async function doLike(){
  ...
  const payload: any = { tablename: 'product', refid: info.id, type: '21' }
  try { payload.name = String(info.id) } catch (e) {}
  const coverField2 = 'productimage'
...
async function cancelLike(){
  ...
  try {
    if (!likeId.value) {
      const st: any = await http.get('social/list', { params: { page: 1, limit: 1, tablename: 'product', refid: info.id, type: '21' } })
      const arr = st?.data?.list || []
...
async function doStep(){
  if (liked.value || stepped.value) return
  const payload: any = { tablename: 'product', refid: info.id, type: '22' }
  try { payload.name = String(info.id) } catch (e) {}
  const coverField3 = 'productimage'
...
async function cancelStep(){
  ...
  try {
    if (!stepId.value) {
      const st: any = await http.get('social/list', { params: { page: 1, limit: 1, tablename: 'product', refid: info.id, type: '22' } })
      const arr = st?.data?.list || []
...
async function loadDiscuss(page = discussPage.value) {
  ...
  try {
    ...
    const res: any = await http.get('discussproduct/list', { params })
    const data = res?.data || {}
...
async function submitDiscuss() {
  ...
  try {
    ...
    const res: any = await http.post('discussproduct/add', payload)
    if (res?.code === 0) {
...
async function updateDiscussCounters(item: any) {
  try {
    await http.post('discussproduct/update', {
      id: item.id,
...
async function deleteDiscuss(item: any) {
  ...
  setDiscussActionLoading(item, true)
  try {
    const res: any = await http.post('discussproduct/delete', [item.id])
    if (res?.code === 0) {
...
const crossButtonsConfig: CrossButtonConfig[] = [
  {
    ...
    statusField: '',
    statusValue: '',
    stockTargetTable: 'product',
    stockMode: 'minus',
    stockField: 'stock',
    stockAmountField: 'stock',
    prevValue: null,
    prevButtonName: '',
    flowIndex: -1
  }]
...
async function onCross(name: string){
  ...
  try {
    ...
    const inlineOnly = !targetTableName || targetTableName === 'product'
    ...
    if (targetTableName === 'cart' && name === '加入购物车') {
      try {
        ...
        const payload: Record<string, any> = {
          tablename: (localStorage.getItem('crossTable') || 'product') || 'product',
...
async function inlineCrossAction(
  ...
  try {
    if (statusKey && statusTarget) {
      ...
      await http.post('product/update', payload)
    ...
    const refId = (info as any)?.crossrefid || (target === 'product' ? (info as any)?.id : undefined)
    if (!refId) {
...
async function refreshSocialStatus(refid: number){
  ...
  try {
    const st: any = await http.get('social/list', { params: { page: 1, limit: 1, tablename: 'product', refid, type: '1' } })
    const arr = st?.data?.list || []
    fav.value = arr.length > 0
    favId.value = arr.length ? arr[0].id : null
  } catch(e) {}
  try {
    const stLike: any = await http.get('social/list', { params: { page: 1, limit: 1, tablename: 'product', refid, type: '21' } })
    const likeArr = stLike?.data?.list || []
    liked.value = likeArr.length > 0
    likeId.value = likeArr.length ? likeArr[0].id : null
  } catch(e) {}
  try {
    const stStep: any = await http.get('social/list', { params: { page: 1, limit: 1, tablename: 'product', refid, type: '22' } })
