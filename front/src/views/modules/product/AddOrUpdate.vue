<template>
  <div class="form-container front-add-page">
    ...
    <el-form-item label="价格" prop="productprice">
      <el-input-number v-model="form.productprice" placeholder="请输入价格" :step="1" :controls="false" :disabled="read.productprice" />
    </el-form-item>
    <el-form-item label="库存" prop="stock">
      <el-input-number v-model="form.stock" placeholder="请输入库存" :step="1" :controls="false" :disabled="read.stock" />
    </el-form-item>
...
const rules = reactive<FormRules>({
    ...
    'favoritenum': [
      { required: false, message: '请输入收藏数', trigger: 'blur' }
    ],
    'likenum': [
      { required: false, message: '请输入点赞数', trigger: 'blur' }
    ],
    'stepnum': [
      { required: false, message: '请输入踩数', trigger: 'blur' }
    ],
    'clicknum': [
      { required: false, message: '请输入点击数', trigger: 'blur' }
    ],
    'clicktime': [
      { required: false, message: '请输入最后点击时间', trigger: 'blur' }
    ],
    'auditstatus': [
      { required: false, message: '请输入审核状态', trigger: 'blur' }
    ],
    'auditreply': [
      { required: false, message: '请输入审核回复', trigger: 'blur' }
    ],
    'discussnum': [
...
async function handleSubmit() {
  ...
  try {
    ...
    const api = isEdit.value && !isCross.value ? 'product/update' : 'product/add'
    const payload = { ...form }
