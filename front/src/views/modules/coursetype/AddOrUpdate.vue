...
async function loadDetail(id: number) {
  try {
    resetReadState()
    const res: any = await http.get('coursetype/detail/' + id)
...
async function handleSubmit() {
  ...
  try {
    ...
    const api = isEdit.value && !isCross.value ? 'coursetype/update' : 'coursetype/add'
    const payload = { ...form }
