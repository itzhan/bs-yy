  ...
  <div v-if="canView" class="front-list-page">
    ...
    <el-form :inline="true" :model="formSearch" class="search-container">
      <el-form-item class="search-item">
        ...
        <el-input
          v-model="formSearch.coursetypename"
          placeholder="请输入分类名称"
    ...
    <section v-if="showHotRecommend && hotRecommend.length" class="area-container hot-recommend">
      <div class="area-title-box">
        <h2 class="area-title">课程分类热门推荐</h2>
        <div class="area-subhead">COURSETYPE RECOMMEND</div>
      </div>
...
const canViewPermission = useAuth('coursetype', '查看')
const canCreate = !false && useAuth('coursetype', '新增')
const canView = computed(() => canViewPermission.value || !token.value)

const imageField = null
const titleField = 'coursetypename'
const summaryFields = [
...
const detailRoute = '/index/coursetypeDetail'
...
function handleAdd() {
  const query: Record<string, any> = {}
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/coursetypeAdd', query })
}
...
function toDetail(row: Record<string, any>) {
  ...
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/coursetypeDetail', query })
}
...
async function fetchList(page = pageIndex.value) {
  ...
  try {
    ...
    if (token.value) {
      const preferParam = resolveCatePreferParam('coursetype')
      if (preferParam) {
    ...
    if (formSearch.coursetypename) {
      query.coursetypename = '%' + formSearch.coursetypename + '%'
    }
    ...
    const endpoint = centerType.value ? 'coursetype/page' : 'coursetype/list'
...
async function fetchHotRecommend(force = false) {
  ...
  try {
    const endpoint = 'coursetype/' + (token.value ? 'autoSort2' : 'autoSort')
