  ...
  <div v-if="canView" class="front-list-page">
    <div class="breadcrumb-container">
      <el-breadcrumb class="breadcrumb-box" :separator="'>'">
        ...
        <el-breadcrumb-item class="breadcrumb-item item-module">
          <a href="javascript:void(0);">好友表</a>
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>
      ...
      >
        ...
        <div class="info-box">
          ...
          <div class="info-item">
            ...
            <span class="value">{{ formatField(item.role) }}</span>
          </div>
          <div class="info-item">
            <span class="label">表名：</span>
            <span class="value">{{ formatField(item.tablename) }}</span>
          </div>
          <div class="info-item">
            <span class="label">好友备注：</span>
            <span class="value">{{ formatField(item.alias) }}</span>
          </div>
          <div class="info-item">
            <span class="label">关系类型：</span>
            <span class="value">{{ formatField(item.type) }}</span>
          </div>
          <div class="info-item">
    ...
    <section v-if="showHotRecommend && hotRecommend.length" class="area-container hot-recommend">
      <div class="area-title-box">
        <h2 class="area-title">好友表热门推荐</h2>
        <div class="area-subhead">FRIEND RECOMMEND</div>
      </div>
...
const canViewPermission = useAuth('friend', '查看')
const canCreate = !false && useAuth('friend', '新增')
const canView = computed(() => canViewPermission.value || !token.value)

const imageField = 'picture'
...
const detailRoute = '/index/friendDetail'
...
function handleAdd() {
  const query: Record<string, any> = {}
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/friendAdd', query })
}
...
function toDetail(row: Record<string, any>) {
  ...
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/friendDetail', query })
}
...
async function fetchList(page = pageIndex.value) {
  ...
  try {
    ...
    if (token.value) {
      const preferParam = resolveCatePreferParam('friend')
      if (preferParam) {
    ...
    const endpoint = centerType.value ? 'friend/page' : 'friend/list'
...
async function fetchHotRecommend(force = false) {
  ...
  try {
    const endpoint = 'friend/' + (token.value ? 'autoSort2' : 'autoSort')
