<template>
  <div class="form-container front-add-page">
    ...
    <el-form-item label="好友备注" prop="alias">
      <el-input
        v-model="form.alias"
        :placeholder="'请输入好友备注'"
        :readonly="read.alias"
    ...
    <el-form-item label="关系类型" prop="type">
      <el-input-number v-model="form.type" placeholder="请输入关系类型" :step="1" :controls="false" :disabled="read.type" />
    </el-form-item>
...
const rules = reactive<FormRules>({
    ...
    'name': [
      { required: false, message: '请输入好友昵称', trigger: 'blur' }
    ],
    'picture': [
      { required: false, message: '请输入头像', trigger: 'blur' }
    ],
    'role': [
      { required: false, message: '请输入角色标识', trigger: 'blur' }
    ],
    'tablename': [
      { required: false, message: '请输入表名', trigger: 'blur' }
    ],
    'alias': [
      { required: false, message: '请输入好友备注', trigger: 'blur' }
    ],
    'type': [
      { required: false, message: '请输入关系类型', trigger: 'blur' }
    ],
    'addtime': [
...
async function loadDetail(id: number) {
  try {
    resetReadState()
    const res: any = await http.get('friend/detail/' + id)
...
async function handleSubmit() {
  ...
  try {
    ...
    const api = isEdit.value && !isCross.value ? 'friend/update' : 'friend/add'
    const payload = { ...form }
