  ...
  <div v-if="canView" class="front-list-page">
    ...
    <div class="filter-container">
      ...
      <div class="options">
        <div
          class="option"
          :class="selectState.readstatus === -1 ? 'active' : ''"
          @click="selectDictOption('readstatus', -1)"
        >全部</div>
        <div
          class="option"
          :class="selectState.readstatus === 0 ? 'active' : ''"
          @click="selectDictOption('readstatus', 0)"
        >未读</div>
        <div
          class="option"
          :class="selectState.readstatus === 1 ? 'active' : ''"
      ...
      >
        ...
        <div class="info-box">
          ...
          <div class="info-item">
            ...
            <span class="value">{{ formatField(item.messagetype) }}</span>
          </div>
          <div class="info-item">
            <span class="label">阅读状态：</span>
            <span class="value">{{ formatField(item.readstatus) }}</span>
          </div>
          <div class="info-item">
            <span class="label">发送人：</span>
            <span class="value">{{ formatField(item.senduser) }}</span>
          </div>
          <div class="info-item">
    ...
    <section v-if="showHotRecommend && hotRecommend.length" class="area-container hot-recommend">
      <div class="area-title-box">
        <h2 class="area-title">消息通知热门推荐</h2>
        <div class="area-subhead">NOTIFY RECOMMEND</div>
      </div>
...
const canViewPermission = useAuth('notify', '查看')
const canCreate = !false && useAuth('notify', '新增')
...
const categoryConfig = { field: 'messagetype', label: '消息类型', table: 'notify', column: 'messagetype' }
...
const detailRoute = '/index/notifyDetail'
...
function handleAdd() {
  const query: Record<string, any> = {}
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/notifyAdd', query })
}
...
function toDetail(row: Record<string, any>) {
  ...
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/notifyDetail', query })
}
...
async function fetchList(page = pageIndex.value) {
  ...
  try {
    ...
    if (token.value) {
      const preferParam = resolveCatePreferParam('notify')
      if (preferParam) {
    ...
    if (formSearch.title) {
      query.title = '%' + formSearch.title + '%'
    }

    if (formSearch.readstatus) {
      query.readstatus = formSearch.readstatus
    }
    ...
    const endpoint = centerType.value ? 'notify/page' : 'notify/list'
...
async function fetchHotRecommend(force = false) {
  ...
  try {
    const endpoint = 'notify/' + (token.value ? 'autoSort2' : 'autoSort')
