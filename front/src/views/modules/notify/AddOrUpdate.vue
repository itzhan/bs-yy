<template>
  <div class="form-container front-add-page">
    ...
    <el-form-item label="消息类型" prop="messagetype">
      <el-select v-model="form.messagetype" placeholder="请选择消息类型" clearable :disabled="read.messagetype">
        <el-option
          v-for="opt in categoryOptions.messagetype"
          :key="opt"
          :label="opt"
          :value="opt"
        />
    ...
    <el-form-item label="阅读状态" prop="readstatus">
      <el-select v-model="form.readstatus" placeholder="请选择阅读状态" clearable :disabled="read.readstatus">
        <el-option
          v-for="opt in dictOptions.readstatus"
          :key="opt"
          :label="opt"
          :value="opt"
        />
...
const rules = reactive<FormRules>({
    'userid': [
      { required: false, message: '请输入接收用户ID', trigger: 'blur' }
    ],
    'title': [
      { required: false, message: '请输入消息标题', trigger: 'blur' }
    ],
    'content': [
      { required: false, message: '请输入消息内容', trigger: 'blur' }
    ],
    'messagetype': [
      { required: false, message: '请输入消息类型', trigger: 'blur' }
    ],
    'readstatus': [
      { required: false, message: '请输入阅读状态', trigger: 'blur' }
    ],
    'senduser': [
      { required: false, message: '请输入发送人', trigger: 'blur' }
    ],
    'addtime': [
...
async function loadCategoryOptions() {
  try {
    const res: any = await http.get('notify/list', { params: { page: 1, limit: 1000, sort: 'id', order: 'asc' } })
    const list = Array.isArray(res?.data?.list) ? res.data.list : Array.isArray(res?.data?.data?.list) ? res.data.data.list : []
    categoryOptions['messagetype'] = list
      .map((item: any) => item?.['messagetype'])
      .filter(Boolean)
  } catch (error) {
    categoryOptions['messagetype'] = []
  }
}
...
async function loadDetail(id: number) {
  try {
    resetReadState()
    const res: any = await http.get('notify/detail/' + id)
...
async function handleSubmit() {
  ...
  try {
    ...
    const api = isEdit.value && !isCross.value ? 'notify/update' : 'notify/add'
    const payload = { ...form }
