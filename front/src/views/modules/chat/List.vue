...
<template>
  <div v-if="canView" class="front-list-page">
    ...
    <el-form :inline="true" :model="formSearch" class="search-container">
      ...
      <el-form-item class="search-item">
        <span class="label">是否回复(0/1)</span>
        <el-input
          v-model="formSearch.isreply"
          placeholder="请输入是否回复(0/1)"
          @keyup.enter.native="triggerSearch()"
          clearable
        />
      </el-form-item>
      <el-form-item class="search-item">
        <span class="label">是否已读(0/1)</span>
        <el-input
          v-model="formSearch.isread"
          placeholder="请输入是否已读(0/1)"
          @keyup.enter.native="triggerSearch()"
          clearable
        />
      </el-form-item>
    ...
    <div v-if="!mapConfig.enabled" class="list-container">
      ...
      >
        ...
        <div class="info-box">
          ...
          <div class="info-item">
            <span class="label">是否回复(0/1)：</span>
            <span class="value">{{ formatField(item.isreply) }}</span>
          </div>
          <div class="info-item">
            <span class="label">是否已读(0/1)：</span>
    ...
    <section v-if="showHotRecommend && hotRecommend.length" class="area-container hot-recommend">
      <div class="area-title-box">
        <h2 class="area-title">在线客服热门推荐</h2>
        <div class="area-subhead">CHAT RECOMMEND</div>
      </div>
...
const canViewPermission = useAuth('chat', '查看')
const canCreate = !false && useAuth('chat', '新增')
const canView = computed(() => canViewPermission.value || !token.value)

const imageField = 'uimage'
const titleField = 'uname'
const summaryFields = [
...
const detailRoute = '/index/chatDetail'
...
function handleAdd() {
  const query: Record<string, any> = {}
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/chatAdd', query })
}
...
function toDetail(row: Record<string, any>) {
  ...
  if (centerType.value) query.centerType = 1
  router.push({ path: '/index/chatDetail', query })
}
...
async function fetchList(page = pageIndex.value) {
  ...
  try {
    ...
    if (token.value) {
      const preferParam = resolveCatePreferParam('chat')
      if (preferParam) {
    ...
    if (formSearch.userid) {
      query.userid = formSearch.userid
    }
    if (formSearch.adminid) {
      query.adminid = formSearch.adminid
    }
    if (formSearch.isreply) {
      query.isreply = formSearch.isreply
    }
    if (formSearch.isread) {
      query.isread = formSearch.isread
    }
    if (formSearch.uname) {
      query.uname = '%' + formSearch.uname + '%'
    }
    ...
    const endpoint = centerType.value ? 'chat/page' : 'chat/list'
...
async function fetchHotRecommend(force = false) {
  ...
  try {
    const endpoint = 'chat/' + (token.value ? 'autoSort2' : 'autoSort')
