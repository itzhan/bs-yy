<template>
  <div class="form-container front-add-page">
    ...
    <el-form-item label="管理员ID" prop="adminid">
      <el-input-number v-model="form.adminid" placeholder="请输入管理员ID" :step="1" :controls="false" :disabled="read.adminid" />
    </el-form-item>
    <el-form-item label="提问" prop="ask">
      <el-input
        v-model="form.ask"
        type="textarea"
        :rows="4"
    ...
    <el-form-item label="提问类型(1文本/2图片)" prop="asktype">
      <el-input-number v-model="form.asktype" placeholder="请输入提问类型(1文本/2图片)" :step="1" :controls="false" :disabled="read.asktype" />
    </el-form-item>
    <el-form-item label="回复类型(1文本/2图片)" prop="replytype">
      <el-input-number v-model="form.replytype" placeholder="请输入回复类型(1文本/2图片)" :step="1" :controls="false" :disabled="read.replytype" />
    </el-form-item>
    <el-form-item label="是否回复(0/1)" prop="isreply">
      <el-input-number v-model="form.isreply" placeholder="请输入是否回复(0/1)" :step="1" :controls="false" :disabled="read.isreply" />
    </el-form-item>
    <el-form-item label="是否已读(0/1)" prop="isread">
      <el-input-number v-model="form.isread" placeholder="请输入是否已读(0/1)" :step="1" :controls="false" :disabled="read.isread" />
    </el-form-item>
    ...
    <el-form-item label="用户头像" prop="uimage">
      <UploadField
        v-model="form.uimage"
        upload-type="image"
        :readonly="read.uimage"
      />
    </el-form-item>
...
const rules = reactive<FormRules>({
    ...
    'ask': [
      { required: false, message: '请输入提问', trigger: 'blur' }
    ],
    'reply': [
      { required: false, message: '请输入回复', trigger: 'blur' }
    ],
    'asktype': [
      { required: false, message: '请输入提问类型(1文本/2图片)', trigger: 'blur' }
    ],
    'replytype': [
      { required: false, message: '请输入回复类型(1文本/2图片)', trigger: 'blur' }
    ],
    'isreply': [
      { required: false, message: '请输入是否回复(0/1)', trigger: 'blur' }
    ],
    'isread': [
      { required: false, message: '请输入是否已读(0/1)', trigger: 'blur' }
    ],
    'uname': [
      { required: false, message: '请输入用户名', trigger: 'blur' }
    ],
    'uimage': [
      { required: false, message: '请输入用户头像', trigger: 'blur' }
    ],
    'addtime': [
...
async function loadDetail(id: number) {
  try {
    resetReadState()
    const res: any = await http.get('chat/detail/' + id)
...
async function handleSubmit() {
  ...
  try {
    ...
    const api = isEdit.value && !isCross.value ? 'chat/update' : 'chat/add'
    const payload = { ...form }
