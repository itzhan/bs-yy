<template>
  <div class="map-distribution">
    ...
    <div class="list-panel">
      ...
      <div v-if="activeMarkers.length" class="list-body">
        <div
...
const detailRoutePath = computed(() => (props.detailRoute || '').trim())
...
async function ensureMap() {
  ...
  const globalAMap = AMap?.Map ? AMap : await waitForAmapReady()
  if (!globalAMap?.Map) {
  ...
  map = new globalAMap.Map(mapContainer.value, options)
  globalAMap.plugin(['AMap.ToolBar', 'AMap.Scale', 'AMap.MapType'], () => {
    map.addControl(new globalAMap.ToolBar({ position: 'RB' }))
    map.addControl(new globalAMap.Scale({ position: 'LB' }))
    map.addControl(new globalAMap.MapType({ position: 'RT' }))
  })
  return map
}
...
const buildDetailQuery = (id: string | number) => {
  const extra = props.detailQuery || {}
...
function navigateToDetail(item: MarkerInternal) {
  if (!detailRoutePath.value) return
  router.push({ path: detailRoutePath.value, query: buildDetailQuery(item.id) })
}
...
function buildInfoWindowContent(item: MarkerInternal): HTMLElement {
  ...
  if (detailRoutePath.value) {
    ...
    button.style.background = '#2161ff'
