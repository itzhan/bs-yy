  ...
  <div class="main-content bg-gray-50 p-6">
    ...
    <div class="bg-white p-4 rounded-lg shadow mb-4">
      <div class="flex justify-between items-center">
        <div class="space-x-2">
          <el-button
            v-if="!false && isAuth('courseenrollment', '新增')"
            type="success"
          ...
          <el-button
            v-if="!false && isAuth('courseenrollment', '删除')"
            type="danger"
          ...
          <download-excel
            v-if="isAuth('courseenrollment', '导出')"
          ...
          <el-button
           v-if="isAuth('courseenrollment', '支付')"
           type="success"
           :disabled="!dataListSelections.length"
           @click="payBatch()"
          >
    ...
    <div v-if="!mapConfig.enabled || viewMode === 'table'" class="bg-white rounded-lg shadow">
      <el-table
        v-if="isAuth('courseenrollment', '查看')"
        class="w-full"
        :data="dataList"
      ...
      >
        ...
        <el-table-column prop="ispay" label="是否支付" width="160">
          <template #default="scope">
            <div class="flex items-center">
              ...
              <el-button
                v-if="scope.row.ispay !== '已支付' && isAuth('courseenrollment', '支付')"
                type="text"
        ...
        <el-table-column label="操作" width="240" fixed="right">
          <template #default="scope">
            ...
            <el-button
                v-if="isAuth('courseenrollment', '修改')"
                type="warning"
                size="small"
            ...
            <el-button
                v-if="isAuth('courseenrollment', '删除')"
                type="danger"
                size="small"
            ...
            <el-button
                v-if="isAuth('courseenrollment', '私聊')"
                type="success"
                size="small"
        ...
        <el-button
                v-if="isAuth('courseenrollment', '取消课程报名记录')"
                type="success"
                size="small"
                @click="cancelcourseenrollmentCrossAddOrUpdateHandler(scope.row, 'cross', '课程报名记录已取消', 'orderstatus', '已取消', '取消课程报名记录')"
            >
            取消课程报名记录
            </el-button>
        <el-button
                v-if="isAuth('courseenrollment', '退款')"
                type="success"
                size="small"
    ...
    <!-- cancelcourseenrollment 跨表组件 -->
    <cancelcourseenrollment-cross-add-or-update
    v-if="cancelcourseenrollmentCrossAddOrUpdateFlag"
    ref="cancelcourseenrollmentCrossAddOrUpdateRef"
    @refreshDataList="getDataList"
    ></cancelcourseenrollment-cross-add-or-update>
    <!-- refundcourseenrollment 跨表组件 -->
    <refundcourseenrollment-cross-add-or-update
    v-if="refundcourseenrollmentCrossAddOrUpdateFlag"
    ref="refundcourseenrollmentCrossAddOrUpdateRef"
    @refreshDataList="getDataList"
    ></refundcourseenrollment-cross-add-or-update>
...
<script setup lang="ts">
  ...
  import type { CourseenrollmentItem } from '@/types/common'
  ...
  import cancelcourseenrollmentCrossAddOrUpdate from '../cancelcourseenrollment/AddOrUpdate.vue'
  import refundcourseenrollmentCrossAddOrUpdate from '../refundcourseenrollment/AddOrUpdate.vue'
  ...
  const chatBackendTargetTable = 'courseenrollment'
  let websock: WebSocket | null = null

  // 响应式数据
  const dataList = ref<CourseenrollmentItem[]>([])
  const dataListLoading = ref(false)
  const dataListSelections = ref<CourseenrollmentItem[]>([])
  ...
  const musicTracks = computed(() => {
    ...
    return dataList.value
      .map(row => {
        ...
        return {
          id: record?.id ?? url,
          name: nameRaw || String(record?.name || '课程报名记录'),
          artist: artistRaw,
  ...
  const playMusic = (row: CourseenrollmentItem) => {
  ...
  // 跨表组件显示标志（去重）
  const cancelcourseenrollmentCrossAddOrUpdateFlag = ref(false)
  const cancelcourseenrollmentCrossAddOrUpdateRef = ref()
  const refundcourseenrollmentCrossAddOrUpdateFlag = ref(false)
  const refundcourseenrollmentCrossAddOrUpdateRef = ref()
  ...
  // 多选
  const selectionChangeHandler = (val: CourseenrollmentItem[]) => {
  ...
  const deleteHandler = async (ids?: number[]) => {
    ...
    try {
      ...
      await http.post('courseenrollment/delete', deleteIds)
  ...
  const payHandler = (row: any) => {
    localStorage.setItem('paytable', 'courseenrollment')
  ...
  // 取消课程报名记录
  const cancelcourseenrollmentCrossAddOrUpdateHandler = (row: any, type: string, tips: string, statusColumnName: string, statusColumnValue: string, buttonName: string) => {
    // 显示取消课程报名记录弹窗
    cancelcourseenrollmentCrossAddOrUpdateFlag.value = true

    localStorage.setItem('crossObj', JSON.stringify(row))
    localStorage.setItem('crossTable', 'courseenrollment')
    ...
    if (statusColumnName && !statusColumnName.startsWith('[')) {
      ...
      if (crossObj[statusColumnName] === statusColumnValue) {
        ...
        cancelcourseenrollmentCrossAddOrUpdateFlag.value = false
        return
      }
    }
    // 调用跨表组件
    nextTick(() => {
      cancelcourseenrollmentCrossAddOrUpdateRef.value?.init(undefined, type)
    })
  }
  
  // 退款
  const refundcourseenrollmentCrossAddOrUpdateHandler = (row: any, type: string, tips: string, statusColumnName: string, statusColumnValue: string, buttonName: string) => {
    // 显示退款弹窗
    refundcourseenrollmentCrossAddOrUpdateFlag.value = true
    ...
    if (statusColumnName && !statusColumnName.startsWith('[')) {
      ...
      if (crossObj[statusColumnName] === statusColumnValue) {
        ...
        refundcourseenrollmentCrossAddOrUpdateFlag.value = false
        return
      }
    }
    // 调用跨表组件
    nextTick(() => {
      refundcourseenrollmentCrossAddOrUpdateRef.value?.init(undefined, type)
    })
  }
  ...
  const inlineCrossActionHandler = async (
    ...
    try {
      ...
      if (statusColumnName && statusColumnValue) {
        ...
        await http.post(`courseenrollment/update`, curr)
