<template>
  <div class="add-edit-dialog">
    <el-dialog
        v-model="visible"
        :title="id ? (type === 'info' ? '查看课程报名记录' : '修改课程报名记录') : '新增课程报名记录'"
        width="60%"
    ...
    >
      ...
      >
        <el-form-item v-if="type === 'info' || true || read.coursename" label="课程名称" prop="coursename">
            <el-select v-model="form.coursename" placeholder="请选择课程名称" :readonly="type === 'info' || read.coursename">
                <el-option
        ...
        <el-form-item v-if="type === 'info' || false || read.courseimage" label="课程封面" prop="courseimage">
            <UploadField
        ...
        <el-form-item v-if="type === 'info' || false || read.coursetype" label="课程分类" prop="coursetype">
        ...
        <el-form-item v-if="type === 'info' || false || read.coachname" label="教练" prop="coachname">
            <el-input
        ...
        <el-form-item v-if="type === 'info' || false || read.useraccount" label="用户账号" prop="useraccount">
            <el-input
...
<script setup lang="ts">
  ...
  const getInfo = async (itemId: number) => {
    try {
      const { data } = await http.get(`courseenrollment/info/${itemId}`)
      if (data){
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = id.value ? 'courseenrollment/update' : 'courseenrollment/save'
        ...
        if (crossObj) {
          ...
          if (statusColumnName && !statusColumnName.startsWith('[')) {
            ...
            if (crossrefid && crossuserid) {
              ...
              const { data } = await http.get('courseenrollment/page', { params })
