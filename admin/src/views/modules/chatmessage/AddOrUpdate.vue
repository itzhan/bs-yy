<template>
  <div class="add-edit-dialog">
    <el-dialog
        v-model="visible"
        :title="id ? (type === 'info' ? '查看消息表' : '修改消息表') : '新增消息表'"
        width="60%"
    ...
    >
      ...
      >
        ...
        <el-form-item v-if="type === 'info' || true || read.format" label="格式(1:文字，2:图片)" prop="format">
            <el-input-number v-model="form.format" placeholder="请输入格式(1:文字，2:图片)" controls-position="right" :readonly="type === 'info' || read.format"/>
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.isread" label="消息已读(0:未读，1:已读)" prop="isread">
            <el-input-number v-model="form.isread" placeholder="请输入消息已读(0:未读，1:已读)" controls-position="right" :readonly="type === 'info' || read.isread"/>
        </el-form-item>
...
<script setup lang="ts">
  ...
  const rules: Record<string, any[]> = {
    ...
    format: [ { validator: validateNumber , trigger: 'blur' } ],
  ...
  const getInfo = async (itemId: number) => {
    try {
      const { data } = await http.get(`chatmessage/info/${itemId}`)
      if (data){
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = id.value ? 'chatmessage/update' : 'chatmessage/save'
        ...
        if (crossObj) {
          ...
          if (statusColumnName && !statusColumnName.startsWith('[')) {
            ...
            if (crossrefid && crossuserid) {
              ...
              const { data } = await http.get('chatmessage/page', { params })
