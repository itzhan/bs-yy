  ...
  <div class="main-content bg-gray-50 p-6">
    ...
    <div class="bg-white p-4 rounded-lg shadow mb-4">
      <el-form :inline="true" :model="searchForm" class="search-form">
        ...
        <div class="inline-block mr-4">
            <label class="inline-block mr-2 leading-10">关联表名</label>
            <el-input
    ...
    <div class="bg-white p-4 rounded-lg shadow mb-4">
      <div class="flex justify-between items-center">
        <div class="space-x-2">
          <el-button
            v-if="!false && isAuth('chatmessage', '新增')"
            type="success"
          ...
          <el-button
            v-if="!false && isAuth('chatmessage', '删除')"
            type="danger"
    ...
    <div v-if="!mapConfig.enabled || viewMode === 'table'" class="bg-white rounded-lg shadow">
      ...
      >
        ...
        <el-table-column prop="format" label="格式(1:文字，2:图片)" />
        <el-table-column prop="isread" label="消息已读(0:未读，1:已读)" />
        <!-- 操作列 -->
...
<script setup lang="ts">
  ...
  // 响应式数据
  const dataList = ref<ChatmessageItem[]>([])
  const dataListLoading = ref(false)
  const dataListSelections = ref<ChatmessageItem[]>([])
  ...
  const musicTracks = computed(() => {
    ...
    return dataList.value
      .map(row => {
        ...
        return {
          id: record?.id ?? url,
          name: nameRaw || String(record?.name || '消息表'),
          artist: artistRaw,
  ...
  const playMusic = (row: ChatmessageItem) => {
  ...
  const getDataList = () => {
    ...
    if (searchForm.tablename) {
      params.tablename = '%' + searchForm.tablename + '%'
    }
    http.get('chatmessage/page', { params }).then((response: any) => {
  ...
  const reset = () => {
    ...
    searchForm.tablename = undefined
    pageIndex.value = 1
    getDataList()
  }
  ...
  // 多选
  const selectionChangeHandler = (val: ChatmessageItem[]) => {
  ...
  const deleteHandler = async (ids?: number[]) => {
    ...
    try {
      ...
      await http.post('chatmessage/delete', deleteIds)
  ...
  const inlineCrossActionHandler = async (
    ...
    try {
      ...
      if (statusColumnName && statusColumnValue) {
        ...
        await http.post(`chatmessage/update`, curr)
