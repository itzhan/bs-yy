    ...
    <div class="bg-white p-4 rounded-lg shadow mb-4">
      <el-form :inline="true" :model="searchForm" class="search-form">
        <div class="inline-block mr-4">
            <label class="inline-block mr-2 leading-10">模块名称</label>
            <el-input
        ...
        <div class="inline-block mr-4">
            <label class="inline-block mr-2 leading-10">操作类型</label>
            <el-input
        ...
        <div class="inline-block mr-4">
            <label class="inline-block mr-2 leading-10">操作人ID</label>
            <el-input
        ...
        <div class="inline-block mr-4">
            <label class="inline-block mr-2 leading-10">操作人名称</label>
            <el-input
        ...
        <div class="inline-block mr-4">
            <label class="inline-block mr-2 leading-10">请求路径</label>
            <el-input
        ...
        <div class="inline-block mr-4">
            <label class="inline-block mr-2 leading-10">请求方法</label>
            <el-input
...
<script setup lang="ts">
  ...
  import type { LogItem } from '@/types/common'
  ...
  const chatBackendTargetTable = 'log'
  let websock: WebSocket | null = null

  // 响应式数据
  const dataList = ref<LogItem[]>([])
  const dataListLoading = ref(false)
  const dataListSelections = ref<LogItem[]>([])
  ...
  const musicTracks = computed(() => {
    ...
    return dataList.value
      .map(row => {
        ...
        return {
          id: record?.id ?? url,
          name: nameRaw || String(record?.name || '操作日志'),
          artist: artistRaw,
  ...
  const getDataList = () => {
    ...
    http.get('log/page', { params }).then((response: any) => {
  ...
  const reset = () => {
    ...
    searchForm.operatorname = undefined
    searchForm.requesturl = undefined
  ...
  // 多选
  const selectionChangeHandler = (val: LogItem[]) => {
  ...
  const deleteHandler = async (ids?: number[]) => {
    ...
    try {
      ...
      await http.post('log/delete', deleteIds)
  ...
  const inlineCrossActionHandler = async (
    ...
    try {
      ...
      if (statusColumnName && statusColumnValue) {
        ...
        await http.post(`log/update`, curr)
