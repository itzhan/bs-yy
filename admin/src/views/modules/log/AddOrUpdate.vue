<template>
  <div class="add-edit-dialog">
    <el-dialog
        v-model="visible"
        :title="id ? (type === 'info' ? '查看操作日志' : '修改操作日志') : '新增操作日志'"
        width="60%"
    ...
    >
      <el-form
        ...
        class="space-y-4"
      >
        <el-form-item v-if="type === 'info' || false || read.module" label="模块名称" prop="module">
            <el-input
                v-model="form.module"
                placeholder="请输入模块名称"
                :readonly="type === 'info' || read.module"
                clearable
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || false || read.operatetype" label="操作类型" prop="operatetype">
            <el-input
                v-model="form.operatetype"
                placeholder="请输入操作类型"
                :readonly="type === 'info' || read.operatetype"
                clearable
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || false || read.businessid" label="业务ID" prop="businessid">
            <el-input-number v-model="form.businessid" placeholder="请输入业务ID" controls-position="right" :readonly="type === 'info' || read.businessid"/>
        </el-form-item>
        <el-form-item v-if="type === 'info' || false || read.operatorid" label="操作人ID" prop="operatorid">
            <el-input-number v-model="form.operatorid" placeholder="请输入操作人ID" controls-position="right" :readonly="type === 'info' || read.operatorid"/>
        </el-form-item>
        <el-form-item v-if="type === 'info' || false || read.operatorname" label="操作人名称" prop="operatorname">
            <el-input
                v-model="form.operatorname"
                placeholder="请输入操作人名称"
                :readonly="type === 'info' || read.operatorname"
                clearable
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || false || read.requesturl" label="请求路径" prop="requesturl">
            <el-input
                v-model="form.requesturl"
                placeholder="请输入请求路径"
                :readonly="type === 'info' || read.requesturl"
                clearable
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || false || read.requestmethod" label="请求方法" prop="requestmethod">
            <el-input
                v-model="form.requestmethod"
                placeholder="请输入请求方法"
                :readonly="type === 'info' || read.requestmethod"
                clearable
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || false || read.requestip" label="请求IP" prop="requestip">
            <el-input
                v-model="form.requestip"
                placeholder="请输入请求IP"
                :readonly="type === 'info' || read.requestip"
                clearable
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || false || read.detail" label="操作明细" prop="detail">
            <el-input
                v-model="form.detail"
                type="textarea"
                placeholder="请输入操作明细"
                :readonly="type === 'info' || read.detail"
                clearable
            />
        </el-form-item>
...
<script setup lang="ts">
  ...
  const rules: Record<string, any[]> = {
    businessid: [ { validator: validateNumber , trigger: 'blur' } ],
    operatorid: [ { validator: validateNumber , trigger: 'blur' } ],
  }
  ...
  const init = async (itemId?: number, itemType?: string) => {
    ...
    if (type.value === 'cross') {
      ...
      for (const key in crossObj) {
        ...
        if (key === 'operatetype') {
            if (!amountFieldName || key !== amountFieldName) {
              form.operatetype=crossObj[key]
              read.operatetype=true
            }
            continue
        }
        if (key === 'businessid') {
            if (!amountFieldName || key !== amountFieldName) {
              form.businessid=crossObj[key]
              read.businessid=true
            }
            continue
        }
        if (key === 'operatorid') {
            if (!amountFieldName || key !== amountFieldName) {
              form.operatorid=crossObj[key]
              read.operatorid=true
            }
            continue
        }
        if (key === 'operatorname') {
            if (!amountFieldName || key !== amountFieldName) {
              form.operatorname=crossObj[key]
              read.operatorname=true
            }
            continue
        }
        if (key === 'requesturl') {
            if (!amountFieldName || key !== amountFieldName) {
              form.requesturl=crossObj[key]
              read.requesturl=true
            }
            continue
        }
        if (key === 'requestmethod') {
            if (!amountFieldName || key !== amountFieldName) {
              form.requestmethod=crossObj[key]
              read.requestmethod=true
            }
            continue
        }
        if (key === 'requestip') {
            if (!amountFieldName || key !== amountFieldName) {
              form.requestip=crossObj[key]
              read.requestip=true
            }
            continue
        }
  ...
  const getInfo = async (itemId: number) => {
    try {
      const { data } = await http.get(`log/info/${itemId}`)
      if (data){
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = id.value ? 'log/update' : 'log/save'
        ...
        if (crossObj) {
          ...
          if (statusColumnName && !statusColumnName.startsWith('[')) {
            ...
            if (crossrefid && crossuserid) {
              ...
              const { data } = await http.get('log/page', { params })
