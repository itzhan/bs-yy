...
import type { DiscussnewsItem } from '@/types/common'
import { useRoute } from 'vue-router'

const route = useRoute()

// 响应式数据
const dataList = ref<DiscussnewsItem[]>([])
...
const currentReplyComment = ref<DiscussnewsItem>({} as DiscussnewsItem)
// 回复表单
...
const deleteHandler = async (ids?: number[]) => {
  ...
  try {
    ...
    await http.post('discussnews/delete', deleteIds)
    await getDataList()
  } catch {
    // 用户取消操作
  }
...
// 回复处理
const replyHandler = (comment: DiscussnewsItem) => {
...
const submitReply = async () => {
  ...
  try {
    ...
    await http.post('discussnews/update', updateData)
