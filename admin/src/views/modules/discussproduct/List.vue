...
import type { DiscussproductItem } from '@/types/common'
import { useRoute } from 'vue-router'

const route = useRoute()

// 响应式数据
const dataList = ref<DiscussproductItem[]>([])
...
const currentReplyComment = ref<DiscussproductItem>({} as DiscussproductItem)
// 回复表单
...
const deleteHandler = async (ids?: number[]) => {
  ...
  try {
    ...
    await http.post('discussproduct/delete', deleteIds)
    await getDataList()
  } catch {
    // 用户取消操作
  }
...
// 回复处理
const replyHandler = (comment: DiscussproductItem) => {
...
const submitReply = async () => {
  ...
  try {
    ...
    await http.post('discussproduct/update', updateData)
