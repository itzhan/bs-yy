<template>
  <div class="add-edit-dialog">
    <el-dialog v-model="visible" :title="id ? (type === 'info' ? '查看教练评价' : '修改教练评价') : '新增教练评价'" width="60%" :close-on-click-modal="false" @close="handleClose">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="120px">
        ...
        <el-form-item label="评分" prop="rating">
          <el-rate v-model="form.rating" :disabled="type === 'info'" />
        </el-form-item>
        <el-form-item label="评价内容" prop="content">
          <el-input v-model="form.content" type="textarea" :rows="3" :readonly="type === 'info'" />
        </el-form-item>
        <el-form-item label="回复" prop="reply">
          <el-input v-model="form.reply" type="textarea" :rows="3" :readonly="type === 'info'" />
        </el-form-item>
      </el-form>
      <template #footer>
...
const init = async (newId?: number, newType?: string) => {
  ...
  await fetchOptions()
  if (id.value) {
    const res: any = await http.get(`coachreview/info/${id.value}`)
...
const handleSave = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    loading.value = true
    try {
      const api = id.value ? 'coachreview/update' : 'coachreview/save'
