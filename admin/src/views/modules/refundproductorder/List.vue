  ...
  <div class="main-content bg-gray-50 p-6">
    ...
    <div class="bg-white p-4 rounded-lg shadow mb-4">
      <div class="flex justify-between items-center">
        <div class="space-x-2">
          <el-button
            v-if="!false && isAuth('refundproductorder', '新增')"
            type="success"
          ...
          <el-button
            v-if="!false && isAuth('refundproductorder', '删除')"
            type="danger"
          ...
          <download-excel
            v-if="isAuth('refundproductorder', '导出')"
    ...
    <div v-if="!mapConfig.enabled || viewMode === 'table'" class="bg-white rounded-lg shadow">
      <el-table
        v-if="isAuth('refundproductorder', '查看')"
        class="w-full"
        :data="dataList"
      ...
      >
        ...
        <el-table-column label="操作" width="240" fixed="right">
          <template #default="scope">
            ...
            <el-button
                v-if="isAuth('refundproductorder', '修改')"
                type="warning"
                size="small"
            ...
            <el-button
                v-if="isAuth('refundproductorder', '删除')"
                type="danger"
                size="small"
            ...
            <el-button
                v-if="isAuth('refundproductorder', '私聊')"
                type="success"
                size="small"
            ...
            <el-button
                v-if="isAuth('refundproductorder', '审核') && scope.row.auditstatus === '待审核'"
                type="success"
                size="small"
...
<script setup lang="ts">
  ...
  import type { RefundproductorderItem } from '@/types/common'
  ...
  const chatBackendTargetTable = 'refundproductorder'
  let websock: WebSocket | null = null

  // 响应式数据
  const dataList = ref<RefundproductorderItem[]>([])
  const dataListLoading = ref(false)
  const dataListSelections = ref<RefundproductorderItem[]>([])
  ...
  const musicTracks = computed(() => {
    ...
    return dataList.value
      .map(row => {
        ...
        return {
          id: record?.id ?? url,
          name: nameRaw || String(record?.name || '商品订单退款'),
          artist: artistRaw,
  ...
  const playMusic = (row: RefundproductorderItem) => {
  ...
  const getDataList = () => {
    ...
    http.get('refundproductorder/page', { params }).then((response: any) => {
  ...
  // 多选
  const selectionChangeHandler = (val: RefundproductorderItem[]) => {
  ...
  const deleteHandler = async (ids?: number[]) => {
    ...
    try {
      ...
      await http.post('refundproductorder/delete', deleteIds)
  ...
  const auditHandler = async (id: number, auditstatus: string) => {
    ...
    try {
      ...
      await http.post('refundproductorder/audit', { id, auditstatus })
  ...
  const inlineCrossActionHandler = async (
    ...
    try {
      ...
      if (statusColumnName && statusColumnValue) {
        ...
        await http.post(`refundproductorder/update`, curr)
