<template>
  <div class="add-edit-dialog">
    <el-dialog
        v-model="visible"
        :title="id ? (type === 'info' ? '查看商品订单退款' : '修改商品订单退款') : '新增商品订单退款'"
        width="60%"
...
<script setup lang="ts">
  ...
  const getInfo = async (itemId: number) => {
    try {
      const { data } = await http.get(`refundproductorder/info/${itemId}`)
      if (data){
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = id.value ? 'refundproductorder/update' : 'refundproductorder/save'
        ...
        if (crossObj) {
          ...
          if (statusColumnName && !statusColumnName.startsWith('[')) {
            ...
            if (crossrefid && crossuserid) {
              ...
              const { data } = await http.get('refundproductorder/page', { params })
