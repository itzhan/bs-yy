<template>
  <div class="add-edit-dialog">
    <el-dialog
        v-model="visible"
        :title="id ? (type === 'info' ? '查看办卡记录' : '修改办卡记录') : '新增办卡记录'"
        width="60%"
    ...
    >
      ...
      >
        ...
        <el-form-item v-if="type === 'info' || false || read.quantity" label="数量" prop="quantity">
...
<script setup lang="ts">
  ...
  const rules: Record<string, any[]> = {
    ...
    quantity: [ { validator: validateNumber , trigger: 'blur' } ],
  }
  ...
  const getInfo = async (itemId: number) => {
    try {
      const { data } = await http.get(`cardapplication/info/${itemId}`)
      if (data){
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = id.value ? 'cardapplication/update' : 'cardapplication/save'
        ...
        if (crossObj) {
          ...
          if (statusColumnName && !statusColumnName.startsWith('[')) {
            ...
            if (crossrefid && crossuserid) {
              ...
              const { data } = await http.get('cardapplication/page', { params })
