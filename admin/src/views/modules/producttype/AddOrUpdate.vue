<template>
  <div class="add-edit-dialog">
    <el-dialog
        v-model="visible"
        :title="id ? (type === 'info' ? '查看商品分类' : '修改商品分类') : '新增商品分类'"
        width="60%"
    ...
    >
      <el-form
        ...
        class="space-y-4"
      >
        <el-form-item v-if="type === 'info' || true || read.producttypename" label="分类名称" prop="producttypename">
            <el-input
                v-model="form.producttypename"
                placeholder="请输入分类名称"
                :readonly="type === 'info' || read.producttypename"
                clearable
            />
        </el-form-item>
...
<script setup lang="ts">
  ...
  const rules: Record<string, any[]> = {
    producttypename: [ { trigger: 'blur' , message: '分类名称不能为空' , required: true } ],
  }
      ...
      for (const key in crossObj) {
        if (key === 'producttypename') {
            if (!amountFieldName || key !== amountFieldName) {
              form.producttypename=crossObj[key]
              read.producttypename=true
            }
            continue
        }
      }
  ...
  const getInfo = async (itemId: number) => {
    try {
      const { data } = await http.get(`producttype/info/${itemId}`)
      if (data){
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = id.value ? 'producttype/update' : 'producttype/save'
        ...
        if (crossObj) {
          ...
          if (statusColumnName && !statusColumnName.startsWith('[')) {
            ...
            if (crossrefid && crossuserid) {
              ...
              const { data } = await http.get('producttype/page', { params })
