  ...
  <div class="main-content bg-gray-50 p-6">
    ...
    <div class="bg-white p-4 rounded-lg shadow mb-4">
      <div class="flex justify-between items-center">
        <div class="space-x-2">
          <el-button
            v-if="!false && isAuth('producttype', '新增')"
            type="success"
          ...
          <el-button
            v-if="!false && isAuth('producttype', '删除')"
            type="danger"
    ...
    <div v-if="!mapConfig.enabled || viewMode === 'table'" class="bg-white rounded-lg shadow">
      ...
      >
        ...
        <el-table-column prop="producttypename" label="分类名称" />
        <!-- 操作列 -->
...
<script setup lang="ts">
  ...
  import type { ProducttypeItem } from '@/types/common'
  ...
  // 响应式数据
  const dataList = ref<ProducttypeItem[]>([])
  const dataListLoading = ref(false)
  const dataListSelections = ref<ProducttypeItem[]>([])
  ...
  const musicTracks = computed(() => {
    ...
    return dataList.value
      .map(row => {
        ...
        return {
          id: record?.id ?? url,
          name: nameRaw || String(record?.name || '商品分类'),
          artist: artistRaw,
  ...
  const playMusic = (row: ProducttypeItem) => {
  ...
  const getDataList = () => {
    ...
    if (searchForm.producttypename) {
      params.producttypename = '%' + searchForm.producttypename + '%'
    }
    http.get('producttype/page', { params }).then((response: any) => {
  ...
  // 重置
  const reset = () => {
    searchForm.producttypename = undefined
    pageIndex.value = 1
    getDataList()
  }
  ...
  // 多选
  const selectionChangeHandler = (val: ProducttypeItem[]) => {
  ...
  const deleteHandler = async (ids?: number[]) => {
    ...
    try {
      ...
      await http.post('producttype/delete', deleteIds)
  ...
  const inlineCrossActionHandler = async (
    ...
    try {
      ...
      if (statusColumnName && statusColumnValue) {
        ...
        await http.post(`producttype/update`, curr)
