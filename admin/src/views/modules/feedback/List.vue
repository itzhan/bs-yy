    ...
    <div class="bg-white p-4 rounded-lg shadow mb-4">
      <el-form :inline="true" :model="searchForm" class="search-form">
        <div class="inline-block mr-4">
            ...
            <el-input
            v-model="searchForm.feedbacktitle"
            placeholder="标题"
            clearable
        ...
        <div class="inline-block mr-4">
            <label class="inline-block mr-2 leading-10">反馈类型</label>
            <div>
                <el-select v-model="searchForm.feedbacktype" placeholder="请选择反馈类型" clearable class="w-40">
...
<script setup lang="ts">
  ...
  const chatBackendTargetTable = 'feedback'
  let websock: WebSocket | null = null

  // 响应式数据
  const dataList = ref<FeedbackItem[]>([])
  const dataListLoading = ref(false)
  const dataListSelections = ref<FeedbackItem[]>([])
  ...
  const musicTracks = computed(() => {
    ...
    return dataList.value
      .map(row => {
        ...
        return {
          id: record?.id ?? url,
          name: nameRaw || String(record?.name || '意见反馈'),
          artist: artistRaw,
  ...
  const playMusic = (row: FeedbackItem) => {
  ...
  const getDataList = () => {
    ...
    if (searchForm.feedbacktitle) {
      params.feedbacktitle = '%' + searchForm.feedbacktitle + '%'
    }
    ...
    http.get('feedback/page', { params }).then((response: any) => {
  ...
  // 重置
  const reset = () => {
    searchForm.feedbacktitle = undefined
  ...
  // 多选
  const selectionChangeHandler = (val: FeedbackItem[]) => {
  ...
  const deleteHandler = async (ids?: number[]) => {
    ...
    try {
      ...
      await http.post('feedback/delete', deleteIds)
  ...
  const inlineCrossActionHandler = async (
    ...
    try {
      ...
      if (statusColumnName && statusColumnValue) {
        ...
        await http.post(`feedback/update`, curr)
  ...
  const auditBatchHandler = async () => {
    ...
    try {
      ...
      if (!valid) return
      await http.post(
              `feedback/audit/batch?auditstatus=${auditBatchForm.auditstatus}&auditreply=${auditBatchForm.auditreply}`,
              batchIds.value
      )
      getDataList()
