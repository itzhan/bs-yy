<template>
  <div class="add-edit-dialog">
    <el-dialog
        v-model="visible"
        :title="id ? (type === 'info' ? '查看意见反馈' : '修改意见反馈') : '新增意见反馈'"
        width="60%"
    ...
    >
      <el-form
        ...
        class="space-y-4"
      >
        <el-form-item v-if="type === 'info' || true || read.feedbacktitle" label="标题" prop="feedbacktitle">
            <el-input
                v-model="form.feedbacktitle"
                placeholder="请输入标题"
                :readonly="type === 'info' || read.feedbacktitle"
                clearable
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.feedbackcontent" label="内容" prop="feedbackcontent">
            <el-input
                v-model="form.feedbackcontent"
                type="textarea"
                placeholder="请输入内容"
                :readonly="type === 'info' || read.feedbackcontent"
                clearable
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.feedbacktype" label="反馈类型" prop="feedbacktype">
            <el-select v-model="form.feedbacktype" placeholder="请选择反馈类型" :readonly="type === 'info' || read.feedbacktype">
...
<script setup lang="ts">
  ...
  const rules: Record<string, any[]> = {
    feedbacktitle: [ { trigger: 'blur' , message: '标题不能为空' , required: true } ],
    feedbackcontent: [ { trigger: 'blur' , message: '内容不能为空' , required: true } ],
    feedbacktype: [ { trigger: 'blur' , message: '反馈类型不能为空' , required: true } ],
  }
  ...
  const init = async (itemId?: number, itemType?: string) => {
    ...
    if (type.value === 'cross') {
      ...
      for (const key in crossObj) {
        if (key === 'feedbacktitle') {
            if (!amountFieldName || key !== amountFieldName) {
              form.feedbacktitle=crossObj[key]
              read.feedbacktitle=true
            }
            continue
        }
        if (key === 'feedbackcontent') {
            if (!amountFieldName || key !== amountFieldName) {
              form.feedbackcontent=crossObj[key]
              read.feedbackcontent=true
            }
            continue
        }
        if (key === 'feedbacktype') {
            if (!amountFieldName || key !== amountFieldName) {
              form.feedbacktype=crossObj[key]
              read.feedbacktype=true
            }
            continue
        }
      }
  ...
  const getInfo = async (itemId: number) => {
    try {
      const { data } = await http.get(`feedback/info/${itemId}`)
      if (data){
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = id.value ? 'feedback/update' : 'feedback/save'
        ...
        if (crossObj) {
          ...
          if (statusColumnName && !statusColumnName.startsWith('[')) {
            ...
            if (crossrefid && crossuserid) {
              ...
              const { data } = await http.get('feedback/page', { params })
