  ...
  <div class="main-content bg-gray-50 p-6">
    ...
    <div class="bg-white p-4 rounded-lg shadow mb-4">
      <div class="flex justify-between items-center">
        <div class="space-x-2">
          <el-button
            v-if="!false && isAuth('cardrenewal', '新增')"
            type="success"
          ...
          <el-button
            v-if="!false && isAuth('cardrenewal', '删除')"
            type="danger"
    ...
    <div v-if="!mapConfig.enabled || viewMode === 'table'" class="bg-white rounded-lg shadow">
      ...
      >
        ...
        <el-table-column prop="ispay" label="是否支付" width="160">
          <template #default="scope">
            <div class="flex items-center">
              ...
              <el-button
                v-if="scope.row.ispay !== '已支付' && isAuth('cardrenewal', '支付')"
                type="text"
        ...
        <el-button
                v-if="isAuth('cardrenewal', '取消续卡记录')"
                type="success"
                size="small"
                @click="cancelcardrenewalCrossAddOrUpdateHandler(scope.row, 'cross', '续卡记录已取消', 'orderstatus', '已取消', '取消续卡记录')"
            >
            取消续卡记录
            </el-button>
        <el-button
                v-if="isAuth('cardrenewal', '退款')"
                type="success"
                size="small"
                @click="refundcardrenewalCrossAddOrUpdateHandler(scope.row, 'cross', '退款已处理', 'orderstatus', '已退款', '退款')"
            >
            退款
            </el-button>
        <el-button
                v-if="isAuth('cardrenewal', '发货')"
                type="success"
                size="small"
    ...
    <!-- cancelcardrenewal 跨表组件 -->
    <cancelcardrenewal-cross-add-or-update
    v-if="cancelcardrenewalCrossAddOrUpdateFlag"
    ref="cancelcardrenewalCrossAddOrUpdateRef"
    @refreshDataList="getDataList"
    ></cancelcardrenewal-cross-add-or-update>
    <!-- refundcardrenewal 跨表组件 -->
    <refundcardrenewal-cross-add-or-update
    v-if="refundcardrenewalCrossAddOrUpdateFlag"
    ref="refundcardrenewalCrossAddOrUpdateRef"
    @refreshDataList="getDataList"
    ></refundcardrenewal-cross-add-or-update>
...
<script setup lang="ts">
  ...
  import type { CardrenewalItem } from '@/types/common'
  ...
  import cancelcardrenewalCrossAddOrUpdate from '../cancelcardrenewal/AddOrUpdate.vue'
  import refundcardrenewalCrossAddOrUpdate from '../refundcardrenewal/AddOrUpdate.vue'
  ...
  const chatBackendTargetTable = 'cardrenewal'
  let websock: WebSocket | null = null

  // 响应式数据
  const dataList = ref<CardrenewalItem[]>([])
  const dataListLoading = ref(false)
  const dataListSelections = ref<CardrenewalItem[]>([])
  ...
  const musicTracks = computed(() => {
    ...
    return dataList.value
      .map(row => {
        ...
        return {
          id: record?.id ?? url,
          name: nameRaw || String(record?.name || '续卡记录'),
          artist: artistRaw,
  ...
  const playMusic = (row: CardrenewalItem) => {
  ...
  // 跨表组件显示标志（去重）
  const cancelcardrenewalCrossAddOrUpdateFlag = ref(false)
  const cancelcardrenewalCrossAddOrUpdateRef = ref()
  const refundcardrenewalCrossAddOrUpdateFlag = ref(false)
  const refundcardrenewalCrossAddOrUpdateRef = ref()
  ...
  // 多选
  const selectionChangeHandler = (val: CardrenewalItem[]) => {
  ...
  const deleteHandler = async (ids?: number[]) => {
    ...
    try {
      ...
      await http.post('cardrenewal/delete', deleteIds)
  ...
  const payHandler = (row: any) => {
    localStorage.setItem('paytable', 'cardrenewal')
  ...
  // 取消续卡记录
  const cancelcardrenewalCrossAddOrUpdateHandler = (row: any, type: string, tips: string, statusColumnName: string, statusColumnValue: string, buttonName: string) => {
    // 显示取消续卡记录弹窗
    cancelcardrenewalCrossAddOrUpdateFlag.value = true

    localStorage.setItem('crossObj', JSON.stringify(row))
    localStorage.setItem('crossTable', 'cardrenewal')
    ...
    if (statusColumnName && !statusColumnName.startsWith('[')) {
      ...
      if (crossObj[statusColumnName] === statusColumnValue) {
        ...
        cancelcardrenewalCrossAddOrUpdateFlag.value = false
        return
      }
    }
    // 调用跨表组件
    nextTick(() => {
      cancelcardrenewalCrossAddOrUpdateRef.value?.init(undefined, type)
    })
  }
  
  // 退款
  const refundcardrenewalCrossAddOrUpdateHandler = (row: any, type: string, tips: string, statusColumnName: string, statusColumnValue: string, buttonName: string) => {
    // 显示退款弹窗
    refundcardrenewalCrossAddOrUpdateFlag.value = true
    ...
    if (statusColumnName && !statusColumnName.startsWith('[')) {
      ...
      if (crossObj[statusColumnName] === statusColumnValue) {
        ...
        refundcardrenewalCrossAddOrUpdateFlag.value = false
        return
      }
    }
    // 调用跨表组件
    nextTick(() => {
      refundcardrenewalCrossAddOrUpdateRef.value?.init(undefined, type)
    })
  }
  ...
  const inlineCrossActionHandler = async (
    ...
    try {
      ...
      if (statusColumnName && statusColumnValue) {
        ...
        await http.post(`cardrenewal/update`, curr)
