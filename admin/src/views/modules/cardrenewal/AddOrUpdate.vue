<template>
  <div class="add-edit-dialog">
    <el-dialog
        v-model="visible"
        :title="id ? (type === 'info' ? '查看续卡记录' : '修改续卡记录') : '新增续卡记录'"
        width="60%"
...
<script setup lang="ts">
  ...
  const getInfo = async (itemId: number) => {
    try {
      const { data } = await http.get(`cardrenewal/info/${itemId}`)
      if (data){
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = id.value ? 'cardrenewal/update' : 'cardrenewal/save'
        ...
        if (crossObj) {
          ...
          if (statusColumnName && !statusColumnName.startsWith('[')) {
            ...
            if (crossrefid && crossuserid) {
              ...
              const { data } = await http.get('cardrenewal/page', { params })
