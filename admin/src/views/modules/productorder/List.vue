  ...
  <div class="main-content bg-gray-50 p-6">
    ...
    <div class="bg-white p-4 rounded-lg shadow mb-4">
      <el-form :inline="true" :model="searchForm" class="search-form">
        <div class="inline-block mr-4">
            <label class="inline-block mr-2 leading-10">商品名称</label>
            <div>
                <el-select
        ...
        <div class="inline-block mr-4">
            <label class="inline-block mr-2 leading-10">商品分类</label>
            <div>
                <el-select
        ...
        <div class="inline-block mr-4">
            <label class="inline-block mr-2 leading-10">是否支付</label>
            <div>
                <el-select v-model="searchForm.ispay" placeholder="请选择是否支付" clearable class="w-40">
        ...
        <div class="inline-block mr-4">
            <label class="inline-block mr-2 leading-10">状态</label>
            <div>
                <el-select v-model="searchForm.orderstatus" placeholder="请选择状态" clearable class="w-40">
                    <el-option label="未支付" value="未支付"/>
                    <el-option label="已支付" value="已支付"/>
    ...
    <div class="bg-white p-4 rounded-lg shadow mb-4">
      <div class="flex justify-between items-center">
        <div class="space-x-2">
          <el-button
            v-if="!false && isAuth('productorder', '新增')"
            type="success"
          ...
          <el-button
            v-if="!false && isAuth('productorder', '删除')"
            type="danger"
    ...
    <div v-if="!mapConfig.enabled || viewMode === 'table'" class="bg-white rounded-lg shadow">
      ...
      >
        ...
        <template #default="scope">
            ...
            <img
                v-if="scope.row.productimage.substring(0,4)=='http'"
                :src="scope.row.productimage.split(',')[0]"
                width="100"
                height="100"
                style="object-fit: cover"
                @click="imgPreView(scope.row.productimage.split(',')[0])"
                class="cursor-pointer"
            >
            <img
                v-else
                :src="baseUrl + scope.row.productimage.split(',')[0]"
                width="100"
                height="100"
                style="object-fit: cover"
                @click="imgPreView(baseUrl + scope.row.productimage.split(',')[0])"
                class="cursor-pointer"
            >
            </div>
        ...
        <el-table-column prop="ispay" label="是否支付" width="160">
          <template #default="scope">
            <div class="flex items-center">
              <span class="mr-2">{{ scope.row.ispay === '已支付' ? '已支付' : '未支付' }}</span>
              <el-button
                v-if="scope.row.ispay !== '已支付' && isAuth('productorder', '支付')"
                type="text"
        ...
        <el-button
                v-if="isAuth('productorder', '取消商品订单')"
                type="success"
                size="small"
                @click="cancelproductorderCrossAddOrUpdateHandler(scope.row, 'cross', '商品订单已取消', 'orderstatus', '已取消', '取消商品订单')"
            >
            取消商品订单
            </el-button>
        <el-button
                v-if="isAuth('productorder', '退款')"
                type="success"
                size="small"
                @click="refundproductorderCrossAddOrUpdateHandler(scope.row, 'cross', '退款已处理', 'orderstatus', '已退款', '退款')"
            >
            退款
            </el-button>
        <el-button
                v-if="isAuth('productorder', '发货')"
                type="success"
                size="small"
    ...
    <!-- cancelproductorder 跨表组件 -->
    <cancelproductorder-cross-add-or-update
    v-if="cancelproductorderCrossAddOrUpdateFlag"
    ref="cancelproductorderCrossAddOrUpdateRef"
    @refreshDataList="getDataList"
    ></cancelproductorder-cross-add-or-update>
    <!-- refundproductorder 跨表组件 -->
    <refundproductorder-cross-add-or-update
    v-if="refundproductorderCrossAddOrUpdateFlag"
    ref="refundproductorderCrossAddOrUpdateRef"
    @refreshDataList="getDataList"
    ></refundproductorder-cross-add-or-update>
...
<script setup lang="ts">
  ...
  import type { ProductorderItem } from '@/types/common'
  ...
  import cancelproductorderCrossAddOrUpdate from '../cancelproductorder/AddOrUpdate.vue'
  import refundproductorderCrossAddOrUpdate from '../refundproductorder/AddOrUpdate.vue'
  ...
  const chatBackendTargetTable = 'productorder'
  let websock: WebSocket | null = null

  // 响应式数据
  const dataList = ref<ProductorderItem[]>([])
  const dataListLoading = ref(false)
  const dataListSelections = ref<ProductorderItem[]>([])
  ...
  const musicTracks = computed(() => {
    ...
    return dataList.value
      .map(row => {
        ...
        return {
          id: record?.id ?? url,
          name: nameRaw || String(record?.name || '商品订单'),
          artist: artistRaw,
  ...
  const playMusic = (row: ProductorderItem) => {
  ...
  const exportRows = computed(() =>
    dataList.value.map((row) => {
      ...
      if (plain.productimage !== undefined && plain.productimage !== null) {
        const images = String(plain.productimage)
          .split(',')
          .map((item) => item.trim())
          .filter(Boolean)
          .map((item) => resolveFileUrl(item))
        plain.productimage = images.join('; ')
      }
      return plain
    })
  )
  ...
  // 跨表组件显示标志（去重）
  const cancelproductorderCrossAddOrUpdateFlag = ref(false)
  const cancelproductorderCrossAddOrUpdateRef = ref()
  const refundproductorderCrossAddOrUpdateFlag = ref(false)
  const refundproductorderCrossAddOrUpdateRef = ref()
  ...
  const getDataList = () => {
    ...
    if (searchForm.producttype) {
      params.producttype = '%' + searchForm.producttype + '%'
    }
    if (searchForm.ispay) {
      params.ispay = '%' + searchForm.ispay + '%'
    }
    if (searchForm.orderstatus) {
      params.orderstatus = '%' + searchForm.orderstatus + '%'
    }
    http.get('productorder/page', { params }).then((response: any) => {
  ...
  // 多选
  const selectionChangeHandler = (val: ProductorderItem[]) => {
  ...
  const deleteHandler = async (ids?: number[]) => {
    ...
    try {
      ...
      await http.post('productorder/delete', deleteIds)
  ...
  // 单条支付
  const payHandler = (row: any) => {
    localStorage.setItem('paytable', 'productorder')
    localStorage.setItem('payObject', JSON.stringify(row))
  ...
  const payBatch = async () => {
    if (!dataListSelections.value.length) {
      ElMessage.error('请选择需要支付的订单')
      return
    }
    // 检查选中的数据是否全部审核通过
    for (const item of dataListSelections.value) {
      if (item.ispay && item.ispay === '已支付') {
        ElMessage.error('存在已支付的订单，无法继续操作！')
        batchIds.value = []
        return false
      }
    ...
    try {
      await ElMessageBox.confirm('是否支付所选记录？', '提示')
    } catch {
      return
    }
    const arr = JSON.parse(JSON.stringify(dataListSelections.value))
    for (const item of arr) {
      item.ispay = '已支付'
      await http.post('${table.name}/update', item)
    }
  ...
  // 取消商品订单
  const cancelproductorderCrossAddOrUpdateHandler = (row: any, type: string, tips: string, statusColumnName: string, statusColumnValue: string, buttonName: string) => {
    // 显示取消商品订单弹窗
    cancelproductorderCrossAddOrUpdateFlag.value = true

    localStorage.setItem('crossObj', JSON.stringify(row))
    localStorage.setItem('crossTable', 'productorder')
    localStorage.setItem('statusColumnName', statusColumnName)
    localStorage.setItem('statusColumnValue', statusColumnValue)
    localStorage.setItem('tips', tips)
    try {
      localStorage.setItem('stockUpdate.targetTable', 'product')
      localStorage.setItem('stockUpdate.mode', 'plus')
      localStorage.setItem('stockUpdate.field', 'stock')
    ...
    if (statusColumnName && !statusColumnName.startsWith('[')) {
      ...
      if (crossObj[statusColumnName] === statusColumnValue) {
        ...
        cancelproductorderCrossAddOrUpdateFlag.value = false
        return
      }
    }
    // 调用跨表组件
    nextTick(() => {
      cancelproductorderCrossAddOrUpdateRef.value?.init(undefined, type)
    })
  }
  
  // 退款
  const refundproductorderCrossAddOrUpdateHandler = (row: any, type: string, tips: string, statusColumnName: string, statusColumnValue: string, buttonName: string) => {
    // 显示退款弹窗
    refundproductorderCrossAddOrUpdateFlag.value = true
    ...
    if (statusColumnName && !statusColumnName.startsWith('[')) {
      ...
      if (crossObj[statusColumnName] === statusColumnValue) {
        ...
        refundproductorderCrossAddOrUpdateFlag.value = false
        return
      }
    }
    // 调用跨表组件
    nextTick(() => {
      refundproductorderCrossAddOrUpdateRef.value?.init(undefined, type)
    })
  }
  ...
  const discussListHandler = (id: number) => {
    router.push({
      path: '/dashboard/modules/discussproductorder',
      query: { refid: id }
    })
  }
  ...
  const inlineCrossActionHandler = async (
    ...
    try {
      ...
      if (statusColumnName && statusColumnValue) {
        ...
        await http.post(`productorder/update`, curr)
