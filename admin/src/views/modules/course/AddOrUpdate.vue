<template>
  <div class="add-edit-dialog">
    <el-dialog
        v-model="visible"
        :title="id ? (type === 'info' ? '查看健身课程' : '修改健身课程') : '新增健身课程'"
        width="60%"
    ...
    >
      <el-form
        ...
        class="space-y-4"
      >
        <el-form-item v-if="type === 'info' || true || read.coursename" label="课程名称" prop="coursename">
            <el-input
                v-model="form.coursename"
                placeholder="请输入课程名称"
                :readonly="type === 'info' || read.coursename"
                clearable
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.courseimage" label="课程封面" prop="courseimage">
            <UploadField
                v-model="form.courseimage"
                upload-type="image"
                :readonly="type === 'info' || read.courseimage"
            />
        </el-form-item>
        ...
        <el-form-item v-if="type === 'info' || true || read.duration" label="课程时长" prop="duration">
            <el-input-number v-model="form.duration" placeholder="请输入课程时长" controls-position="right" :readonly="type === 'info' || read.duration"/>
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.coachname" label="教练" prop="coachname">
            <el-select v-model="form.coachname" placeholder="请选择教练" :readonly="type === 'info' || read.coachname">
                <el-option
                    v-for="(item, index) in coachnameOptions"
                    :key="index"
                    :label="item"
                    :value="item"
                />
        ...
        <el-form-item v-if="type === 'info' || false || read.coachaccount" label="教练账号" prop="coachaccount">
            <el-input
        ...
        <el-form-item v-if="type === 'info' || true || read.coursedesc" label="课程简介" prop="coursedesc">
            <el-input
                v-model="form.coursedesc"
                type="textarea"
                placeholder="请输入课程简介"
                :readonly="type === 'info' || read.coursedesc"
                clearable
            />
        </el-form-item>
...
<script setup lang="ts">
  ...
  // 教练选项
  const coachnameOptions = ref<string[]>([])
  ...
  const rules: Record<string, any[]> = {
    ...
    coursetype: [ { trigger: 'blur' , message: '课程分类不能为空' , required: true } ],
      ...
      for (const key in crossObj) {
        if (key === 'coursename') {
            if (!amountFieldName || key !== amountFieldName) {
              form.coursename=crossObj[key]
              read.coursename=true
            }
            continue
        }
        if (key === 'courseimage') {
            if (!amountFieldName || key !== amountFieldName) {
              form.courseimage=crossObj[key]
              read.courseimage=true
            }
            continue
        }
        if (key === 'coursetype') {
            if (!amountFieldName || key !== amountFieldName) {
              form.coursetype=crossObj[key]
              read.coursetype=true
            }
            continue
        }
        if (key === 'classtime') {
            if (!amountFieldName || key !== amountFieldName) {
              form.classtime=crossObj[key]
              read.classtime=true
            }
            continue
        }
        ...
        if (key === 'coursedesc') {
            if (!amountFieldName || key !== amountFieldName) {
              form.coursedesc=crossObj[key]
              read.coursedesc=true
            }
            continue
        }
      }
  ...
  const getInfo = async (itemId: number) => {
    try {
      const { data } = await http.get(`course/info/${itemId}`)
      if (data){
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = id.value ? 'course/update' : 'course/save'
  ...
  // 获取教练选项（支持联动条件）
  const getCoachnameOptions = async () => {
    try {
      const params: any = {}
      const response = await http.get('option/coach/coachname', { params })
      if (response && response.code === 0) {
        coachnameOptions.value = response.data || []
      } else {
        coachnameOptions.value = []
      }
    } catch (error) {
      console.error('获取教练选项失败:', error)
      coachnameOptions.value = []
    }
  }
        ...
        if (crossObj) {
          ...
          if (statusColumnName && !statusColumnName.startsWith('[')) {
            ...
            if (crossrefid && crossuserid) {
              ...
              const { data } = await http.get('course/page', { params })
