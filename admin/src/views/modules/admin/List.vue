  ...
  <div class="main-content bg-gray-50 p-6">
    ...
    <div class="bg-white p-4 rounded-lg shadow mb-4">
      <el-form :inline="true" :model="searchForm" class="search-form">
        ...
        <div class="inline-block mr-4">
            <label class="inline-block mr-2 leading-10">角色</label>
            <el-input
...
<script setup lang="ts">
  ...
  import type { AdminItem } from '@/types/common'
  ...
  const chatBackendUserLabelField = 'adminaccount'
  const chatBackendTargetTable = 'admin'
  let websock: WebSocket | null = null

  // 响应式数据
  const dataList = ref<AdminItem[]>([])
  const dataListLoading = ref(false)
  const dataListSelections = ref<AdminItem[]>([])
  ...
  const musicTracks = computed(() => {
    ...
    return dataList.value
      .map(row => {
        ...
        return {
          id: record?.id ?? url,
          name: nameRaw || String(record?.name || '管理员'),
          artist: artistRaw,
  ...
  const getDataList = () => {
    ...
    if (searchForm.adminaccount) {
      params.adminaccount = '%' + searchForm.adminaccount + '%'
    }
    if (searchForm.role) {
      params.role = '%' + searchForm.role + '%'
    }
    http.get('admin/page', { params }).then((response: any) => {
  ...
  // 重置
  const reset = () => {
    searchForm.adminaccount = undefined
  ...
  // 多选
  const selectionChangeHandler = (val: AdminItem[]) => {
  ...
  const deleteHandler = async (ids?: number[]) => {
    ...
    try {
      ...
      await http.post('admin/delete', deleteIds)
  ...
  const inlineCrossActionHandler = async (
    ...
    try {
      ...
      if (statusColumnName && statusColumnValue) {
        ...
        await http.post(`admin/update`, curr)
