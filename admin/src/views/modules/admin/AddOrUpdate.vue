<template>
  <div class="add-edit-dialog">
    <el-dialog
        v-model="visible"
        :title="id ? (type === 'info' ? '查看管理员' : '修改管理员') : '新增管理员'"
        width="60%"
    ...
    >
      <el-form
        ...
        class="space-y-4"
      >
        <el-form-item v-if="type === 'info' || true || read.adminaccount" label="账号" prop="adminaccount">
            <el-input
                v-model="form.adminaccount"
                placeholder="请输入账号"
                :readonly="type === 'info' || read.adminaccount"
                clearable
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.adminpassword" label="密码" prop="adminpassword">
            <el-input
                v-model="form.adminpassword"
                placeholder="请输入密码"
                :readonly="type === 'info' || read.adminpassword"
                clearable
            />
        </el-form-item>
        ...
        <el-form-item v-if="type === 'info' || true || read.role" label="角色" prop="role">
            <el-input
                v-model="form.role"
                placeholder="请输入角色"
...
<script setup lang="ts">
      ...
      for (const key in crossObj) {
        if (key === 'adminaccount') {
            if (!amountFieldName || key !== amountFieldName) {
              form.adminaccount=crossObj[key]
              read.adminaccount=true
            }
            continue
        }
        if (key === 'adminpassword') {
            if (!amountFieldName || key !== amountFieldName) {
              form.adminpassword=crossObj[key]
              read.adminpassword=true
            }
            continue
        }
        if (key === 'image') {
  ...
  const getInfo = async (itemId: number) => {
    try {
      const { data } = await http.get(`admin/info/${itemId}`)
      if (data){
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = id.value ? 'admin/update' : 'admin/save'
        ...
        if (crossObj) {
          ...
          if (statusColumnName && !statusColumnName.startsWith('[')) {
            ...
            if (crossrefid && crossuserid) {
              ...
              const { data } = await http.get('admin/page', { params })
