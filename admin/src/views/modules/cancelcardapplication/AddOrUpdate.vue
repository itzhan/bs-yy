<template>
  <div class="add-edit-dialog">
    <el-dialog
        v-model="visible"
        :title="id ? (type === 'info' ? '查看取消办卡记录' : '修改取消办卡记录') : '新增取消办卡记录'"
        width="60%"
    ...
    >
      <el-form
        ...
        class="space-y-4"
      >
        <el-form-item v-if="type === 'info' || true || read.packagename" label="套餐名称" prop="packagename">
            <el-select v-model="form.packagename" placeholder="请选择套餐名称" :readonly="type === 'info' || read.packagename">
                <el-option
        ...
        <el-form-item v-if="type === 'info' || true || read.packageimage" label="套餐图片" prop="packageimage">
            <UploadField
                v-model="form.packageimage"
                upload-type="image"
                :readonly="type === 'info' || read.packageimage"
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.packagetype" label="套餐类型" prop="packagetype">
            <el-input
                v-model="form.packagetype"
                placeholder="请输入套餐类型"
                :readonly="type === 'info' || read.packagetype"
                clearable
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.packageprice" label="价格" prop="packageprice">
            <el-input-number v-model="form.packageprice" placeholder="请输入价格" controls-position="right" :readonly="type === 'info' || read.packageprice"/>
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.validdays" label="有效期" prop="validdays">
            <el-input-number v-model="form.validdays" placeholder="请输入有效期" controls-position="right" :readonly="type === 'info' || read.validdays"/>
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.includedcourses" label="包含课时" prop="includedcourses">
            <el-input-number v-model="form.includedcourses" placeholder="请输入包含课时" controls-position="right" :readonly="type === 'info' || read.includedcourses"/>
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.cancelreason" label="取消原因" prop="cancelreason">
            <el-input
                v-model="form.cancelreason"
                type="textarea"
                placeholder="请输入取消原因"
                :readonly="type === 'info' || read.cancelreason"
                clearable
            />
        </el-form-item>
...
<script setup lang="ts">
  ...
  const rules: Record<string, any[]> = {
    packagename: [ { trigger: 'blur' , message: '套餐名称不能为空' , required: true } ],
    packageprice: [ { trigger: 'blur' , message: '价格不能为空' , required: true }, { validator: validateNumber , trigger: 'blur' } ],
    validdays: [ { validator: validateNumber , trigger: 'blur' } ],
    includedcourses: [ { validator: validateNumber , trigger: 'blur' } ],
  }
      ...
      for (const key in crossObj) {
        if (key === 'packagename') {
            if (!amountFieldName || key !== amountFieldName) {
              form.packagename=crossObj[key]
              read.packagename=true
            }
            continue
        }
        if (key === 'packageimage') {
            if (!amountFieldName || key !== amountFieldName) {
              form.packageimage=crossObj[key]
              read.packageimage=true
            }
            continue
        }
        if (key === 'packagetype') {
            if (!amountFieldName || key !== amountFieldName) {
              form.packagetype=crossObj[key]
              read.packagetype=true
            }
            continue
        }
        if (key === 'packageprice') {
            if (!amountFieldName || key !== amountFieldName) {
              form.packageprice=crossObj[key]
              read.packageprice=true
            }
            continue
        }
        if (key === 'validdays') {
            if (!amountFieldName || key !== amountFieldName) {
              form.validdays=crossObj[key]
              read.validdays=true
            }
            continue
        }
        if (key === 'includedcourses') {
            if (!amountFieldName || key !== amountFieldName) {
              form.includedcourses=crossObj[key]
              read.includedcourses=true
            }
            continue
        }
        if (key === 'cancelreason') {
            if (!amountFieldName || key !== amountFieldName) {
              form.cancelreason=crossObj[key]
              read.cancelreason=true
            }
            continue
        }
      }
  ...
  const getInfo = async (itemId: number) => {
    try {
      const { data } = await http.get(`cancelcardapplication/info/${itemId}`)
      if (data){
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = id.value ? 'cancelcardapplication/update' : 'cancelcardapplication/save'
        ...
        if (crossObj) {
          ...
          if (statusColumnName && !statusColumnName.startsWith('[')) {
            ...
            if (crossrefid && crossuserid) {
              ...
              const { data } = await http.get('cancelcardapplication/page', { params })
