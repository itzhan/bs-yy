  ...
  <div class="main-content bg-gray-50 p-6">
    ...
    <div class="bg-white p-4 rounded-lg shadow mb-4">
      <el-form :inline="true" :model="searchForm" class="search-form">
        ...
        <div class="inline-block mr-4">
            <label class="inline-block mr-2 leading-10">姓名</label>
            <el-input
        ...
        <div class="inline-block mr-4">
            <label class="inline-block mr-2 leading-10">最小年龄</label>
            <el-input
        ...
        <div class="inline-block mr-4">
            <label class="inline-block mr-2 leading-10">最大年龄</label>
            <el-input
    ...
    <div v-if="!mapConfig.enabled || viewMode === 'table'" class="bg-white rounded-lg shadow">
      ...
      >
        ...
        <el-table-column prop="remainingcourses" label="剩余课时" />
        <!-- 操作列 -->
...
<script setup lang="ts">
  ...
  import type { UserItem } from '@/types/common'
  ...
  const chatBackendUserLabelField = 'useraccount'
  const chatBackendTargetTable = 'user'
  let websock: WebSocket | null = null

  // 响应式数据
  const dataList = ref<UserItem[]>([])
  const dataListLoading = ref(false)
  const dataListSelections = ref<UserItem[]>([])
  ...
  const musicTracks = computed(() => {
    ...
    return dataList.value
      .map(row => {
        ...
        return {
          id: record?.id ?? url,
          name: nameRaw || String(record?.name || '用户'),
          artist: artistRaw,
  ...
  const getDataList = () => {
    ...
    if (searchForm.age_start) {
      params.age_start = searchForm.age_start
    }
    if (searchForm.age_end) {
      params.age_end = searchForm.age_end
    }
    http.get('user/page', { params }).then((response: any) => {
  ...
  const reset = () => {
    ...
    searchForm.age_start = undefined
    searchForm.age_end = undefined
    pageIndex.value = 1
    getDataList()
  }
  ...
  // 多选
  const selectionChangeHandler = (val: UserItem[]) => {
  ...
  const deleteHandler = async (ids?: number[]) => {
    ...
    try {
      ...
      await http.post('user/delete', deleteIds)
  ...
  const inlineCrossActionHandler = async (
    ...
    try {
      ...
      if (statusColumnName && statusColumnValue) {
        ...
        await http.post(`user/update`, curr)
