<template>
  <div class="add-edit-dialog">
    <el-dialog
        v-model="visible"
        :title="id ? (type === 'info' ? '查看用户' : '修改用户') : '新增用户'"
        width="60%"
    ...
    >
      <el-form
        ...
        class="space-y-4"
      >
        <el-form-item v-if="type === 'info' || true || read.useraccount" label="账号" prop="useraccount">
            <el-input
                v-model="form.useraccount"
                placeholder="请输入账号"
                :readonly="type === 'info' || read.useraccount"
                clearable
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.userpassword" label="密码" prop="userpassword">
            <el-input
                v-model="form.userpassword"
                placeholder="请输入密码"
                :readonly="type === 'info' || read.userpassword"
                clearable
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.name" label="姓名" prop="name">
            <el-input
                v-model="form.name"
                placeholder="请输入姓名"
                :readonly="type === 'info' || read.name"
                clearable
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.sex" label="性别" prop="sex">
            <el-radio-group v-model="form.sex" :readonly="type === 'info' || read.sex">
                <el-radio label="男">男</el-radio>
                <el-radio label="女">女</el-radio>
            </el-radio-group>
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.age" label="年龄" prop="age">
            <el-input-number v-model="form.age" placeholder="请输入年龄" controls-position="right" :readonly="type === 'info' || read.age"/>
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.phone" label="手机号码" prop="phone">
            <el-input
                v-model="form.phone"
                placeholder="请输入手机号码"
                :readonly="type === 'info' || read.phone"
                clearable
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.image" label="头像" prop="image">
            <UploadField
                v-model="form.image"
                upload-type="image"
                :readonly="type === 'info' || read.image"
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || false || read.money" label="余额" prop="money">
            <el-input-number v-model="form.money" placeholder="请输入余额" controls-position="right" :readonly="type === 'info' || read.money"/>
        </el-form-item>
        <el-form-item v-if="type === 'info' || false || read.cardno" label="会员卡号" prop="cardno">
            <el-input
                v-model="form.cardno"
                placeholder="请输入会员卡号"
                :readonly="type === 'info' || read.cardno"
                clearable
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || false || read.memberlevel" label="会员等级" prop="memberlevel">
            <el-select v-model="form.memberlevel" placeholder="请选择会员等级" :readonly="type === 'info' || read.memberlevel">
        ...
        <el-form-item v-if="type === 'info' || false || read.expiretime" label="会员到期时间" prop="expiretime">
            <el-date-picker
                value-format="YYYY-MM-DD HH:mm:ss"
                v-model="form.expiretime"
                type="datetime"
                placeholder="请选择会员到期时间"
                :readonly="type === 'info' || read.expiretime"
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || false || read.remainingcourses" label="剩余课时" prop="remainingcourses">
            <el-input-number v-model="form.remainingcourses" placeholder="请输入剩余课时" controls-position="right" :readonly="type === 'info' || read.remainingcourses"/>
        </el-form-item>
...
<script setup lang="ts">
  ...
  const rules: Record<string, any[]> = {
    useraccount: [ { trigger: 'blur' , message: '账号不能为空' , required: true } ],
    userpassword: [ { trigger: 'blur' , message: '密码不能为空' , required: true } ],
    name: [ { trigger: 'blur' , message: '姓名不能为空' , required: true } ],
    age: [ { validator: validateNumber , trigger: 'blur' } ],
    phone: [ { validator: validateMobile , trigger: 'blur' } ],
    money: [ { validator: validateNumber , trigger: 'blur' } ],
    remainingcourses: [ { validator: validateNumber , trigger: 'blur' } ],
  }
      ...
      for (const key in crossObj) {
        if (key === 'useraccount') {
            if (!amountFieldName || key !== amountFieldName) {
              form.useraccount=crossObj[key]
              read.useraccount=true
            }
            continue
        }
        if (key === 'userpassword') {
            if (!amountFieldName || key !== amountFieldName) {
              form.userpassword=crossObj[key]
              read.userpassword=true
            }
            continue
        }
        if (key === 'name') {
            if (!amountFieldName || key !== amountFieldName) {
              form.name=crossObj[key]
              read.name=true
            }
            continue
        }
        ...
        if (key === 'memberlevel') {
            if (!amountFieldName || key !== amountFieldName) {
              form.memberlevel=crossObj[key]
              read.memberlevel=true
            }
            continue
        }
        if (key === 'expiretime') {
            if (!amountFieldName || key !== amountFieldName) {
              form.expiretime=crossObj[key]
              read.expiretime=true
            }
            continue
        }
        if (key === 'remainingcourses') {
            if (!amountFieldName || key !== amountFieldName) {
              form.remainingcourses=crossObj[key]
              read.remainingcourses=true
            }
            continue
        }
      }
  ...
  const getInfo = async (itemId: number) => {
    try {
      const { data } = await http.get(`user/info/${itemId}`)
      if (data){
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = id.value ? 'user/update' : 'user/save'
        ...
        if (crossObj) {
          ...
          if (statusColumnName && !statusColumnName.startsWith('[')) {
            ...
            if (crossrefid && crossuserid) {
              ...
              const { data } = await http.get('user/page', { params })
