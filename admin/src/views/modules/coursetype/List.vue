  ...
  <div class="main-content bg-gray-50 p-6">
    ...
    <div class="bg-white p-4 rounded-lg shadow mb-4">
      <el-form :inline="true" :model="searchForm" class="search-form">
        <div class="inline-block mr-4">
            ...
            <el-input
            v-model="searchForm.coursetypename"
            placeholder="分类名称"
            clearable
    ...
    <div class="bg-white p-4 rounded-lg shadow mb-4">
      <div class="flex justify-between items-center">
        <div class="space-x-2">
          <el-button
            v-if="!false && isAuth('coursetype', '新增')"
            type="success"
          ...
          <el-button
            v-if="!false && isAuth('coursetype', '删除')"
            type="danger"
    ...
    <div v-if="!mapConfig.enabled || viewMode === 'table'" class="bg-white rounded-lg shadow">
      ...
      >
        ...
        <el-table-column prop="coursetypename" label="分类名称" />
        <!-- 操作列 -->
...
<script setup lang="ts">
  ...
  import type { CoursetypeItem } from '@/types/common'
  ...
  // 响应式数据
  const dataList = ref<CoursetypeItem[]>([])
  const dataListLoading = ref(false)
  const dataListSelections = ref<CoursetypeItem[]>([])
  ...
  const musicTracks = computed(() => {
    ...
    return dataList.value
      .map(row => {
        ...
        return {
          id: record?.id ?? url,
          name: nameRaw || String(record?.name || '课程分类'),
          artist: artistRaw,
  ...
  const playMusic = (row: CoursetypeItem) => {
  ...
  const getDataList = () => {
    ...
    if (searchForm.coursetypename) {
      params.coursetypename = '%' + searchForm.coursetypename + '%'
    }
    http.get('coursetype/page', { params }).then((response: any) => {
  ...
  // 重置
  const reset = () => {
    searchForm.coursetypename = undefined
    pageIndex.value = 1
    getDataList()
  }
  ...
  // 多选
  const selectionChangeHandler = (val: CoursetypeItem[]) => {
  ...
  const deleteHandler = async (ids?: number[]) => {
    ...
    try {
      ...
      await http.post('coursetype/delete', deleteIds)
  ...
  const inlineCrossActionHandler = async (
    ...
    try {
      ...
      if (statusColumnName && statusColumnValue) {
        ...
        await http.post(`coursetype/update`, curr)
