<template>
  <div class="add-edit-dialog">
    <el-dialog
        v-model="visible"
        :title="id ? (type === 'info' ? '查看课程分类' : '修改课程分类') : '新增课程分类'"
        width="60%"
    ...
    >
      <el-form
        ...
        class="space-y-4"
      >
        <el-form-item v-if="type === 'info' || true || read.coursetypename" label="分类名称" prop="coursetypename">
            <el-input
                v-model="form.coursetypename"
                placeholder="请输入分类名称"
                :readonly="type === 'info' || read.coursetypename"
                clearable
            />
        </el-form-item>
...
<script setup lang="ts">
  ...
  const rules: Record<string, any[]> = {
    coursetypename: [ { trigger: 'blur' , message: '分类名称不能为空' , required: true } ],
  }
  ...
  const init = async (itemId?: number, itemType?: string) => {
    ...
    if (type.value === 'cross') {
      ...
      for (const key in crossObj) {
        if (key === 'coursetypename') {
            if (!amountFieldName || key !== amountFieldName) {
              form.coursetypename=crossObj[key]
              read.coursetypename=true
            }
            continue
        }
      }
  ...
  const getInfo = async (itemId: number) => {
    try {
      const { data } = await http.get(`coursetype/info/${itemId}`)
      if (data){
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = id.value ? 'coursetype/update' : 'coursetype/save'
        ...
        if (crossObj) {
          ...
          if (statusColumnName && !statusColumnName.startsWith('[')) {
            ...
            if (crossrefid && crossuserid) {
              ...
              const { data } = await http.get('coursetype/page', { params })
