<template>
  <div class="add-edit-dialog">
    <el-dialog
        v-model="visible"
        :title="id ? (type === 'info' ? '查看公告分类' : '修改公告分类') : '新增公告分类'"
        width="60%"
      ...
      >
        <el-form-item v-if="type === 'info' || true || read.typename" label="分类名称" prop="typename">
            <el-input
                ...
                placeholder="请输入分类名称"
                :readonly="type === 'info' || read.typename"
                clearable
            />
        </el-form-item>
...
<script setup lang="ts">
  ...
  const getInfo = async (itemId: number) => {
    try {
      const { data } = await http.get(`newstype/info/${itemId}`)
      if (data){
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = id.value ? 'newstype/update' : 'newstype/save'
        ...
        if (crossObj) {
          ...
          if (statusColumnName && !statusColumnName.startsWith('[')) {
            ...
            if (crossrefid && crossuserid) {
              ...
              const { data } = await http.get('newstype/page', { params })
