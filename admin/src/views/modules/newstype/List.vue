...
<script setup lang="ts">
  ...
  import type { NewstypeItem } from '@/types/common'
  ...
  // 响应式数据
  const dataList = ref<NewstypeItem[]>([])
  const dataListLoading = ref(false)
  const dataListSelections = ref<NewstypeItem[]>([])
  ...
  const musicTracks = computed(() => {
    ...
    return dataList.value
      .map(row => {
        ...
        return {
          id: record?.id ?? url,
          name: nameRaw || String(record?.name || '公告分类'),
          artist: artistRaw,
  ...
  const playMusic = (row: NewstypeItem) => {
  ...
  const getDataList = () => {
    ...
    http.get('newstype/page', { params }).then((response: any) => {
  ...
  // 多选
  const selectionChangeHandler = (val: NewstypeItem[]) => {
  ...
  const deleteHandler = async (ids?: number[]) => {
    ...
    try {
      ...
      await http.post('newstype/delete', deleteIds)
  ...
  const inlineCrossActionHandler = async (
    ...
    try {
      ...
      if (statusColumnName && statusColumnValue) {
        ...
        await http.post(`newstype/update`, curr)
