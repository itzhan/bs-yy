<template>
  <div class="add-edit-dialog">
    <el-dialog
        v-model="visible"
        :title="id ? (type === 'info' ? '查看轮播图' : '修改轮播图') : '新增轮播图'"
        width="60%"
    ...
    >
      <el-form
        ...
        class="space-y-4"
      >
        <el-form-item v-if="type === 'info' || true || read.name" label="配置参数名称" prop="name">
            <el-input
                v-model="form.name"
        ...
        <el-form-item v-if="type === 'info' || true || read.value" label="配置参数值" prop="value">
            <el-input
                v-model="form.value"
                placeholder="请输入配置参数值"
                :readonly="type === 'info' || read.value"
                clearable
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.url" label="url" prop="url">
            <UploadField
                v-model="form.url"
                upload-type="image"
                :readonly="type === 'info' || read.url"
            />
        </el-form-item>
...
<script setup lang="ts">
  ...
  const getInfo = async (itemId: number) => {
    try {
      const { data } = await http.get(`config/info/${itemId}`)
      if (data){
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = id.value ? 'config/update' : 'config/save'
        ...
        if (crossObj) {
          ...
          if (statusColumnName && !statusColumnName.startsWith('[')) {
            ...
            if (crossrefid && crossuserid) {
              ...
              const { data } = await http.get('config/page', { params })
