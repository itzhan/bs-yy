<template>
  <div class="add-edit-dialog">
    <el-dialog
        v-model="visible"
        :title="id ? (type === 'info' ? '查看会员交流评论表' : '修改会员交流评论表') : '新增会员交流评论表'"
        width="60%"
...
<script setup lang="ts">
  ...
  const getInfo = async (itemId: number) => {
    try {
      const { data } = await http.get(`discussshare/info/${itemId}`)
      if (data){
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = id.value ? 'discussshare/update' : 'discussshare/save'
        ...
        if (crossObj) {
          ...
          if (statusColumnName && !statusColumnName.startsWith('[')) {
            ...
            if (crossrefid && crossuserid) {
              ...
              const { data } = await http.get('discussshare/page', { params })
