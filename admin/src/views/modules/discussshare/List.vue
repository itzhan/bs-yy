...
import type { DiscussshareItem } from '@/types/common'
import { useRoute } from 'vue-router'

const route = useRoute()

// 响应式数据
const dataList = ref<DiscussshareItem[]>([])
...
const currentReplyComment = ref<DiscussshareItem>({} as DiscussshareItem)
// 回复表单
...
const deleteHandler = async (ids?: number[]) => {
  ...
  try {
    ...
    await http.post('discussshare/delete', deleteIds)
    await getDataList()
  } catch {
    // 用户取消操作
  }
...
// 回复处理
const replyHandler = (comment: DiscussshareItem) => {
...
const submitReply = async () => {
  ...
  try {
    ...
    await http.post('discussshare/update', updateData)
