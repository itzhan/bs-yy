  ...
  <div class="main-content bg-gray-50 p-6">
    ...
    <div class="bg-white p-4 rounded-lg shadow mb-4">
      <div class="flex justify-between items-center">
        <div class="space-x-2">
          <el-button
            v-if="!false && isAuth('cancelcardrenewal', '新增')"
            type="success"
          ...
          <el-button
            v-if="!false && isAuth('cancelcardrenewal', '删除')"
            type="danger"
          ...
          <download-excel
            v-if="isAuth('cancelcardrenewal', '导出')"
    ...
    <div v-if="!mapConfig.enabled || viewMode === 'table'" class="bg-white rounded-lg shadow">
      <el-table
        v-if="isAuth('cancelcardrenewal', '查看')"
        class="w-full"
        :data="dataList"
      ...
      >
        ...
        <el-table-column label="操作" width="240" fixed="right">
          <template #default="scope">
            ...
            <el-button
                v-if="isAuth('cancelcardrenewal', '修改')"
                type="warning"
                size="small"
            ...
            <el-button
                v-if="isAuth('cancelcardrenewal', '删除')"
                type="danger"
                size="small"
            ...
            <el-button
                v-if="isAuth('cancelcardrenewal', '私聊')"
                type="success"
                size="small"
...
<script setup lang="ts">
  ...
  import type { CancelcardrenewalItem } from '@/types/common'
  ...
  const chatBackendTargetTable = 'cancelcardrenewal'
  let websock: WebSocket | null = null

  // 响应式数据
  const dataList = ref<CancelcardrenewalItem[]>([])
  const dataListLoading = ref(false)
  const dataListSelections = ref<CancelcardrenewalItem[]>([])
  ...
  const musicTracks = computed(() => {
    ...
    return dataList.value
      .map(row => {
        ...
        return {
          id: record?.id ?? url,
          name: nameRaw || String(record?.name || '取消续卡记录'),
          artist: artistRaw,
  ...
  const playMusic = (row: CancelcardrenewalItem) => {
  ...
  const getDataList = () => {
    ...
    http.get('cancelcardrenewal/page', { params }).then((response: any) => {
  ...
  // 多选
  const selectionChangeHandler = (val: CancelcardrenewalItem[]) => {
  ...
  const deleteHandler = async (ids?: number[]) => {
    ...
    try {
      ...
      await http.post('cancelcardrenewal/delete', deleteIds)
  ...
  const inlineCrossActionHandler = async (
    ...
    try {
      ...
      if (statusColumnName && statusColumnValue) {
        ...
        await http.post(`cancelcardrenewal/update`, curr)
