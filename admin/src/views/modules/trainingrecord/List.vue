<template>
  <div class="main-content bg-gray-50 p-6">
    ...
    <div class="bg-white p-4 rounded-lg shadow mb-4">
      <el-button v-if="isAuth('trainingrecord', '新增')" type="success" @click="addOrUpdateHandler()">新增</el-button>
      <el-button v-if="isAuth('trainingrecord', '删除')" type="danger" :disabled="!dataListSelections.length" @click="deleteHandler()">删除</el-button>
    </div>
    ...
    <div class="bg-white p-4 rounded-lg shadow">
      <el-table :data="dataList" border v-loading="dataListLoading" @selection-change="selectionChangeHandler">
        ...
        <el-table-column prop="recorddate" label="记录时间" min-width="160" />
        ...
        <el-table-column label="操作" width="160">
          <template #default="scope">
            ...
            <el-button v-if="isAuth('trainingrecord', '修改')" size="small" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
...
const getDataList = async () => {
  ...
  try {
    ...
    const res: any = await http.get('trainingrecord/page', { params })
...
const deleteHandler = async (ids?: number[]) => {
  ...
  try {
    ...
    await http.post('trainingrecord/delete', deleteIds)
