  ...
  <div class="main-content bg-gray-50 p-6">
    ...
    <div class="bg-white p-4 rounded-lg shadow mb-4">
      <el-form :inline="true" :model="searchForm" class="search-form">
        ...
        <div class="inline-block mr-4">
            <label class="inline-block mr-2 leading-10">管理员ID</label>
            <el-input
        ...
        <div class="inline-block mr-4">
            <label class="inline-block mr-2 leading-10">是否回复(0/1)</label>
            <el-input
            v-model="searchForm.isreply"
            placeholder="是否回复(0/1)"
            clearable
            @keydown.enter="search()"
            />
        </div>
        <div class="inline-block mr-4">
            <label class="inline-block mr-2 leading-10">是否已读(0/1)</label>
            <el-input
            v-model="searchForm.isread"
            placeholder="是否已读(0/1)"
            clearable
            @keydown.enter="search()"
            />
        </div>
    ...
    <div v-if="!mapConfig.enabled || viewMode === 'table'" class="bg-white rounded-lg shadow">
      ...
      >
        ...
        <el-table-column prop="isreply" label="是否回复(0/1)" />
        <el-table-column prop="isread" label="是否已读(0/1)" />
...
<script setup lang="ts">
  ...
  import type { ChatItem } from '@/types/common'
  ...
  const chatBackendTargetTable = 'chat'
  let websock: WebSocket | null = null

  // 响应式数据
  const dataList = ref<ChatItem[]>([])
  const dataListLoading = ref(false)
  const dataListSelections = ref<ChatItem[]>([])
  ...
  const musicTracks = computed(() => {
    ...
    return dataList.value
      .map(row => {
        ...
        return {
          id: record?.id ?? url,
          name: nameRaw || String(record?.name || '在线客服'),
          artist: artistRaw,
  ...
  const getDataList = () => {
    ...
    if (searchForm.uname) {
      params.uname = '%' + searchForm.uname + '%'
    }
    http.get('chat/page', { params }).then((response: any) => {
  ...
  const reset = () => {
    ...
    searchForm.isreply = undefined
    searchForm.isread = undefined
    searchForm.uname = undefined
    pageIndex.value = 1
    getDataList()
  }
  ...
  // 多选
  const selectionChangeHandler = (val: ChatItem[]) => {
  ...
  const deleteHandler = async (ids?: number[]) => {
    ...
    try {
      ...
      await http.post('chat/delete', deleteIds)
  ...
  const inlineCrossActionHandler = async (
    ...
    try {
      ...
      if (statusColumnName && statusColumnValue) {
        ...
        await http.post(`chat/update`, curr)
