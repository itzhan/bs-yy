<template>
  <div class="add-edit-dialog">
    <el-dialog
        v-model="visible"
        :title="id ? (type === 'info' ? '查看在线客服' : '修改在线客服') : '新增在线客服'"
        width="60%"
    ...
    >
      <el-form
        ...
        class="space-y-4"
      >
        <el-form-item v-if="type === 'info' || false || read.adminid" label="管理员ID" prop="adminid">
            <el-input-number v-model="form.adminid" placeholder="请输入管理员ID" controls-position="right" :readonly="type === 'info' || read.adminid"/>
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.ask" label="提问" prop="ask">
            <el-input
                v-model="form.ask"
                type="textarea"
                placeholder="请输入提问"
                :readonly="type === 'info' || read.ask"
                clearable
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.reply" label="回复" prop="reply">
            <el-input
                v-model="form.reply"
                type="textarea"
                placeholder="请输入回复"
                :readonly="type === 'info' || read.reply"
                clearable
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.asktype" label="提问类型(1文本/2图片)" prop="asktype">
            <el-input-number v-model="form.asktype" placeholder="请输入提问类型(1文本/2图片)" controls-position="right" :readonly="type === 'info' || read.asktype"/>
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.replytype" label="回复类型(1文本/2图片)" prop="replytype">
            <el-input-number v-model="form.replytype" placeholder="请输入回复类型(1文本/2图片)" controls-position="right" :readonly="type === 'info' || read.replytype"/>
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.isreply" label="是否回复(0/1)" prop="isreply">
            <el-input-number v-model="form.isreply" placeholder="请输入是否回复(0/1)" controls-position="right" :readonly="type === 'info' || read.isreply"/>
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.isread" label="是否已读(0/1)" prop="isread">
            <el-input-number v-model="form.isread" placeholder="请输入是否已读(0/1)" controls-position="right" :readonly="type === 'info' || read.isread"/>
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.uname" label="用户名" prop="uname">
            <el-input
                v-model="form.uname"
                placeholder="请输入用户名"
                :readonly="type === 'info' || read.uname"
                clearable
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.uimage" label="用户头像" prop="uimage">
            <UploadField
                v-model="form.uimage"
                upload-type="image"
                :readonly="type === 'info' || read.uimage"
            />
        </el-form-item>
...
<script setup lang="ts">
  ...
  const rules: Record<string, any[]> = {
    adminid: [ { validator: validateNumber , trigger: 'blur' } ],
    asktype: [ { validator: validateNumber , trigger: 'blur' } ],
    replytype: [ { validator: validateNumber , trigger: 'blur' } ],
    isreply: [ { validator: validateNumber , trigger: 'blur' } ],
    isread: [ { validator: validateNumber , trigger: 'blur' } ],
  }
  ...
  const getInfo = async (itemId: number) => {
    try {
      const { data } = await http.get(`chat/info/${itemId}`)
      if (data){
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = id.value ? 'chat/update' : 'chat/save'
        ...
        if (crossObj) {
          ...
          if (statusColumnName && !statusColumnName.startsWith('[')) {
            ...
            if (crossrefid && crossuserid) {
              ...
              const { data } = await http.get('chat/page', { params })
