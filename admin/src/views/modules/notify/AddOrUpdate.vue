<template>
  <div class="add-edit-dialog">
    <el-dialog
        v-model="visible"
        :title="id ? (type === 'info' ? '查看消息通知' : '修改消息通知') : '新增消息通知'"
        width="60%"
    ...
    >
      <el-form
        ...
        class="space-y-4"
      >
        <el-form-item v-if="type === 'info' || true || read.title" label="消息标题" prop="title">
            <el-input
                v-model="form.title"
                placeholder="请输入消息标题"
        ...
        <el-form-item v-if="type === 'info' || true || read.content" label="消息内容" prop="content">
            <el-input
        ...
        <el-form-item v-if="type === 'info' || true || read.messagetype" label="消息类型" prop="messagetype">
            <el-select v-model="form.messagetype" placeholder="请选择消息类型" :readonly="type === 'info' || read.messagetype">
                <el-option
        ...
        <el-form-item v-if="type === 'info' || true || read.readstatus" label="阅读状态" prop="readstatus">
            <el-select v-model="form.readstatus" placeholder="请选择阅读状态" :readonly="type === 'info' || read.readstatus">
                <el-option label="未读" value="未读"/>
                <el-option label="已读" value="已读"/>
            </el-select>
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.senduser" label="发送人" prop="senduser">
            <el-input
                v-model="form.senduser"
                placeholder="请输入发送人"
                :readonly="type === 'info' || read.senduser"
                clearable
            />
        </el-form-item>
...
<script setup lang="ts">
  ...
  // 消息类型选项
  const messagetypeOptions = ref<string[]>([])
  ...
  const rules: Record<string, any[]> = {
    title: [ { trigger: 'blur' , message: '消息标题不能为空' , required: true } ],
    content: [ { trigger: 'blur' , message: '消息内容不能为空' , required: true } ],
  }
  ...
  const init = async (itemId?: number, itemType?: string) => {
    ...
    if (type.value === 'cross') {
      ...
      for (const key in crossObj) {
        ...
        if (key === 'messagetype') {
            if (!amountFieldName || key !== amountFieldName) {
              form.messagetype=crossObj[key]
              read.messagetype=true
            }
            continue
        }
        if (key === 'readstatus') {
            if (!amountFieldName || key !== amountFieldName) {
              form.readstatus=crossObj[key]
  ...
  const getInfo = async (itemId: number) => {
    try {
      const { data } = await http.get(`notify/info/${itemId}`)
      if (data){
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = id.value ? 'notify/update' : 'notify/save'
  ...
  // 获取消息类型选项（支持联动条件）
  const getMessagetypeOptions = async () => {
    try {
      const params: any = {}
      const response = await http.get('option/notify/messagetype', { params })
      if (response && response.code === 0) {
        messagetypeOptions.value = response.data || []
      } else {
        messagetypeOptions.value = []
      }
    } catch (error) {
      console.error('获取消息类型选项失败:', error)
      messagetypeOptions.value = []
    }
  }
        ...
        if (crossObj) {
          ...
          if (statusColumnName && !statusColumnName.startsWith('[')) {
            ...
            if (crossrefid && crossuserid) {
              ...
              const { data } = await http.get('notify/page', { params })
