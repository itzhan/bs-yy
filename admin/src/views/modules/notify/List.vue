  ...
  <div class="main-content bg-gray-50 p-6">
    ...
    <div class="bg-white p-4 rounded-lg shadow mb-4">
      <el-form :inline="true" :model="searchForm" class="search-form">
        <div class="inline-block mr-4">
            <label class="inline-block mr-2 leading-10">接收用户ID</label>
            <el-input
        ...
        <div class="inline-block mr-4">
            <label class="inline-block mr-2 leading-10">消息标题</label>
            <el-input
        ...
        <div class="inline-block mr-4">
            <label class="inline-block mr-2 leading-10">消息类型</label>
            <div>
                <el-select
        ...
        <div class="inline-block mr-4">
            <label class="inline-block mr-2 leading-10">阅读状态</label>
            <div>
                <el-select v-model="searchForm.readstatus" placeholder="请选择阅读状态" clearable class="w-40">
    ...
    <div v-if="!mapConfig.enabled || viewMode === 'table'" class="bg-white rounded-lg shadow">
      ...
      >
        ...
        <el-table-column prop="senduser" label="发送人" />
        <!-- 操作列 -->
...
<script setup lang="ts">
  ...
  import type { NotifyItem } from '@/types/common'
  ...
  const chatBackendTargetTable = 'notify'
  let websock: WebSocket | null = null

  // 响应式数据
  const dataList = ref<NotifyItem[]>([])
  const dataListLoading = ref(false)
  const dataListSelections = ref<NotifyItem[]>([])
  ...
  const musicTracks = computed(() => {
    ...
    return dataList.value
      .map(row => {
        ...
        return {
          id: record?.id ?? url,
          name: nameRaw || String(record?.name || '消息通知'),
          artist: artistRaw,
  ...
  const playMusic = (row: NotifyItem) => {
  ...
  const getDataList = () => {
    ...
    if (searchForm.readstatus) {
      params.readstatus = '%' + searchForm.readstatus + '%'
    }
    http.get('notify/page', { params }).then((response: any) => {
  ...
  const reset = () => {
    ...
    searchForm.readstatus = undefined
    pageIndex.value = 1
    getDataList()
  }
  ...
  // 多选
  const selectionChangeHandler = (val: NotifyItem[]) => {
  ...
  const deleteHandler = async (ids?: number[]) => {
    ...
    try {
      ...
      await http.post('notify/delete', deleteIds)
  ...
  const inlineCrossActionHandler = async (
    ...
    try {
      ...
      if (statusColumnName && statusColumnValue) {
        ...
        await http.post(`notify/update`, curr)
