<template>
  <div class="add-edit-dialog">
    <el-dialog
        v-model="visible"
        :title="id ? (type === 'info' ? '查看办卡记录退款' : '修改办卡记录退款') : '新增办卡记录退款'"
        width="60%"
    ...
    >
      ...
      >
        ...
        <el-form-item v-if="type === 'info' || true || read.refundreason" label="退款原因" prop="refundreason">
            <el-input
                v-model="form.refundreason"
                type="textarea"
                placeholder="请输入退款原因"
                :readonly="type === 'info' || read.refundreason"
                clearable
            />
        </el-form-item>
...
<script setup lang="ts">
  ...
  const init = async (itemId?: number, itemType?: string) => {
    ...
    if (type.value === 'cross') {
      ...
      for (const key in crossObj) {
        ...
        if (key === 'refundreason') {
            if (!amountFieldName || key !== amountFieldName) {
              form.refundreason=crossObj[key]
              read.refundreason=true
            }
            continue
        }
      }
  ...
  const getInfo = async (itemId: number) => {
    try {
      const { data } = await http.get(`refundcardapplication/info/${itemId}`)
      if (data){
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = id.value ? 'refundcardapplication/update' : 'refundcardapplication/save'
        ...
        if (crossObj) {
          ...
          if (statusColumnName && !statusColumnName.startsWith('[')) {
            ...
            if (crossrefid && crossuserid) {
              ...
              const { data } = await http.get('refundcardapplication/page', { params })
