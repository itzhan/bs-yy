  ...
  <div class="main-content bg-gray-50 p-6">
    ...
    <div class="bg-white p-4 rounded-lg shadow mb-4">
      <el-form :inline="true" :model="searchForm" class="search-form">
        <div class="inline-block mr-4">
            <label class="inline-block mr-2 leading-10">标题</label>
            <el-input
        ...
        <div class="inline-block mr-4">
            <label class="inline-block mr-2 leading-10">分类</label>
            <div>
                <el-select
    ...
    <div v-if="!mapConfig.enabled || viewMode === 'table'" class="bg-white rounded-lg shadow">
      ...
      >
        ...
        <el-table-column prop="sharetitle" label="标题" />
        <el-table-column prop="shareimage" label="配图" width="200">
        <template #default="scope">
            <div v-if="scope.row.shareimage">
            <img
                v-if="scope.row.shareimage.substring(0,4)=='http'"
                :src="scope.row.shareimage.split(',')[0]"
                width="100"
                height="100"
                style="object-fit: cover"
                @click="imgPreView(scope.row.shareimage.split(',')[0])"
                class="cursor-pointer"
            >
            <img
                v-else
                :src="baseUrl + scope.row.shareimage.split(',')[0]"
                width="100"
                height="100"
                style="object-fit: cover"
                @click="imgPreView(baseUrl + scope.row.shareimage.split(',')[0])"
                class="cursor-pointer"
            >
            </div>
        ...
        <el-table-column prop="sharetype" label="分类" />
...
<script setup lang="ts">
  ...
  import type { ShareItem } from '@/types/common'
  ...
  // 响应式数据
  const dataList = ref<ShareItem[]>([])
  const dataListLoading = ref(false)
  const dataListSelections = ref<ShareItem[]>([])
  ...
  const musicTracks = computed(() => {
    ...
    return dataList.value
      .map(row => {
        ...
        return {
          id: record?.id ?? url,
          name: nameRaw || String(record?.name || '会员交流'),
          artist: artistRaw,
  ...
  const exportRows = computed(() =>
    dataList.value.map((row) => {
      ...
      if (plain.shareimage !== undefined && plain.shareimage !== null) {
        const images = String(plain.shareimage)
          .split(',')
          .map((item) => item.trim())
          .filter(Boolean)
          .map((item) => resolveFileUrl(item))
        plain.shareimage = images.join('; ')
      }
      return plain
    })
  )
  ...
  const getDataList = () => {
    ...
    if (searchForm.sharetitle) {
      params.sharetitle = '%' + searchForm.sharetitle + '%'
    }
    if (searchForm.sharetype) {
      params.sharetype = '%' + searchForm.sharetype + '%'
    }
    if (searchForm.auditstatus) {
      params.auditstatus = '%' + searchForm.auditstatus + '%'
    }
    http.get('share/page', { params }).then((response: any) => {
  ...
  // 重置
  const reset = () => {
    searchForm.sharetitle = undefined
  ...
  // 多选
  const selectionChangeHandler = (val: ShareItem[]) => {
  ...
  const deleteHandler = async (ids?: number[]) => {
    ...
    try {
      ...
      await http.post('share/delete', deleteIds)
  ...
  const discussListHandler = (id: number) => {
    router.push({
      path: '/dashboard/modules/discussshare',
      query: { refid: id }
    })
  }
  ...
  const inlineCrossActionHandler = async (
    ...
    try {
      ...
      if (statusColumnName && statusColumnValue) {
        ...
        await http.post(`share/update`, curr)
  ...
  const auditBatchHandler = async () => {
    ...
    try {
      ...
      if (!valid) return
      await http.post(
              `share/audit/batch?auditstatus=${auditBatchForm.auditstatus}&auditreply=${auditBatchForm.auditreply}`,
              batchIds.value
      )
      getDataList()
  ...
  // 分类 选项数据
  const sharetypeOptions = ref<string[]>([])
  // 加载分类选项（支持条件联动）
  const getSharetypeOptions = async () => {
    try {
      const params: any = {}
      const response = await http.get('option/sharetype/sharetypename', { params })
      if (response && response.code === 0) {
        sharetypeOptions.value = response.data || []
      } else {
