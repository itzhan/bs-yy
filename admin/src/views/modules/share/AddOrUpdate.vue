<template>
  <div class="add-edit-dialog">
    <el-dialog
        v-model="visible"
        :title="id ? (type === 'info' ? '查看会员交流' : '修改会员交流') : '新增会员交流'"
        width="60%"
    ...
    >
      <el-form
        ...
        class="space-y-4"
      >
        <el-form-item v-if="type === 'info' || true || read.sharetitle" label="标题" prop="sharetitle">
            <el-input
                v-model="form.sharetitle"
                placeholder="请输入标题"
                :readonly="type === 'info' || read.sharetitle"
                clearable
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.sharecontent" label="内容" prop="sharecontent">
            <RichEditor
                    v-model="form.sharecontent"
                    :readonly="type === 'info' || read.sharecontent"
                    placeholder="请输入内容"
                    height="400px"
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.shareimage" label="配图" prop="shareimage">
            <UploadField
                v-model="form.shareimage"
                upload-type="image"
                :readonly="type === 'info' || read.shareimage"
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.sharetype" label="分类" prop="sharetype">
            <el-select v-model="form.sharetype" placeholder="请选择分类" :readonly="type === 'info' || read.sharetype">
                <el-option
...
<script setup lang="ts">
  ...
  const rules: Record<string, any[]> = {
    sharetitle: [ { trigger: 'blur' , message: '标题不能为空' , required: true } ],
    sharecontent: [ { trigger: 'blur' , message: '内容不能为空' , required: true } ],
    sharetype: [ { trigger: 'blur' , message: '分类不能为空' , required: true } ],
  }
      ...
      for (const key in crossObj) {
        if (key === 'sharetitle') {
            if (!amountFieldName || key !== amountFieldName) {
              form.sharetitle=crossObj[key]
              read.sharetitle=true
            }
            continue
        }
        if (key === 'sharecontent') {
            if (!amountFieldName || key !== amountFieldName) {
              form.sharecontent=crossObj[key]
              read.sharecontent=true
            }
            continue
        }
        if (key === 'shareimage') {
            if (!amountFieldName || key !== amountFieldName) {
              form.shareimage=crossObj[key]
              read.shareimage=true
            }
            continue
        }
        if (key === 'sharetype') {
            if (!amountFieldName || key !== amountFieldName) {
              form.sharetype=crossObj[key]
              read.sharetype=true
            }
            continue
        }
  ...
  const getInfo = async (itemId: number) => {
    try {
      const { data } = await http.get(`share/info/${itemId}`)
      if (data){
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = id.value ? 'share/update' : 'share/save'
        ...
        if (crossObj) {
          ...
          if (statusColumnName && !statusColumnName.startsWith('[')) {
            ...
            if (crossrefid && crossuserid) {
              ...
              const { data } = await http.get('share/page', { params })
