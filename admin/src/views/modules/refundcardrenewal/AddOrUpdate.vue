<template>
  <div class="add-edit-dialog">
    <el-dialog
        v-model="visible"
        :title="id ? (type === 'info' ? '查看续卡记录退款' : '修改续卡记录退款') : '新增续卡记录退款'"
        width="60%"
    ...
    >
      ...
      >
        ...
        <el-form-item v-if="type === 'info' || true || read.renewaldays" label="续费时长" prop="renewaldays">
            <el-input-number v-model="form.renewaldays" placeholder="请输入续费时长" controls-position="right" :readonly="type === 'info' || read.renewaldays"/>
        </el-form-item>
...
<script setup lang="ts">
  ...
  const rules: Record<string, any[]> = {
    ...
    renewaldays: [ { trigger: 'blur' , message: '续费时长不能为空' , required: true }, { validator: validateNumber , trigger: 'blur' } ],
  }
  ...
  const init = async (itemId?: number, itemType?: string) => {
    ...
    if (type.value === 'cross') {
      ...
      for (const key in crossObj) {
        ...
        if (key === 'renewaldays') {
            if (!amountFieldName || key !== amountFieldName) {
              form.renewaldays=crossObj[key]
              read.renewaldays=true
            }
            continue
        }
        if (key === 'refundreason') {
  ...
  const getInfo = async (itemId: number) => {
    try {
      const { data } = await http.get(`refundcardrenewal/info/${itemId}`)
      if (data){
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = id.value ? 'refundcardrenewal/update' : 'refundcardrenewal/save'
        ...
        if (crossObj) {
          ...
          if (statusColumnName && !statusColumnName.startsWith('[')) {
            ...
            if (crossrefid && crossuserid) {
              ...
              const { data } = await http.get('refundcardrenewal/page', { params })
