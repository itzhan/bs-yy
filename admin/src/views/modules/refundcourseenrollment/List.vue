  ...
  <div class="main-content bg-gray-50 p-6">
    ...
    <div class="bg-white p-4 rounded-lg shadow mb-4">
      <div class="flex justify-between items-center">
        <div class="space-x-2">
          <el-button
            v-if="!false && isAuth('refundcourseenrollment', '新增')"
            type="success"
          ...
          <el-button
            v-if="!false && isAuth('refundcourseenrollment', '删除')"
            type="danger"
          ...
          <download-excel
            v-if="isAuth('refundcourseenrollment', '导出')"
    ...
    <div v-if="!mapConfig.enabled || viewMode === 'table'" class="bg-white rounded-lg shadow">
      <el-table
        v-if="isAuth('refundcourseenrollment', '查看')"
        class="w-full"
        :data="dataList"
      ...
      >
        ...
        <el-table-column label="操作" width="240" fixed="right">
          <template #default="scope">
            ...
            <el-button
                v-if="isAuth('refundcourseenrollment', '修改')"
                type="warning"
                size="small"
            ...
            <el-button
                v-if="isAuth('refundcourseenrollment', '删除')"
                type="danger"
                size="small"
            ...
            <el-button
                v-if="isAuth('refundcourseenrollment', '私聊')"
                type="success"
                size="small"
            ...
            <el-button
                v-if="isAuth('refundcourseenrollment', '审核') && scope.row.auditstatus === '待审核'"
                type="success"
                size="small"
...
<script setup lang="ts">
  ...
  import type { RefundcourseenrollmentItem } from '@/types/common'
  ...
  const chatBackendTargetTable = 'refundcourseenrollment'
  let websock: WebSocket | null = null

  // 响应式数据
  const dataList = ref<RefundcourseenrollmentItem[]>([])
  const dataListLoading = ref(false)
  const dataListSelections = ref<RefundcourseenrollmentItem[]>([])
  ...
  const musicTracks = computed(() => {
    ...
    return dataList.value
      .map(row => {
        ...
        return {
          id: record?.id ?? url,
          name: nameRaw || String(record?.name || '课程报名记录退款'),
          artist: artistRaw,
  ...
  const playMusic = (row: RefundcourseenrollmentItem) => {
  ...
  const getDataList = () => {
    ...
    http.get('refundcourseenrollment/page', { params }).then((response: any) => {
  ...
  // 多选
  const selectionChangeHandler = (val: RefundcourseenrollmentItem[]) => {
  ...
  const deleteHandler = async (ids?: number[]) => {
    ...
    try {
      ...
      await http.post('refundcourseenrollment/delete', deleteIds)
  ...
  const auditHandler = async (id: number, auditstatus: string) => {
    ...
    try {
      ...
      await http.post('refundcourseenrollment/audit', { id, auditstatus })
  ...
  const inlineCrossActionHandler = async (
    ...
    try {
      ...
      if (statusColumnName && statusColumnValue) {
        ...
        await http.post(`refundcourseenrollment/update`, curr)
