<template>
  <div class="add-edit-dialog">
    <el-dialog
        v-model="visible"
        :title="id ? (type === 'info' ? '查看健身商品' : '修改健身商品') : '新增健身商品'"
        width="60%"
      ...
      >
        <el-form-item v-if="type === 'info' || true || read.productname" label="商品名称" prop="productname">
            <el-input
                ...
                placeholder="请输入商品名称"
                :readonly="type === 'info' || read.productname"
                clearable
            />
        </el-form-item>
        ...
        <el-form-item v-if="type === 'info' || true || read.productprice" label="价格" prop="productprice">
            <el-input-number v-model="form.productprice" placeholder="请输入价格" controls-position="right" :readonly="type === 'info' || read.productprice"/>
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.stock" label="库存" prop="stock">
            <el-input-number v-model="form.stock" placeholder="请输入库存" controls-position="right" :readonly="type === 'info' || read.stock"/>
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.productdesc" label="商品描述" prop="productdesc">
            <RichEditor
                    v-model="form.productdesc"
                    :readonly="type === 'info' || read.productdesc"
                    placeholder="请输入商品描述"
                    height="400px"
            />
        </el-form-item>
...
<script setup lang="ts">
  ...
  const rules: Record<string, any[]> = {
    ...
    producttype: [ { trigger: 'blur' , message: '商品分类不能为空' , required: true } ],
    productprice: [ { trigger: 'blur' , message: '价格不能为空' , required: true }, { validator: validateNumber , trigger: 'blur' } ],
    stock: [ { trigger: 'blur' , message: '库存不能为空' , required: true }, { validator: validateNumber , trigger: 'blur' } ],
  }
  ...
  const init = async (itemId?: number, itemType?: string) => {
    ...
    if (type.value === 'cross') {
      ...
      for (const key in crossObj) {
        ...
        if (key === 'productdesc') {
            if (!amountFieldName || key !== amountFieldName) {
              form.productdesc=crossObj[key]
              read.productdesc=true
            }
            continue
        }
      }
  ...
  const getInfo = async (itemId: number) => {
    try {
      const { data } = await http.get(`product/info/${itemId}`)
      if (data){
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = id.value ? 'product/update' : 'product/save'
        ...
        if (crossObj) {
          ...
          if (statusColumnName && !statusColumnName.startsWith('[')) {
            ...
            if (crossrefid && crossuserid) {
              ...
              const { data } = await http.get('product/page', { params })
