  ...
  <div class="main-content bg-gray-50 p-6">
    ...
    <div class="bg-white p-4 rounded-lg shadow mb-4">
      <el-form :inline="true" :model="searchForm" class="search-form">
        ...
        <div class="inline-block mr-4">
            ...
            <el-input
            v-model="searchForm.productprice_start"
            placeholder="最小价格"
            clearable
            @keydown.enter="search()"
            />
        ...
        <div class="inline-block mr-4">
            ...
            <el-input
            v-model="searchForm.productprice_end"
            placeholder="最大价格"
            clearable
            @keydown.enter="search()"
            />
    ...
    <div v-if="!mapConfig.enabled || viewMode === 'table'" class="bg-white rounded-lg shadow">
      ...
      >
        ...
        <el-button
                v-if="isAuth('product', '购买')"
                type="success"
                size="small"
                @click="productorderCrossAddOrUpdateHandler(scope.row, 'cross', '下单成功', '', '', '购买')"
            >
            购买
            </el-button>
            <el-button
                v-if="isAuth('product', '查看评论')"
                type="success"
                size="small"
    ...
    <!-- productorder 跨表组件 -->
    <productorder-cross-add-or-update
    v-if="productorderCrossAddOrUpdateFlag"
    ref="productorderCrossAddOrUpdateRef"
    @refreshDataList="getDataList"
    ></productorder-cross-add-or-update>
...
<script setup lang="ts">
  ...
  import productorderCrossAddOrUpdate from '../productorder/AddOrUpdate.vue'
  ...
  // 响应式数据
  const dataList = ref<ProductItem[]>([])
  const dataListLoading = ref(false)
  const dataListSelections = ref<ProductItem[]>([])
  ...
  const musicTracks = computed(() => {
    ...
    return dataList.value
      .map(row => {
        ...
        return {
          id: record?.id ?? url,
          name: nameRaw || String(record?.name || '健身商品'),
          artist: artistRaw,
  ...
  const playMusic = (row: ProductItem) => {
  ...
  // 跨表组件显示标志（去重）
  const productorderCrossAddOrUpdateFlag = ref(false)
  const productorderCrossAddOrUpdateRef = ref()
  ...
  const searchForm = reactive({
    ...
    productprice_start: undefined,
    productprice_end: undefined,
  ...
  const getDataList = () => {
    ...
    if (searchForm.productprice_start) {
      params.productprice_start = searchForm.productprice_start
    }
    if (searchForm.productprice_end) {
      params.productprice_end = searchForm.productprice_end
    }
    if (searchForm.auditstatus) {
      params.auditstatus = '%' + searchForm.auditstatus + '%'
    }
    http.get('product/page', { params }).then((response: any) => {
  ...
  const reset = () => {
    ...
    searchForm.productprice_start = undefined
    searchForm.productprice_end = undefined
  ...
  // 多选
  const selectionChangeHandler = (val: ProductItem[]) => {
  ...
  const deleteHandler = async (ids?: number[]) => {
    ...
    try {
      ...
      await http.post('product/delete', deleteIds)
  ...
  // 购买
  const productorderCrossAddOrUpdateHandler = (row: any, type: string, tips: string, statusColumnName: string, statusColumnValue: string, buttonName: string) => {
    // 显示购买弹窗
    productorderCrossAddOrUpdateFlag.value = true

    localStorage.setItem('crossObj', JSON.stringify(row))
    localStorage.setItem('crossTable', 'product')
    ...
    if (statusColumnName && !statusColumnName.startsWith('[')) {
      ...
      if (crossObj[statusColumnName] === statusColumnValue) {
        ...
        productorderCrossAddOrUpdateFlag.value = false
        return
      }
    }
    // 调用跨表组件
    nextTick(() => {
      productorderCrossAddOrUpdateRef.value?.init(undefined, type)
    })
  }
  ...
  const discussListHandler = (id: number) => {
    router.push({
      path: '/dashboard/modules/discussproduct',
      query: { refid: id }
    })
  }
  ...
  const inlineCrossActionHandler = async (
    ...
    try {
      ...
      if (statusColumnName && statusColumnValue) {
        ...
        await http.post(`product/update`, curr)
  ...
  const auditBatchHandler = async () => {
    ...
    try {
      ...
      if (!valid) return
      await http.post(
              `product/audit/batch?auditstatus=${auditBatchForm.auditstatus}&auditreply=${auditBatchForm.auditreply}`,
              batchIds.value
      )
      getDataList()
