<template>
  <div class="add-edit-dialog">
    <el-dialog
        v-model="visible"
        :title="id ? (type === 'info' ? '查看教练' : '修改教练') : '新增教练'"
        width="60%"
    ...
    >
      <el-form
        ...
        class="space-y-4"
      >
        <el-form-item v-if="type === 'info' || true || read.coachaccount" label="账号" prop="coachaccount">
            <el-input
                v-model="form.coachaccount"
                placeholder="请输入账号"
                :readonly="type === 'info' || read.coachaccount"
                clearable
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.coachpassword" label="密码" prop="coachpassword">
            <el-input
                v-model="form.coachpassword"
                placeholder="请输入密码"
                :readonly="type === 'info' || read.coachpassword"
                clearable
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.coachname" label="教练姓名" prop="coachname">
            <el-input
                v-model="form.coachname"
                placeholder="请输入教练姓名"
                :readonly="type === 'info' || read.coachname"
                clearable
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.coachimage" label="教练头像" prop="coachimage">
            <UploadField
                v-model="form.coachimage"
                upload-type="image"
                :readonly="type === 'info' || read.coachimage"
            />
        </el-form-item>
        ...
        <el-form-item v-if="type === 'info' || true || read.jobno" label="工号" prop="jobno">
            <el-input
                v-model="form.jobno"
                placeholder="请输入工号"
                :readonly="type === 'info' || read.jobno"
                clearable
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.specialty" label="擅长领域" prop="specialty">
            <el-input
                v-model="form.specialty"
                placeholder="请输入擅长领域"
                :readonly="type === 'info' || read.specialty"
                clearable
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.coachlevel" label="教练等级" prop="coachlevel">
            <el-select v-model="form.coachlevel" placeholder="请选择教练等级" :readonly="type === 'info' || read.coachlevel">
        ...
        <el-form-item v-if="type === 'info' || true || read.workyears" label="从业年限" prop="workyears">
            <el-input-number v-model="form.workyears" placeholder="请输入从业年限" controls-position="right" :readonly="type === 'info' || read.workyears"/>
        </el-form-item>
...
<script setup lang="ts">
  ...
  const rules: Record<string, any[]> = {
    coachaccount: [ { trigger: 'blur' , message: '账号不能为空' , required: true } ],
    coachpassword: [ { trigger: 'blur' , message: '密码不能为空' , required: true } ],
    coachname: [ { trigger: 'blur' , message: '教练姓名不能为空' , required: true } ],
    phone: [ { validator: validateMobile , trigger: 'blur' } ],
    jobno: [ { trigger: 'blur' , message: '工号不能为空' , required: true } ],
    workyears: [ { validator: validateNumber , trigger: 'blur' } ],
  }
      ...
      for (const key in crossObj) {
        if (key === 'coachaccount') {
            if (!amountFieldName || key !== amountFieldName) {
              form.coachaccount=crossObj[key]
              read.coachaccount=true
            }
            continue
        }
        if (key === 'coachpassword') {
            if (!amountFieldName || key !== amountFieldName) {
              form.coachpassword=crossObj[key]
              read.coachpassword=true
            }
            continue
        }
        if (key === 'coachname') {
            if (!amountFieldName || key !== amountFieldName) {
              form.coachname=crossObj[key]
              read.coachname=true
            }
            continue
        }
        if (key === 'coachimage') {
            if (!amountFieldName || key !== amountFieldName) {
              form.coachimage=crossObj[key]
              read.coachimage=true
            }
            continue
        }
        if (key === 'sex') {
        ...
        if (key === 'coachlevel') {
            if (!amountFieldName || key !== amountFieldName) {
              form.coachlevel=crossObj[key]
              read.coachlevel=true
            }
            continue
        }
        if (key === 'workyears') {
            if (!amountFieldName || key !== amountFieldName) {
              form.workyears=crossObj[key]
              read.workyears=true
            }
            continue
        }
  ...
  const getInfo = async (itemId: number) => {
    try {
      const { data } = await http.get(`coach/info/${itemId}`)
      if (data){
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = id.value ? 'coach/update' : 'coach/save'
        ...
        if (crossObj) {
          ...
          if (statusColumnName && !statusColumnName.startsWith('[')) {
            ...
            if (crossrefid && crossuserid) {
              ...
              const { data } = await http.get('coach/page', { params })
