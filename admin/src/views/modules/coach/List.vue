  ...
  <div class="main-content bg-gray-50 p-6">
    ...
    <div class="bg-white p-4 rounded-lg shadow mb-4">
      <el-form :inline="true" :model="searchForm" class="search-form">
        <div class="inline-block mr-4">
            <label class="inline-block mr-2 leading-10">账号</label>
            <el-input
            v-model="searchForm.coachaccount"
            placeholder="账号"
            clearable
        ...
        <div class="inline-block mr-4">
            <label class="inline-block mr-2 leading-10">教练姓名</label>
            <el-input
        ...
        <div class="inline-block mr-4">
            <label class="inline-block mr-2 leading-10">性别</label>
            <div>
                <el-select v-model="searchForm.sex" placeholder="请选择性别" clearable class="w-40">
        ...
        <div class="inline-block mr-4">
            <label class="inline-block mr-2 leading-10">工号</label>
            <el-input
            v-model="searchForm.jobno"
            placeholder="工号"
            clearable
            @keydown.enter="search()"
            />
    ...
    <div v-if="!mapConfig.enabled || viewMode === 'table'" class="bg-white rounded-lg shadow">
      ...
      >
        ...
        <el-table-column prop="coachaccount" label="账号" />
        <el-table-column prop="coachname" label="教练姓名" />
        <el-table-column prop="coachimage" label="教练头像" width="200">
        <template #default="scope">
            <div v-if="scope.row.coachimage">
            <img
                v-if="scope.row.coachimage.substring(0,4)=='http'"
                :src="scope.row.coachimage.split(',')[0]"
                width="100"
                height="100"
                style="object-fit: cover"
                @click="imgPreView(scope.row.coachimage.split(',')[0])"
                class="cursor-pointer"
            >
            <img
                v-else
                :src="baseUrl + scope.row.coachimage.split(',')[0]"
                width="100"
                height="100"
                style="object-fit: cover"
                @click="imgPreView(baseUrl + scope.row.coachimage.split(',')[0])"
                class="cursor-pointer"
            >
            </div>
        ...
        <el-table-column prop="coachlevel" label="教练等级" />
...
<script setup lang="ts">
  ...
  import type { CoachItem } from '@/types/common'
  ...
  const chatBackendUserIdField = 'id'
  const chatBackendUserLabelField = 'coachaccount'
  const chatBackendTargetTable = 'coach'
  let websock: WebSocket | null = null

  // 响应式数据
  const dataList = ref<CoachItem[]>([])
  const dataListLoading = ref(false)
  const dataListSelections = ref<CoachItem[]>([])
  ...
  const musicTracks = computed(() => {
    ...
    return dataList.value
      .map(row => {
        ...
        return {
          id: record?.id ?? url,
          name: nameRaw || String(record?.name || '教练'),
          artist: artistRaw,
  ...
  const exportRows = computed(() =>
    dataList.value.map((row) => {
      ...
      if (plain.coachimage !== undefined && plain.coachimage !== null) {
        const images = String(plain.coachimage)
          .split(',')
          .map((item) => item.trim())
          .filter(Boolean)
          .map((item) => resolveFileUrl(item))
        plain.coachimage = images.join('; ')
      }
      return plain
    })
  )
  ...
  const getDataList = () => {
    ...
    if (searchForm.coachaccount) {
      params.coachaccount = '%' + searchForm.coachaccount + '%'
    }
    if (searchForm.coachname) {
      params.coachname = '%' + searchForm.coachname + '%'
    }
    if (searchForm.sex) {
      params.sex = '%' + searchForm.sex + '%'
    }
    if (searchForm.jobno) {
      params.jobno = '%' + searchForm.jobno + '%'
    }
    http.get('coach/page', { params }).then((response: any) => {
  ...
  // 重置
  const reset = () => {
    searchForm.coachaccount = undefined
    searchForm.coachname = undefined
    searchForm.sex = undefined
    searchForm.jobno = undefined
    pageIndex.value = 1
    getDataList()
  }
  ...
  // 多选
  const selectionChangeHandler = (val: CoachItem[]) => {
  ...
  const deleteHandler = async (ids?: number[]) => {
    ...
    try {
      ...
      await http.post('coach/delete', deleteIds)
  ...
  const inlineCrossActionHandler = async (
    ...
    try {
      ...
      if (statusColumnName && statusColumnValue) {
        ...
        await http.post(`coach/update`, curr)
