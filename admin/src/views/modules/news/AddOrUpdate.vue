<template>
  <div class="add-edit-dialog">
    <el-dialog
        v-model="visible"
        :title="id ? (type === 'info' ? '查看公告资讯' : '修改公告资讯') : '新增公告资讯'"
        width="60%"
    ...
    >
      <el-form
        ...
        class="space-y-4"
      >
        <el-form-item v-if="type === 'info' || true || read.title" label="标题" prop="title">
            <el-input
                v-model="form.title"
                placeholder="请输入标题"
                :readonly="type === 'info' || read.title"
                clearable
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.introduction" label="简介" prop="introduction">
            <el-input
                v-model="form.introduction"
                type="textarea"
                placeholder="请输入简介"
                :readonly="type === 'info' || read.introduction"
                clearable
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.typename" label="分类名称" prop="typename">
            <el-select v-model="form.typename" placeholder="请选择分类名称" :readonly="type === 'info' || read.typename">
                <el-option
        ...
        <el-form-item v-if="type === 'info' || true || read.name" label="发布人" prop="name">
            <el-input
                v-model="form.name"
                placeholder="请输入发布人"
        ...
        <el-form-item v-if="type === 'info' || true || read.avatar" label="头像" prop="avatar">
            <UploadField
                v-model="form.avatar"
                upload-type="image"
                :readonly="type === 'info' || read.avatar"
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.image" label="图片" prop="image">
            <UploadField
                v-model="form.image"
                upload-type="image"
        ...
        <el-form-item v-if="type === 'info' || true || read.content" label="内容" prop="content">
            <RichEditor
                    v-model="form.content"
                    :readonly="type === 'info' || read.content"
                    placeholder="请输入内容"
                    height="400px"
            />
        </el-form-item>
...
<script setup lang="ts">
  ...
  const init = async (itemId?: number, itemType?: string) => {
    ...
    if (type.value === 'cross') {
      ...
      for (const key in crossObj) {
        ...
        if (key === 'introduction') {
            if (!amountFieldName || key !== amountFieldName) {
              form.introduction=crossObj[key]
              read.introduction=true
            }
            continue
        }
        if (key === 'typename') {
            if (!amountFieldName || key !== amountFieldName) {
              form.typename=crossObj[key]
              read.typename=true
            }
            continue
        }
  ...
  const getInfo = async (itemId: number) => {
    try {
      const { data } = await http.get(`news/info/${itemId}`)
      if (data){
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = id.value ? 'news/update' : 'news/save'
        ...
        if (crossObj) {
          ...
          if (statusColumnName && !statusColumnName.startsWith('[')) {
            ...
            if (crossrefid && crossuserid) {
              ...
              const { data } = await http.get('news/page', { params })
