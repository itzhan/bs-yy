  ...
  <div class="main-content bg-gray-50 p-6">
    ...
    <div class="bg-white p-4 rounded-lg shadow mb-4">
      <el-form :inline="true" :model="searchForm" class="search-form">
        ...
        <div class="inline-block mr-4">
            <label class="inline-block mr-2 leading-10">分类名称</label>
            <div>
                <el-select
    ...
    <div v-if="!mapConfig.enabled || viewMode === 'table'" class="bg-white rounded-lg shadow">
      ...
      >
        ...
        <template #default="scope">
            ...
            <img
                v-if="scope.row.avatar.substring(0,4)=='http'"
                :src="scope.row.avatar.split(',')[0]"
                width="100"
                height="100"
                style="object-fit: cover"
                @click="imgPreView(scope.row.avatar.split(',')[0])"
                class="cursor-pointer"
            >
            <img
                v-else
                :src="baseUrl + scope.row.avatar.split(',')[0]"
                width="100"
                height="100"
                style="object-fit: cover"
                @click="imgPreView(baseUrl + scope.row.avatar.split(',')[0])"
                class="cursor-pointer"
            >
            </div>
        ...
        <template #default="scope">
            ...
            <img
                v-if="scope.row.image.substring(0,4)=='http'"
                :src="scope.row.image.split(',')[0]"
                width="100"
                height="100"
                style="object-fit: cover"
                @click="imgPreView(scope.row.image.split(',')[0])"
                class="cursor-pointer"
            >
            <img
                v-else
                :src="baseUrl + scope.row.image.split(',')[0]"
                width="100"
                height="100"
                style="object-fit: cover"
                @click="imgPreView(baseUrl + scope.row.image.split(',')[0])"
                class="cursor-pointer"
            >
            </div>
            <div v-else class="text-gray-400">无图片</div>
        ...
        <el-table-column prop="clicktime" label="最后点击时间" />
...
<script setup lang="ts">
  ...
  const dataListSelections = ref<NewsItem[]>([])
  ...
  const musicTracks = computed(() => {
    ...
    return dataList.value
      .map(row => {
        ...
        return {
          id: record?.id ?? url,
          name: nameRaw || String(record?.name || '公告资讯'),
          artist: artistRaw,
  ...
  const exportRows = computed(() =>
    dataList.value.map((row) => {
      ...
      if (plain.avatar !== undefined && plain.avatar !== null) {
        const images = String(plain.avatar)
          .split(',')
          .map((item) => item.trim())
          .filter(Boolean)
          .map((item) => resolveFileUrl(item))
        plain.avatar = images.join('; ')
      }
      if (plain.image !== undefined && plain.image !== null) {
        const images = String(plain.image)
          .split(',')
          .map((item) => item.trim())
          .filter(Boolean)
          .map((item) => resolveFileUrl(item))
        plain.image = images.join('; ')
      }
      return plain
    })
  )
  ...
  const getDataList = () => {
    ...
    http.get('news/page', { params }).then((response: any) => {
  ...
  // 多选
  const selectionChangeHandler = (val: NewsItem[]) => {
  ...
  const deleteHandler = async (ids?: number[]) => {
    ...
    try {
      ...
      await http.post('news/delete', deleteIds)
  ...
  const discussListHandler = (id: number) => {
    router.push({
      path: '/dashboard/modules/discussnews',
      query: { refid: id }
    })
  }
  ...
  const inlineCrossActionHandler = async (
    ...
    try {
      ...
      if (statusColumnName && statusColumnValue) {
        ...
        await http.post(`news/update`, curr)
  ...
  const auditBatchHandler = async () => {
    ...
    try {
      ...
      if (!valid) return
      await http.post(
              `news/audit/batch?auditstatus=${auditBatchForm.auditstatus}&auditreply=${auditBatchForm.auditreply}`,
              batchIds.value
      )
      getDataList()
  ...
  // 分类名称 选项数据
  const typenameOptions = ref<string[]>([])
  // 加载分类名称选项（支持条件联动）
  const getTypenameOptions = async () => {
    try {
      const params: any = {}
      const response = await http.get('option/newstype/typename', { params })
      if (response && response.code === 0) {
        typenameOptions.value = response.data || []
      } else {
        typenameOptions.value = []
      }
    } catch (error) {
      console.error('获取分类名称选项失败:', error)
      typenameOptions.value = []
    }
  }
