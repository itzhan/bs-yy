<template>
  <div class="add-edit-dialog">
    <el-dialog
        v-model="visible"
        :title="id ? (type === 'info' ? '查看好友表' : '修改好友表') : '新增好友表'"
        width="60%"
    ...
    >
      <el-form
        ...
        class="space-y-4"
      >
        <el-form-item v-if="type === 'info' || true || read.uid" label="用户ID" prop="uid">
            <el-input-number v-model="form.uid" placeholder="请输入用户ID" controls-position="right" :readonly="type === 'info' || read.uid"/>
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.fid" label="好友用户ID" prop="fid">
            <el-input-number v-model="form.fid" placeholder="请输入好友用户ID" controls-position="right" :readonly="type === 'info' || read.fid"/>
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.name" label="好友昵称" prop="name">
            <el-input
                v-model="form.name"
                placeholder="请输入好友昵称"
        ...
        <el-form-item v-if="type === 'info' || true || read.picture" label="头像" prop="picture">
            <UploadField
                v-model="form.picture"
                upload-type="image"
                :readonly="type === 'info' || read.picture"
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.role" label="角色标识" prop="role">
            <el-input
                v-model="form.role"
                placeholder="请输入角色标识"
                :readonly="type === 'info' || read.role"
                clearable
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.alias" label="好友备注" prop="alias">
            <el-input
                v-model="form.alias"
                placeholder="请输入好友备注"
                :readonly="type === 'info' || read.alias"
                clearable
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.type" label="关系类型" prop="type">
            <el-input-number v-model="form.type" placeholder="请输入关系类型" controls-position="right" :readonly="type === 'info' || read.type"/>
        </el-form-item>
...
<script setup lang="ts">
  ...
  const rules: Record<string, any[]> = {
    uid: [ { validator: validateNumber , trigger: 'blur' } ],
    fid: [ { validator: validateNumber , trigger: 'blur' } ],
    type: [ { validator: validateNumber , trigger: 'blur' } ],
  }
  ...
  const getInfo = async (itemId: number) => {
    try {
      const { data } = await http.get(`friend/info/${itemId}`)
      if (data){
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = id.value ? 'friend/update' : 'friend/save'
        ...
        if (crossObj) {
          ...
          if (statusColumnName && !statusColumnName.startsWith('[')) {
            ...
            if (crossrefid && crossuserid) {
              ...
              const { data } = await http.get('friend/page', { params })
