  ...
  <div class="main-content bg-gray-50 p-6">
    ...
    <div class="bg-white p-4 rounded-lg shadow mb-4">
      <el-form :inline="true" :model="searchForm" class="search-form">
        <div class="inline-block mr-4">
            <label class="inline-block mr-2 leading-10">用户ID</label>
            <el-input
        ...
        <div class="inline-block mr-4">
            <label class="inline-block mr-2 leading-10">好友用户ID</label>
            <el-input
        ...
        <div class="inline-block mr-4">
            <label class="inline-block mr-2 leading-10">好友昵称</label>
            <el-input
        ...
        <div class="inline-block mr-4">
            <label class="inline-block mr-2 leading-10">好友备注</label>
            <el-input
        ...
        <div class="inline-block mr-4">
            <label class="inline-block mr-2 leading-10">关系类型</label>
            <el-input
    ...
    <div v-if="!mapConfig.enabled || viewMode === 'table'" class="bg-white rounded-lg shadow">
      ...
      >
        ...
        <el-table-column prop="fid" label="好友用户ID" />
        <el-table-column prop="name" label="好友昵称" />
        ...
        <template #default="scope">
            ...
            <img
                ...
                @click="imgPreView(scope.row.picture.split(',')[0])"
                class="cursor-pointer"
            >
            <img
                v-else
                :src="baseUrl + scope.row.picture.split(',')[0]"
                width="100"
                height="100"
                style="object-fit: cover"
                @click="imgPreView(baseUrl + scope.row.picture.split(',')[0])"
                class="cursor-pointer"
            >
            </div>
        ...
        <el-table-column prop="alias" label="好友备注" />
...
<script setup lang="ts">
  ...
  import type { FriendItem } from '@/types/common'
  ...
  const chatBackendTargetTable = 'friend'
  let websock: WebSocket | null = null

  // 响应式数据
  const dataList = ref<FriendItem[]>([])
  const dataListLoading = ref(false)
  const dataListSelections = ref<FriendItem[]>([])
  ...
  const musicTracks = computed(() => {
    ...
    return dataList.value
      .map(row => {
        ...
        return {
          id: record?.id ?? url,
          name: nameRaw || String(record?.name || '好友表'),
          artist: artistRaw,
  ...
  const playMusic = (row: FriendItem) => {
  ...
  const exportRows = computed(() =>
    dataList.value.map((row) => {
      ...
      if (plain.picture !== undefined && plain.picture !== null) {
        const images = String(plain.picture)
          .split(',')
          .map((item) => item.trim())
          .filter(Boolean)
          .map((item) => resolveFileUrl(item))
        plain.picture = images.join('; ')
      }
      return plain
    })
  )
  ...
  const getDataList = () => {
    ...
    if (searchForm.alias) {
      params.alias = '%' + searchForm.alias + '%'
    }
    http.get('friend/page', { params }).then((response: any) => {
  ...
  const reset = () => {
    ...
    searchForm.fid = undefined
    searchForm.name = undefined
    searchForm.alias = undefined
  ...
  // 多选
  const selectionChangeHandler = (val: FriendItem[]) => {
  ...
  const deleteHandler = async (ids?: number[]) => {
    ...
    try {
      ...
      await http.post('friend/delete', deleteIds)
  ...
  const inlineCrossActionHandler = async (
    ...
    try {
      ...
      if (statusColumnName && statusColumnValue) {
        ...
        await http.post(`friend/update`, curr)
