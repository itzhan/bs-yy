<template>
  <div class="add-edit-dialog">
    <el-dialog
        v-model="visible"
        :title="id ? (type === 'info' ? '查看会员卡套餐' : '修改会员卡套餐') : '新增会员卡套餐'"
        width="60%"
      ...
      >
        <el-form-item v-if="type === 'info' || true || read.packagename" label="套餐名称" prop="packagename">
            <el-input
                ...
                placeholder="请输入套餐名称"
                :readonly="type === 'info' || read.packagename"
                clearable
            />
        </el-form-item>
        ...
        <el-form-item v-if="type === 'info' || true || read.packagetype" label="套餐类型" prop="packagetype">
            <el-select v-model="form.packagetype" placeholder="请选择套餐类型" :readonly="type === 'info' || read.packagetype">
        ...
        <el-form-item v-if="type === 'info' || true || read.packagedesc" label="套餐说明" prop="packagedesc">
            <el-input
                v-model="form.packagedesc"
                type="textarea"
                placeholder="请输入套餐说明"
                :readonly="type === 'info' || read.packagedesc"
                clearable
            />
        </el-form-item>
...
<script setup lang="ts">
  ...
  const rules: Record<string, any[]> = {
    ...
    packagetype: [ { trigger: 'blur' , message: '套餐类型不能为空' , required: true } ],
    packageprice: [ { trigger: 'blur' , message: '价格不能为空' , required: true }, { validator: validateNumber , trigger: 'blur' } ],
    validdays: [ { trigger: 'blur' , message: '有效期不能为空' , required: true }, { validator: validateNumber , trigger: 'blur' } ],
    includedcourses: [ { trigger: 'blur' , message: '包含课时不能为空' , required: true }, { validator: validateNumber , trigger: 'blur' } ],
  }
  ...
  const init = async (itemId?: number, itemType?: string) => {
    ...
    if (type.value === 'cross') {
      ...
      for (const key in crossObj) {
        ...
        if (key === 'packagedesc') {
            if (!amountFieldName || key !== amountFieldName) {
              form.packagedesc=crossObj[key]
              read.packagedesc=true
            }
            continue
        }
      }
  ...
  const getInfo = async (itemId: number) => {
    try {
      const { data } = await http.get(`membershippackage/info/${itemId}`)
      if (data){
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = id.value ? 'membershippackage/update' : 'membershippackage/save'
        ...
        if (crossObj) {
          ...
          if (statusColumnName && !statusColumnName.startsWith('[')) {
            ...
            if (crossrefid && crossuserid) {
              ...
              const { data } = await http.get('membershippackage/page', { params })
