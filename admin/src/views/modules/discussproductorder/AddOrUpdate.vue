<template>
  <div class="add-edit-dialog">
    <el-dialog
        v-model="visible"
        :title="id ? (type === 'info' ? '查看商品订单评论表' : '修改商品订单评论表') : '新增商品订单评论表'"
        width="60%"
    ...
    >
      <el-form
        ...
        class="space-y-4"
      >
        <el-form-item v-if="type === 'info' || true || read.refid" label="关联表id" prop="refid">
            <el-input-number v-model="form.refid" placeholder="请输入关联表id" controls-position="right" :readonly="type === 'info' || read.refid"/>
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.nickname" label="用户名" prop="nickname">
            <UploadField
                v-model="form.nickname"
                upload-type="image"
                :readonly="type === 'info' || read.nickname"
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.avatarurl" label="头像" prop="avatarurl">
            <el-input
                v-model="form.avatarurl"
                placeholder="请输入头像"
                :readonly="type === 'info' || read.avatarurl"
                clearable
            />
        </el-form-item>
        <el-form-item v-if="type === 'info' || true || read.content" label="评论内容" prop="content">
            <el-input
        ...
        <el-form-item v-if="type === 'info' || true || read.reply" label="回复内容" prop="reply">
            <el-input
                v-model="form.reply"
                placeholder="请输入回复内容"
...
<script setup lang="ts">
  ...
  const rules: Record<string, any[]> = {
    refid: [ { validator: validateNumber , trigger: 'blur' } ],
  }
  ...
  const getInfo = async (itemId: number) => {
    try {
      const { data } = await http.get(`discussproductorder/info/${itemId}`)
      if (data){
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = id.value ? 'discussproductorder/update' : 'discussproductorder/save'
        ...
        if (crossObj) {
          ...
          if (statusColumnName && !statusColumnName.startsWith('[')) {
            ...
            if (crossrefid && crossuserid) {
              ...
              const { data } = await http.get('discussproductorder/page', { params })
