...
import type { DiscussproductorderItem } from '@/types/common'
import { useRoute } from 'vue-router'

const route = useRoute()

// 响应式数据
const dataList = ref<DiscussproductorderItem[]>([])
...
const currentReplyComment = ref<DiscussproductorderItem>({} as DiscussproductorderItem)
// 回复表单
...
const deleteHandler = async (ids?: number[]) => {
  ...
  try {
    ...
    await http.post('discussproductorder/delete', deleteIds)
    await getDataList()
  } catch {
    // 用户取消操作
  }
...
// 回复处理
const replyHandler = (comment: DiscussproductorderItem) => {
...
const submitReply = async () => {
  ...
  try {
    ...
    await http.post('discussproductorder/update', updateData)
