<template>
  <div class="add-edit-dialog">
    <el-dialog
        v-model="visible"
        :title="id ? (type === 'info' ? '查看交流分类' : '修改交流分类') : '新增交流分类'"
        width="60%"
    ...
    >
      <el-form
        ...
        class="space-y-4"
      >
        <el-form-item v-if="type === 'info' || true || read.sharetypename" label="分类名称" prop="sharetypename">
            <el-input
                v-model="form.sharetypename"
                placeholder="请输入分类名称"
                :readonly="type === 'info' || read.sharetypename"
                clearable
            />
        </el-form-item>
...
<script setup lang="ts">
  ...
  const rules: Record<string, any[]> = {
    sharetypename: [ { trigger: 'blur' , message: '分类名称不能为空' , required: true } ],
  }
  ...
  const init = async (itemId?: number, itemType?: string) => {
    ...
    if (type.value === 'cross') {
      ...
      for (const key in crossObj) {
        if (key === 'sharetypename') {
            if (!amountFieldName || key !== amountFieldName) {
              form.sharetypename=crossObj[key]
              read.sharetypename=true
            }
            continue
        }
      }
  ...
  const getInfo = async (itemId: number) => {
    try {
      const { data } = await http.get(`sharetype/info/${itemId}`)
      if (data){
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = id.value ? 'sharetype/update' : 'sharetype/save'
        ...
        if (crossObj) {
          ...
          if (statusColumnName && !statusColumnName.startsWith('[')) {
            ...
            if (crossrefid && crossuserid) {
              ...
              const { data } = await http.get('sharetype/page', { params })
