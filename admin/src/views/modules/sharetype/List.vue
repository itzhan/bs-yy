  ...
  <div class="main-content bg-gray-50 p-6">
    ...
    <div class="bg-white p-4 rounded-lg shadow mb-4">
      <el-form :inline="true" :model="searchForm" class="search-form">
        <div class="inline-block mr-4">
            ...
            <el-input
            v-model="searchForm.sharetypename"
            placeholder="分类名称"
            clearable
    ...
    <div class="bg-white p-4 rounded-lg shadow mb-4">
      <div class="flex justify-between items-center">
        <div class="space-x-2">
          <el-button
            v-if="!false && isAuth('sharetype', '新增')"
            type="success"
          ...
          <el-button
            v-if="!false && isAuth('sharetype', '删除')"
            type="danger"
    ...
    <div v-if="!mapConfig.enabled || viewMode === 'table'" class="bg-white rounded-lg shadow">
      ...
      >
        ...
        <el-table-column prop="sharetypename" label="分类名称" />
        <!-- 操作列 -->
...
<script setup lang="ts">
  ...
  import type { SharetypeItem } from '@/types/common'
  ...
  // 响应式数据
  const dataList = ref<SharetypeItem[]>([])
  const dataListLoading = ref(false)
  const dataListSelections = ref<SharetypeItem[]>([])
  ...
  const musicTracks = computed(() => {
    ...
    return dataList.value
      .map(row => {
        ...
        return {
          id: record?.id ?? url,
          name: nameRaw || String(record?.name || '交流分类'),
          artist: artistRaw,
  ...
  const playMusic = (row: SharetypeItem) => {
  ...
  const getDataList = () => {
    ...
    if (searchForm.sharetypename) {
      params.sharetypename = '%' + searchForm.sharetypename + '%'
    }
    http.get('sharetype/page', { params }).then((response: any) => {
  ...
  // 重置
  const reset = () => {
    searchForm.sharetypename = undefined
    pageIndex.value = 1
    getDataList()
  }
  ...
  // 多选
  const selectionChangeHandler = (val: SharetypeItem[]) => {
  ...
  const deleteHandler = async (ids?: number[]) => {
    ...
    try {
      ...
      await http.post('sharetype/delete', deleteIds)
  ...
  const inlineCrossActionHandler = async (
    ...
    try {
      ...
      if (statusColumnName && statusColumnValue) {
        ...
        await http.post(`sharetype/update`, curr)
